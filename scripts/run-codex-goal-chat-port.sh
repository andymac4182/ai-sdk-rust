...
worktree_root="/Users/andrewmcclenaghan/dev/andymac4182/ai-sdk-rust-chat-worktrees"
goal_file="$main_repo/scripts/codex-goal-chat/goal-condition.md"
stamp="$(date +%Y%m%d-%H%M%S)"
branch="goal/chat-sdk-port-$stamp"
worktree="$worktree_root/chat-sdk-port-$stamp"
tmux_session="chat_sdk_rust_goal_$stamp"
...
if [[ -n "$(git status --short)" ]]; then
  echo "ai-sdk-rust main checkout is dirty. Commit or stash changes before launching the chat-sdk Codex goal." >&2
  git status --short >&2
  exit 1
fi
...
echo "Created chat-sdk Codex goal worktree: $worktree"
echo "Branch: $branch"
echo "Shared merge lock with ai-sdk session: /tmp/ai-sdk-rust-main-merge.lock"
