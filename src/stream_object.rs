...
mod tests {
    ...
    #[test]
    fn stream_object_calls_globally_registered_integration_listeners() {
        ...
        register_telemetry_integration(
            TelemetryIntegration::new()
                ...
                .with_callback(TelemetryEventKind::OnObjectStepFinish, move |_| {
                    if !global_step_finish_seen_for_callback.swap(true, Ordering::SeqCst) {
                        global_step_finish_events
                            .lock()
                            .expect("telemetry event lock")
                            .push("global-onObjectStepFinish".to_string());
                    }
                })
        ...
        assert_eq!(
            ...
            [
                ...
                "global-onObjectStepFinish".to_string(),
    ...
    #[test]
    fn stream_object_prefers_per_call_integrations_over_global_integrations() {
        let _guard = telemetry_test_guard_for_tests();
        reset_telemetry_state_for_tests();
        let model =
            MockLanguageModel::new().with_stream_result(LanguageModelStreamResult::new(vec![
        ...
        register_telemetry_integration(TelemetryIntegration::new().with_callback(
            TelemetryEventKind::OnStart,
            move |event| {
                if event.function_id.as_deref() == Some("stream-object-per-call-precedence") {
                    global_events
                        .lock()
                        .expect("telemetry event lock")
        ...
        let result = poll_ready(stream_object(
            StreamObjectOptions::new(&model, prompt())
                ...
                .with_telemetry(
                    TelemetryOptions::new()
                        .with_function_id("stream-object-per-call-precedence")
    ...
    #[test]
    fn stream_object_calls_integration_listeners_alongside_user_callbacks() {
        let model =
            MockLanguageModel::new().with_stream_result(LanguageModelStreamResult::new(vec![
                LanguageModelStreamPart::TextDelta(LanguageModelTextDelta::new(
                    "1",
        ...
        let integration = TelemetryIntegration::new()
            ...
            .with_callback(TelemetryEventKind::OnObjectStepFinish, move |_| {
                integration_step_finish_events
                    .lock()
                    .expect("telemetry event lock")
                    .push("integration-onObjectStepFinish".to_string());
            })
            .with_callback(TelemetryEventKind::OnEnd, move |_| {
        ...
        assert_eq!(
            ...
            [
                ...
                "integration-onObjectStepFinish".to_string(),
    ...
    #[test]
    fn stream_object_does_not_break_when_integration_listener_panics() {
        let model =
            MockLanguageModel::new().with_stream_result(LanguageModelStreamResult::new(vec![
                LanguageModelStreamPart::TextDelta(LanguageModelTextDelta::new(
                    "1",
        ...
        let integration = TelemetryIntegration::new()
            ...
            .with_callback(TelemetryEventKind::OnObjectStepFinish, |_| {
                panic!("integration error");
            })
    ...
    #[test]
    fn stream_object_supports_multiple_per_call_telemetry_integrations_as_array() {
        let model =
            MockLanguageModel::new().with_stream_result(LanguageModelStreamResult::new(vec![
                LanguageModelStreamPart::TextDelta(LanguageModelTextDelta::new(
                    "1",
