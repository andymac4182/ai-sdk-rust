...
| `packages/hume` (`@ai-sdk/hume`) | provider package | in-progress | `crates/ai-sdk-hume` | `hume_provider_creates_speech_model_with_headers_options_and_body`; `hume_speech_model_defaults_voice_format_and_warns_for_unsupported_inputs`; `hume_speech_model_uses_real_date_when_no_custom_date_provider_is_specified`; `hume_speech_model_handles_different_audio_formats`; `hume_speech_model_includes_warnings_if_any_are_generated`; `hume_speech_model_maps_generation_context_and_api_errors_to_metadata`; `hume_error_data_schema_parses_hume_resource_exhausted_error`; `hume_provider_reports_unsupported_model_families_and_trait_speech`; `hume_provider_settings_serde_accepts_upstream_shape`; `hume_default_factory_creates_speech_model` | Initial provider-owned crate mirrors upstream `createHume` settings for `HUME_API_KEY`, `X-Hume-Api-Key`, custom headers, Hume user-agent suffix, default `hume()` speech factory, provider-v4 trait integration, unsupported language/embedding/image lookups, `/v0/tts/file` JSON request shaping, upstream default voice id, `mp3`/`pcm`/`wav` output formats, unsupported output-format and language warnings, provider `context.generationId` and context utterance mapping, binary audio response mapping, response headers, timestamp/model metadata, and Hume error metadata. The remaining Hume gap is live-provider validation; the deterministic upstream speech/error inventory is now mapped. |
