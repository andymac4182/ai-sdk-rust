...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_available_models` | Minimal provider settings, `create_gateway`, `gateway`, non-streaming language model calls, SSE streaming, raw-chunk filtering, model request headers, `get_available_models`, `get_credits`, `get_spend_report`, and `get_generation_info` are ported. Model-family helpers, metadata cache refresh, and gateway error classes remain unported. |
...
3. Expand Gateway beyond language model generation, streaming, and account
   metadata APIs: metadata cache refresh, embedding/image/reranking/video
