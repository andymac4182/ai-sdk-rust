...
| `packages/google` (`@ai-sdk/google`) | provider package | in-progress | `crates/ai-sdk-google`; `docs/ai-foundational-provider-inventory.md` | `google_foundational_inventory_tracks_current_upstream_cases`; `google_foundational_inventory_has_no_ported_cases_yet`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | AI-01 inventory slice created the package-owned crate boundary and row-level current-upstream inventory for 50 source files, 36 fixture/snapshot files, 21 test files, and 568 upstream test cases. The inventory records 566 portable unmapped cases and 2 `type-system-impossible` TypeScript compile-error exceptions. No Google runtime behavior is claimed yet. Remaining work: AI-01C must port Gemini language, embedding, image, video, files, interactions, schema conversion, URL support, tool preparation, JSON accumulator behavior, deterministic fixtures, and ignored live-provider proof. |
