...
| `packages/xai` (`@ai-sdk/xai`) | provider package | in-progress | `src/xai.rs`, `src/openai_compatible.rs`, `src/open_responses.rs` | `xai_provider_creates_responses_model_with_headers_base_url_and_body`; `xai_responses_model_prepares_server_tools_custom_tool_and_usage`; `xai_provider_creates_chat_model_with_openai_compatible_transport`; `xai_provider_creates_image_model_and_reports_unsupported_embeddings`; `xai_provider_settings_serde_accepts_upstream_base_url`; `xai_provider_implements_provider_trait` | Initial xAI provider foundation mirrors upstream settings, `XAI_API_KEY`, base URL defaults, custom headers, user-agent suffix, callable-style `xai(...)`, provider-v4 trait integration, Responses model construction through the shared Open Responses transport, shared Open Responses server-tool/custom-tool preparation and usage conversion, OpenAI-compatible chat construction, image model construction, and unsupported embeddings. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; video, files, xAI-specific provider tool IDs/conversion helpers, xAI chat usage/message conversion, full image generation behavior, and broader Responses streaming/error/provider-metadata behavior remain open. |
