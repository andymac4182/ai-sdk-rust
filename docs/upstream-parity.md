...
| `packages/fireworks` (`@ai-sdk/fireworks`) | provider package | in-progress | `src/fireworks.rs`, `src/openai_compatible.rs` | `fireworks_provider_creates_chat_model_with_transformed_provider_options`; `fireworks_provider_creates_completion_embedding_and_image_models`; `fireworks_image_model_sends_workflow_request_and_returns_binary`; `fireworks_image_model_sends_async_edit_input_image_and_warnings`; `fireworks_image_model_maps_image_generation_size_and_aspect_warning`; `fireworks_image_model_aborts_before_request`; `fireworks_provider_uses_default_base_url_and_function_alias`; `fireworks_provider_implements_provider_trait`; `fireworks_provider_settings_serde_accepts_upstream_base_url` | Initial Fireworks provider foundation mirrors upstream settings, `FIREWORKS_API_KEY`, base URL defaults, custom headers, user-agent suffix, callable-style `fireworks(...)`, provider-v4 trait integration, OpenAI-compatible chat/completion/embedding model construction, Fireworks reasoning option transform, Fireworks error message extraction, and provider-specific image routes for `/workflows/.../text_to_image`, `/workflows/...`, `/workflows/.../get_result`, and `/image_generation/...` with provider options, input-image data URIs, binary image responses, warnings, and abort propagation. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; exact factory split parity, polling delay timing, thrown-error class identity, and live/API behavior remain open. |
