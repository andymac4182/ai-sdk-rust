...
| Language model middleware: wrap/default settings | verified | `src/language_model_middleware.rs` | `wrap_language_model_*`; `default_settings_middleware_*`; `add_tool_input_examples_middleware_*` tests | Mirrors upstream v4 model-level hooks plus default settings and tool input example description transforms. |
...
2. Continue `streamObject` parity with repair callbacks, telemetry,
   ...
   edge cases.
3. Port extract-json, extract-reasoning, and simulate-streaming middleware.
