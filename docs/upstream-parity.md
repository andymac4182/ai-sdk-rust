...
| --- | --- | --- | --- | --- | --- |
| `packages/ai` (`ai`) | core package | in-progress | `src/lib.rs`, `src/generate_text.rs`, `src/generate_object.rs`, `src/embed.rs`, `src/generate_image.rs`, `src/generate_speech.rs`, `src/generate_video.rs`, `src/transcribe.rs`, `src/rerank.rs`, `src/upload_file.rs`, `src/upload_skill.rs`, `src/registry.rs`, `src/provider_middleware.rs`, `src/mock_models.rs` | Unit tests in matching modules; `mock_models::tests::*`; `examples/kitchen_sink.rs` | Non-streaming generation, object, image, speech, video, transcription, embeddings, reranking, upload, registry, provider-level middleware wrapping, tool input examples middleware, extract JSON middleware, simulate streaming middleware, tool loops, public mock provider-v4 models, and many provider-v4 shapes exist. Agent, stream APIs, UI-message APIs, telemetry, logger, and text-stream response helpers remain unported. |
...
1. Continue `streamText` parity with multi-step streamed tool execution,
   callbacks, aborts/retries, text/UI response helpers, error
   propagation, and stop conditions.
2. Continue `streamObject` parity with repair callbacks, telemetry,
   retries/abort, text response helpers, and remaining partial-output strategy
   edge cases.
