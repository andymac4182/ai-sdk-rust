...
| `packages/gateway` (`@ai-sdk/gateway`) | provider package | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_model_filters_raw_stream_parts_unless_requested`; `gateway_embedding_model_embeds_through_embed`; `gateway_embedding_model_maps_gateway_error_to_metadata`; `gateway_image_model_generates_through_generate_image`; `gateway_image_model_encodes_files_and_mask`; `gateway_image_model_maps_gateway_error_to_metadata`; `gateway_provider_creates_embedding_model_aliases`; `gateway_provider_creates_image_model_aliases`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_refreshes_available_models_when_cache_disabled`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; `gateway_model_maps_gateway_error_to_error_finish_reason`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_openai_embed`; ignored `live_gateway_openai_generate_image`; ignored `live_gateway_available_models` | Native AI SDK Gateway language model generation, streaming, embedding generation, image generation, available-model metadata discovery with refresh cache, credit balance, spend report, and generation info slices exist for `gateway("openai/...")` with fake-transport tests and optional `.env.local` live OpenAI model/embedding/image/metadata validation. Reranking/video model contracts and gateway-specific error types remain unported. |
...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_embedding_model_embeds_through_embed`; `gateway_image_model_generates_through_generate_image`; `gateway_image_model_encodes_files_and_mask`; `gateway_provider_creates_embedding_model_aliases`; `gateway_provider_creates_image_model_aliases`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_refreshes_available_models_when_cache_disabled`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_openai_embed`; ignored `live_gateway_openai_generate_image`; ignored `live_gateway_available_models` | Minimal provider settings, `create_gateway`, `gateway`, non-streaming language model calls, SSE streaming, embedding model calls, image model calls, raw-chunk filtering, model request headers, cached `get_available_models`, `get_credits`, `get_spend_report`, and `get_generation_info` are ported. Reranking/video model-family helpers and gateway error classes remain unported. |
...
3. Expand Gateway beyond language model generation, streaming, image/embedding,
   and account metadata APIs: reranking/video models, gateway-specific errors,
   and broader provider package tests.
