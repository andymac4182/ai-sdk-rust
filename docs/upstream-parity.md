...
| `packages/gateway` (`@ai-sdk/gateway`) | provider package | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_model_filters_raw_stream_parts_unless_requested`; `gateway_embedding_model_embeds_through_embed`; `gateway_embedding_model_maps_gateway_error_to_metadata`; `gateway_image_model_generates_through_generate_image`; `gateway_image_model_encodes_files_and_mask`; `gateway_image_model_maps_gateway_error_to_metadata`; `gateway_reranking_model_reranks_through_rerank`; `gateway_reranking_model_omits_optional_body_fields`; `gateway_reranking_model_maps_gateway_error_to_metadata`; `gateway_video_model_generates_through_generate_video`; `gateway_video_model_encodes_image_inputs_and_returns_url_videos`; `gateway_video_model_maps_sse_error_to_metadata`; `gateway_provider_creates_embedding_model_aliases`; `gateway_provider_creates_image_model_aliases`; `gateway_provider_creates_reranking_model_aliases`; `gateway_provider_creates_video_model_aliases`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_refreshes_available_models_when_cache_disabled`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; `gateway_model_maps_gateway_error_to_error_finish_reason`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_openai_embed`; ignored `live_gateway_openai_generate_image`; ignored `live_gateway_rerank`; ignored `live_gateway_generate_video`; ignored `live_gateway_available_models` | Native AI SDK Gateway language model generation, streaming, embedding generation, image generation, reranking, video generation, available-model metadata discovery with refresh cache, credit balance, spend report, and generation info slices exist for `gateway("openai/...")` plus Gateway model-family helpers with fake-transport tests and optional `.env.local` live model/embedding/image/reranking/video/metadata validation. Gateway-specific error types and broader provider package tests remain unported. |
...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_embedding_model_embeds_through_embed`; `gateway_image_model_generates_through_generate_image`; `gateway_image_model_encodes_files_and_mask`; `gateway_reranking_model_reranks_through_rerank`; `gateway_reranking_model_omits_optional_body_fields`; `gateway_video_model_generates_through_generate_video`; `gateway_video_model_encodes_image_inputs_and_returns_url_videos`; `gateway_provider_creates_embedding_model_aliases`; `gateway_provider_creates_image_model_aliases`; `gateway_provider_creates_reranking_model_aliases`; `gateway_provider_creates_video_model_aliases`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_refreshes_available_models_when_cache_disabled`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_openai_embed`; ignored `live_gateway_openai_generate_image`; ignored `live_gateway_rerank`; ignored `live_gateway_generate_video`; ignored `live_gateway_available_models` | Minimal provider settings, `create_gateway`, `gateway`, non-streaming language model calls, SSE streaming, embedding model calls, image model calls, reranking model calls, video model calls, raw-chunk filtering, model request headers, cached `get_available_models`, `get_credits`, `get_spend_report`, and `get_generation_info` are ported. Gateway error classes and broader provider package tests remain unported. |
...
3. Expand Gateway beyond language model generation, streaming,
   image/embedding/reranking/video, and account metadata APIs:
   gateway-specific errors and broader provider package tests.
