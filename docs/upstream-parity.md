...
| `packages/amazon-bedrock` (`@ai-sdk/amazon-bedrock`) | provider package | in-progress | `crates/ai-sdk-amazon-bedrock`; `docs/ai-foundational-provider-inventory.md` | `amazon_bedrock_foundational_inventory_tracks_current_upstream_cases`; `amazon_bedrock_foundational_inventory_has_no_ported_cases_yet`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | AI-01 inventory slice created the package-owned crate boundary and row-level current-upstream inventory for 31 source files, 25 fixture/snapshot files, 17 test files, and 383 upstream test cases. The inventory records 380 portable unmapped cases and 3 `js-only-documented` callable-constructor exceptions. No Bedrock runtime behavior is claimed yet. Remaining work: AI-01B must port chat, Anthropic-on-Bedrock, embeddings, image, reranking, event-stream response handling, SigV4/API-key fetch wrappers, tool preparation, usage conversion, model settings, deterministic fixtures, and ignored live-provider proof. |
