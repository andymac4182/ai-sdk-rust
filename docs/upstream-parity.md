...
| `packages/alibaba` (`@ai-sdk/alibaba`) | provider package | in-progress | `crates/ai-sdk-alibaba` | `alibaba_chat_model_builds_request_body_with_options_reasoning_tools_cache_and_usage`; `alibaba_chat_model_converts_multimodal_assistant_and_tool_messages`; `alibaba_chat_model_streams_reasoning_text_tool_calls_usage_and_raw_chunks`; `alibaba_video_model_generates_video_with_headers_body_and_metadata`; `alibaba_video_model_maps_i2v_r2v_resolution_and_warnings`; `alibaba_video_model_maps_api_and_status_errors_to_metadata`; `alibaba_provider_reports_unsupported_model_families_and_trait_video`; `alibaba_provider_settings_serde_accepts_upstream_shape` | Initial provider-owned crate mirrors upstream `createAlibaba` settings for default/custom chat and video base URLs, `ALIBABA_API_KEY`, custom headers, Alibaba user-agent suffix, `alibaba()`/`create_alibaba`, `language_model`/`chat`/`chat_model`, `video`/`video_model`, provider-v4 and `ProviderWithVideoModel` integration, and unsupported embedding/image lookups. Current upstream package snapshot has 6 deterministic test files and 87 `it` cases: `alibaba-provider.test.ts` maps provider construction, include-usage, custom URL/header, chat, language-model, video, and video-model cases to the provider/settings tests plus the chat/video request tests; `alibaba-chat-language-model.test.ts`, fixtures, and snapshots map request body, text/tool/reasoning extraction, usage, cache tokens, thinking options, and SSE text/tool/reasoning chunks to the chat request, message-conversion, and stream tests; `convert-to-alibaba-chat-messages.test.ts` maps system/user/assistant/tool message conversion, image URL/data conversion, cache-control placement, and provider-reference rejection to the chat request and message-conversion tests; `convert-alibaba-usage.test.ts` maps prompt/completion/cache write/cache read/no-cache/reasoning token conversion to the chat request and stream tests; `get-cache-control.test.ts` maps metadata extraction, missing metadata, and the over-4-breakpoint warning to the chat request and message-conversion tests; `alibaba-video-model.test.ts` maps provider/model identity, T2V/I2V/R2V request shaping, resolution conversion, headers, provider options, warnings, polling, success metadata, and API/status errors to the video request, resolution/warning, and error tests. Remaining work before `verified`: split any aggregate mappings that need one-to-one row names, add ignored credential-gated live Alibaba proof, and document or close JavaScript-only edges for callable/new constructor identity, function-valued header callbacks, exact thrown `AISDKError` class and fake-timer/abort timing semantics, workflow serialization hooks, and zod/runtime passthrough validation beyond typed Rust provider options. |
