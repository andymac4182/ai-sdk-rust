...
| `packages/cohere` (`@ai-sdk/cohere`) | provider package | in-progress | `src/cohere.rs` | `cohere_provider_creates_embedding_model_with_options_headers_and_usage`; `cohere_embedding_model_uses_default_input_type_and_exposes_raw_response`; `cohere_provider_creates_reranking_model_with_object_warning_and_options`; `cohere_reranking_model_sends_text_documents_without_warnings`; `cohere_provider_reports_chat_and_image_exceptions`; `cohere_provider_implements_embedding_and_reranking_traits`; `cohere_provider_settings_serde_accepts_upstream_base_url` | Initial Cohere provider foundation mirrors upstream settings, `COHERE_API_KEY`, base URL defaults, bearer auth, custom headers, user-agent suffix, embedding and reranking provider-v4 trait integration, `/embed` request options, default input type, output dimension, usage mapping, response headers/raw body preservation, `/rerank` request options, text and object documents, object-document compatibility warning, ranking/response mapping, API error metadata, and explicit unsupported chat/image model errors. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; Cohere chat, prompt conversion, tool preparation, streaming/citations, full validation, chunk-limit errors, and broader reranking/embedding edge parity remain open. |
