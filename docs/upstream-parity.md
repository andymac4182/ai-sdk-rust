...
7. Continue `packages/ai` utility/core parity from the upstream corpus.
   ...
   and queued run calls preserving submission order. Continue from the
   remaining utility corpus such as stream/stitching/download/runtime-specific
   helpers, documenting JavaScript-only stream/server-response cases where Rust
   has no equivalent runtime boundary.
