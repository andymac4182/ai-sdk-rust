...
| `packages/gateway` (`@ai-sdk/gateway`) | provider package | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_model_filters_raw_stream_parts_unless_requested`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_model_maps_gateway_error_to_error_finish_reason`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_available_models` | Native AI SDK Gateway language model generation, streaming, available-model metadata discovery, and credit balance slices exist for `gateway("openai/...")` with fake-transport tests and optional `.env.local` live OpenAI model/metadata validation. Spend report, generation info, embedding/image/reranking/video model contracts, metadata cache refresh, and gateway-specific error types remain unported. |
...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_fetches_credits_from_gateway_origin`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_available_models` | Minimal provider settings, `create_gateway`, `gateway`, non-streaming language model calls, SSE streaming, raw-chunk filtering, model request headers, `get_available_models`, and `get_credits` are ported. Spend reports, generation info, model-family helpers, metadata cache refresh, and gateway error classes remain unported. |
...
3. Expand Gateway beyond language model generation, streaming, metadata
   discovery, and credits: spend reports, generation info, metadata cache refresh,
   embedding/image/reranking/video models, gateway-specific errors, and broader
