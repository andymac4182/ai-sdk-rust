...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs`, `src/gateway_error.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_embedding_model_embeds_through_embed`; `gateway_image_model_generates_through_generate_image`; `gateway_image_model_encodes_files_and_mask`; `gateway_reranking_model_reranks_through_rerank`; `gateway_reranking_model_omits_optional_body_fields`; `gateway_video_model_generates_through_generate_video`; `gateway_video_model_encodes_image_inputs_and_returns_url_videos`; `gateway_provider_creates_embedding_model_aliases`; `gateway_provider_creates_image_model_aliases`; `gateway_provider_creates_reranking_model_aliases`; `gateway_provider_creates_video_model_aliases`; `get_gateway_auth_token_matches_upstream_precedence`; `get_gateway_auth_token_ignores_empty_values_without_trimming_whitespace`; `gateway_provider_headers_support_oidc_auth_method`; `gateway_observability_headers_map_vercel_environment`; `gateway_observability_headers_skip_empty_values_and_use_request_env_fallback`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_refreshes_available_models_when_cache_disabled`; `gateway_provider_fetches_credits_from_gateway_origin`; `gateway_provider_fetches_spend_report_with_query_params`; `gateway_provider_fetches_generation_info_and_unwraps_data`; `gateway_provider_metadata_surfaces_api_errors`; `gateway_error_types_expose_upstream_names_status_and_retryability`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text`; ignored `live_gateway_openai_embed`; ignored `live_gateway_openai_generate_image`; ignored `live_gateway_rerank`; ignored `live_gateway_generate_video`; ignored `live_gateway_available_models` | Minimal provider settings, `create_gateway`, deprecated `create_gateway_provider` alias, `gateway`, non-streaming language model calls, SSE streaming, embedding model calls, image model calls, reranking model calls, video model calls, raw-chunk filtering, model request headers, API-key/OIDC auth resolution, Vercel observability headers, Gateway error classification, cached `get_available_models`, `get_credits`, `get_spend_report`, and `get_generation_info` are ported. Broader provider package tests remain unported. |
...
3. Expand Gateway beyond the current language, streaming,
   image/embedding/reranking/video, account metadata, auth/observability, and
   error-classification slices with provider-executed tools and broader
