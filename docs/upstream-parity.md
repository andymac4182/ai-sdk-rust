...
| Provider registry | in-progress | `src/registry.rs` | `create_provider_registry_*` tests; `create_provider_registry_should_wrap_all_language_models_accessed_through_the_provider_registry`; `create_provider_registry_should_wrap_all_image_models_accessed_through_the_provider_registry` | Core provider/model lookup, custom separators, missing-provider/model errors, files/skills interfaces, and language/image model middleware wrapping through registry lookup are covered. Remaining registry-family gap: broader `customProvider` alias/fallback/global-default-provider coverage. Gateway-specific registry helpers remain unported. |
