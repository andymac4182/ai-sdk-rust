...
| File upload: `uploadFile` | verified | `src/upload_file.rs` | `upload_file_*` tests; `upload_file_resolves_files_v4_from_provider_v4_with_files_method` | Mirrors upstream direct `FilesV4` upload behavior plus typed provider-v4 `files()` resolution through `upload_file_with_provider`. The upstream missing `files()` runtime throw is JavaScript dynamic object behavior; Rust callers use the `ProviderWithFiles` bound. Provider implementations remain unported. |
| Skill upload: `uploadSkill` | verified | `src/upload_skill.rs` | `upload_skill_*` tests; `upload_skill_resolves_skills_v4_from_provider_v4_with_skills_method` | Mirrors upstream direct `SkillsV4` upload behavior plus typed provider-v4 `skills()` resolution through `upload_skill_with_provider`. The upstream missing `skills()` runtime throw is JavaScript dynamic object behavior; Rust callers use the `ProviderWithSkills` bound. Provider implementations remain unported. |
...
- 2026-05-24: `packages/ai` `processUIMessageStream` data-part parity added
  ...
  `process_ui_message_stream_replaces_data_parts_with_matching_id`. Rust now
  persists `data-*` UI parts, omits transient data parts from message state,
  and replaces earlier data parts with the same id. The remaining wire-level
  dynamic `type: "data-*"` chunk deserialization gap is documented in the UI
  message stream row because Rust currently exposes a typed `Data` variant.
