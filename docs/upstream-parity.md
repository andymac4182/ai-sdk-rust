...
| `packages/gateway` (`@ai-sdk/gateway`) | provider package | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; `gateway_model_filters_raw_stream_parts_unless_requested`; `gateway_model_maps_gateway_error_to_error_finish_reason`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text` | Native AI SDK Gateway language model generation and streaming slices exist for `gateway("openai/...")` with fake-transport tests and optional `.env.local` live OpenAI model validation. Metadata, credits, spend report, embedding/image/reranking/video model contracts, and gateway-specific error types remain unported. |
...
| Gateway provider and metadata APIs | in-progress | `src/gateway.rs` | `gateway_model_generates_text_through_generate_text`; `gateway_model_streams_text_through_stream_text`; ignored `live_gateway_openai_generate_text`; ignored `live_gateway_openai_stream_text` | Minimal provider settings, `create_gateway`, `gateway`, non-streaming language model calls, SSE streaming, raw-chunk filtering, and model request headers are ported. Metadata discovery, credits, spend reports, generation info, model-family helpers, and gateway error classes remain unported. |
...
3. Expand Gateway beyond language model generation and streaming: metadata
