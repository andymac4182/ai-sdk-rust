...
| `packages/provider` | 1 | in-progress | Upstream `get-error-message.test.ts` now has a one-to-one Rust test split for every portable original case, including `get_error_message_returns_unknown_error_for_null`, `get_error_message_returns_unknown_error_for_undefined`, `get_error_message_returns_string_as_is`, `get_error_message_returns_empty_string_as_is`, `get_error_message_handles_custom_error_subclasses`, `get_error_message_respects_custom_to_string_overrides`, `get_error_message_json_stringifies_plain_objects`, `get_error_message_json_stringifies_numbers`, `get_error_message_json_stringifies_booleans`, and `get_error_message_json_stringifies_arrays`. The package row remains in progress while v2/v3 compatibility surfaces and exact stream abstractions remain unported. |
