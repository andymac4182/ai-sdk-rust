...
| `packages/google-vertex` (`@ai-sdk/google-vertex`) | provider package | in-progress | `crates/ai-sdk-google-vertex`; `docs/ai-foundational-provider-inventory.md` | `google_vertex_foundational_inventory_tracks_current_upstream_cases`; `google_vertex_foundational_inventory_has_no_ported_cases_yet`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | AI-01 inventory slice created the package-owned crate boundary and row-level current-upstream inventory for 38 source files, 5 fixture/snapshot files, 17 test files, and 204 upstream test cases. The inventory records 201 portable unmapped cases and 3 `js-only-documented` callable-constructor exceptions. No Vertex runtime behavior is claimed yet. Remaining work: AI-01D must port Vertex auth, provider base/edge variants, embedding, image, video, Anthropic-on-Vertex, MaaS, xAI-on-Vertex, deterministic fixtures, and ignored live-provider proof. |
