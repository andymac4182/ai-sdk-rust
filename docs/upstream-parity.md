...
| `streamObject` | in-progress | `src/stream_object.rs` | `stream_object_calls_model_with_json_response_format_and_standardized_prompt`; `stream_object_collects_partial_objects_text_and_finish_metadata`; `stream_object_array_output_formats_single_chunk_text_delta`; `stream_object_enum_output_completes_unambiguous_prefixes`; `stream_object_repairs_parse_failures`; `stream_object_repairs_schema_validation_failures`; `stream_object_keeps_original_error_when_repair_returns_none`; `stream_object_reports_repaired_text_error_when_repair_fails`; array/enum/error tests | Initial dependency-light collector over provider-v4 streams. It sends JSON response formats, accumulates text deltas, emits partial objects/text/full-stream parts, unwraps array/enum outputs, maps array output text deltas to upstream array syntax, streams complete array elements, applies enum prefix partial-output rules, exposes text response helpers, parses final objects, and applies upstream-style repair callbacks to final parse and validation failures. Telemetry, retries/abort, and remaining partial-output strategy edge cases remain unported. |
