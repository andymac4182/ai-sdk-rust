...
| `packages/anthropic` (`@ai-sdk/anthropic`) | provider package | in-progress | `crates/ai-sdk-anthropic`; `docs/ai-foundational-provider-inventory.md` | `anthropic_foundational_inventory_tracks_current_upstream_cases`; `anthropic_foundational_inventory_has_no_ported_cases_yet`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | AI-01 inventory slice created the package-owned crate boundary and row-level current-upstream inventory for 40 source files, 52 fixture/snapshot files, 13 test files, and 424 upstream test cases. The inventory records 418 portable unmapped cases and 6 `type-system-impossible` TypeScript generic-inference exceptions. No Anthropic runtime behavior is claimed yet. Remaining work: AI-01A must port language model, files, skills, cache control, prompt conversion, provider tools, usage conversion, error mapping, deterministic fixtures, and ignored live-provider proof. |
