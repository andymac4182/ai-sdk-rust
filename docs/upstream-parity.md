...
| `streamText` and language model streaming orchestration | in-progress | `src/stream_text.rs` | `stream_text_calls_language_model_do_stream_with_standardized_prompt`; `stream_text_collects_text_deltas_and_finish_metadata`; `stream_text_maps_tool_input_deltas_and_high_level_tool_outputs`; `stream_text_executes_local_tool_and_continues_to_final_text`; `stream_text_stops_after_max_steps_even_when_tool_calls_continue`; `stream_text_honors_stop_condition_after_streamed_tool_call`; `stream_text_applies_denied_tool_approval_to_continuation_messages`; `stream_text_repairs_and_refines_streamed_tool_call_before_execution`; raw chunk, error chunk, and reasoning/source/custom mapping tests | Dependency-light collector over provider-v4 streams. It standardizes prompts, calls `do_stream`, maps text/reasoning/source/custom/tool/raw/error parts, collects final text, usage, warnings, request/response metadata, raw chunks when requested, tool-input deltas, high-level parsed tool calls/results, text response helpers, local streamed tool execution, continuation prompts, total usage across steps, max-step bounds, stop-condition checks, static tool approval, repair, and input refinement. UI response helpers, aborts, retries, callbacks, deferred provider-result edge cases, and some upstream stream transform details remain unported. |
...
1. Continue `streamText` parity with callbacks, deferred provider-result edge
   cases, aborts/retries, UI response helpers, and error propagation.
