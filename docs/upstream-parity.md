...
| --- | --- | --- | --- | --- | --- |
| `packages/ai` (`ai`) | core package | in-progress | `src/lib.rs`, `src/generate_text.rs`, `src/generate_object.rs`, `src/embed.rs`, `src/generate_image.rs`, `src/generate_speech.rs`, `src/generate_video.rs`, `src/transcribe.rs`, `src/rerank.rs`, `src/upload_file.rs`, `src/upload_skill.rs`, `src/registry.rs`, `src/provider_middleware.rs`, `src/mock_models.rs`, `src/text_stream_response.rs` | Unit tests in matching modules; `mock_models::tests::*`; `examples/kitchen_sink.rs` | Non-streaming generation, object, image, speech, video, transcription, embeddings, reranking, upload, registry, provider-level middleware wrapping, tool input examples middleware, extract JSON middleware, simulate streaming middleware, text-stream response helpers, tool loops, public mock provider-v4 models, and many provider-v4 shapes exist. Agent, stream APIs, UI-message APIs, telemetry, and logger remain unported. |
...
| `streamText` and language model streaming orchestration | in-progress | `src/stream_text.rs` | `stream_text_calls_language_model_do_stream_with_standardized_prompt`; `stream_text_collects_text_deltas_and_finish_metadata`; `stream_text_maps_tool_input_deltas_and_high_level_tool_outputs`; raw chunk, error chunk, and reasoning/source/custom mapping tests | Initial dependency-light collector over provider-v4 streams. It standardizes prompts, calls `do_stream`, maps text/reasoning/source/custom/tool/raw/error parts, collects final text, usage, warnings, request/response metadata, raw chunks when requested, tool-input deltas, and high-level parsed tool calls/results. Multi-step tool execution from streams, UI response helpers, aborts, retries, callbacks, and stop conditions remain unported. |
| Text stream response helpers | verified | `src/text_stream_response.rs` | `create_text_stream_response_sets_headers_status_and_encoded_chunks`; `pipe_text_stream_to_response_writes_headers_chunks_and_end` | Mirrors upstream `text-stream/create-text-stream-response.ts` and `pipe-text-stream-to-response.ts` with Rust-native collected responses and a writer trait instead of JS Web/Node response types. |
...
1. Continue `streamText` parity with multi-step streamed tool execution,
   callbacks, aborts/retries, UI response helpers, error
   propagation, and stop conditions.
2. Continue `streamObject` parity with repair callbacks, telemetry,
   retries/abort, response helper methods, and remaining partial-output strategy
   edge cases.
