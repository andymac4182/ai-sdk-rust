...
8. Continue `streamText` parity with true post-return `createUIMessageStream`
   ...
   segmentation.
9. Continue `streamText`/`streamObject` parity with any remaining portable
   output-strategy stream-result edge cases after the Gateway text/stream/UI
   path is stronger. The `streamObject` Node.js response-like writer case is
   already documented as JavaScript-only. The upstream `streamText`
