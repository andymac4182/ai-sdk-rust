...
| `packages/xai` (`@ai-sdk/xai`) | provider package | in-progress | `src/xai.rs`, `src/openai_compatible.rs`, `src/open_responses.rs` | `xai_provider_creates_responses_model_with_headers_base_url_and_body`; `xai_provider_creates_chat_model_with_openai_compatible_transport`; `xai_provider_creates_image_model_and_reports_unsupported_embeddings`; `xai_provider_settings_serde_accepts_upstream_base_url`; `xai_provider_implements_provider_trait` | Initial xAI provider foundation mirrors upstream settings, `XAI_API_KEY`, base URL defaults, custom headers, user-agent suffix, callable-style `xai(...)`, provider-v4 trait integration, Responses model construction through the shared Open Responses transport, OpenAI-compatible chat construction, image model construction, and unsupported embeddings. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; video, files, xAI-specific conversion helpers, tool preparation, full image generation behavior, and broader Responses behavior remain open. |
...
| `packages/cohere` (`@ai-sdk/cohere`) | provider package | in-progress | `src/cohere.rs` | `cohere_provider_creates_embedding_model_with_options_headers_and_usage`; `cohere_provider_creates_reranking_model_with_object_warning_and_options`; `cohere_provider_reports_chat_and_image_exceptions`; `cohere_provider_implements_embedding_and_reranking_traits`; `cohere_provider_settings_serde_accepts_upstream_base_url` | Initial Cohere provider foundation mirrors upstream settings, `COHERE_API_KEY`, base URL defaults, bearer auth, custom headers, user-agent suffix, embedding and reranking provider-v4 trait integration, `/embed` request options and usage mapping, `/rerank` request options and object-document compatibility warning, response headers/raw body preservation, API error metadata, and explicit unsupported chat/image model errors. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; Cohere chat, prompt conversion, tool preparation, full validation, chunk-limit errors, and broader reranking/embedding parity remain open. |
...
| `packages/fireworks` (`@ai-sdk/fireworks`) | provider package | in-progress | `src/fireworks.rs`, `src/openai_compatible.rs` | `fireworks_provider_creates_chat_model_with_transformed_provider_options`; `fireworks_provider_creates_completion_embedding_and_image_models`; `fireworks_provider_uses_default_base_url_and_function_alias`; `fireworks_provider_implements_provider_trait`; `fireworks_provider_settings_serde_accepts_upstream_base_url` | Initial Fireworks provider foundation mirrors upstream settings, `FIREWORKS_API_KEY`, base URL defaults, custom headers, user-agent suffix, callable-style `fireworks(...)`, provider-v4 trait integration, OpenAI-compatible chat/completion/embedding/image model construction, Fireworks reasoning option transform, and Fireworks error message extraction. Current upstream case disposition and explicit remaining exceptions live in `docs/ai-02-openai-compatible-providers.md`; custom image generation, polling, editing, binary output, abort handling, live/API behavior, and exact provider factory split parity remain open. |
