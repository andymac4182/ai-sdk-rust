...
| `packages/fal` (`@ai-sdk/fal`) | provider package | in-progress | `crates/ai-sdk-fal` | `fal_image_model_maps_request_downloads_metadata_and_warnings`; `fal_image_model_warns_when_multiple_images_are_not_enabled`; `fal_video_model_maps_queue_response_and_metadata`; `fal_models_map_api_errors_to_provider_metadata` | Provider-owned media slice mirrors upstream `createFal` settings for default/custom image and queue base URLs, `FAL_API_KEY`/`FAL_KEY`, custom headers, Fal user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, image request shaping, image URL downloads, multiple-image warnings and provider option override, queue submit/poll response handling, video URL results, response headers, provider metadata, deprecated snake-case option warnings, and API error metadata. Fal speech and transcription are non-media surfaces and remain for AI-04. |
...
| `packages/klingai` (`@ai-sdk/klingai`) | provider package | in-progress | `crates/ai-sdk-klingai` | `klingai_t2v_model_maps_body_headers_polling_and_metadata`; `klingai_i2v_model_maps_file_image_and_aspect_ratio_warning`; `klingai_video_model_maps_api_errors_to_provider_metadata`; `klingai_motion_control_requires_provider_options` | Provider-owned media slice mirrors upstream KlingAI settings for default/custom base URL, `KLINGAI_ACCESS_KEY`, `KLINGAI_SECRET_KEY`, HS256 bearer auth, custom headers, KlingAI user-agent suffix, `video`/`video_model`, provider-v4 trait integration, text-to-video, image-to-video, and motion-control endpoints, task polling, URL video results, task/video provider metadata, file image base64 conversion, unsupported standard option warnings, provider-option request mapping, passthrough options, and API/error metadata. Exact JWT edge fixtures and live KlingAI validation remain credential-gated. |
...
| `packages/prodia` (`@ai-sdk/prodia`) | provider package | in-progress | `crates/ai-sdk-prodia` | `prodia_image_model_maps_json_request_multipart_response_and_metadata`; `prodia_video_model_maps_json_request_and_binary_video_response`; `prodia_video_model_sends_multipart_for_image_to_video`; `prodia_url_video_input_downloads_media_before_multipart_post`; `prodia_models_map_api_errors_to_metadata` | Provider-owned media slice mirrors upstream Prodia settings for default/custom base URL, `PRODIA_TOKEN`, bearer auth, custom headers, Prodia user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, `/job?price=true` submit, JSON and multipart request bodies, image/video multipart response parsing, image/video binary results, URL media download before multipart image-to-video, response headers, job provider metadata, and API error metadata. The Prodia language model is non-media and remains outside AI-03. |
| `packages/replicate` (`@ai-sdk/replicate`) | provider package | in-progress | `crates/ai-sdk-replicate` | `replicate_image_model_maps_flux2_request_downloads_and_warnings`; `replicate_image_model_warns_when_extra_non_flux_images_are_ignored`; `replicate_video_model_maps_prediction_response_and_metadata`; `replicate_image_model_maps_api_errors_to_metadata` | Provider-owned media slice mirrors upstream Replicate settings for default/custom base URL, `REPLICATE_API_TOKEN`, bearer auth, custom headers, Replicate user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, versioned and owner/model prediction endpoints, `Prefer: wait` handling, Flux 2 multi-image request mapping, non-Flux image warnings, output URL downloads, video URL results, response headers, prediction provider metadata, and API error metadata. Live Replicate validation remains credential-gated. |
