...
| Inventory command | `npx opensrc fetch github:vercel/ai` |
...
| --- | --- | --- | --- | --- | --- |
| `packages/ai` (`ai`) | root core SDK package | in-progress | root crate `src/*`; exact row mappings in `docs/ai-core-package-inventory.md` | AIS-01 maps 250 portable `packages/ai` rows to named Rust tests and records 3 JavaScript-only middleware-array identity exceptions; focused agent tests `tool_loop_agent_generate_passes_timeout_to_generate_text`, `tool_loop_agent_stream_passes_timeout_to_stream_text`, and `tool_loop_agent_stream_merges_start_callbacks_in_order`; `cargo test -p ai-sdk-rust tool_loop_agent --lib` | `packages/ai` is not verified. The strict queue is split into agent/UI stream response, middleware/model wrapping, prepare-call helpers, text generation/streaming, object generation/streaming, UI message conversion/transport, telemetry/logging, registries, media/embed/rerank, and shared utilities. This slice closes the mapped agent, middleware, and prepare-call helper rows only; remaining portable rows stay open in `docs/ai-strict-test-inventory.md`. |
| `packages/provider-utils` (`@ai-sdk/provider-utils`) | provider support library | verified | `crates/ai-sdk-provider-utils`; root facade shim in `src/provider_utils.rs` | 760 upstream-shape unit tests in `crates/ai-sdk-provider-utils/src/provider_utils.rs`, including one-to-one `asArray`, `stripFileExtension`, `addAdditionalPropertiesToJsonSchema`, `filterNullable`, `removeUndefinedEntries`, complete portable `validateTypes`/`safeValidateTypes`, complete portable `secureJsonParse`, complete portable `parseJSON`/`safeParseJSON`/`isParsableJson`, complete portable `injectJsonInstruction`, exact `mediaTypeToExtension` table-row test splits, complete portable `detectMediaType`/`getTopLevelMediaType`/`isFullMediaType` case splits, complete portable `resolveFullMediaType` case splits, complete portable `isUrlSupported`, `validateDownloadUrl`, `downloadBlob`/`DownloadError`, `getFromApi`, `readResponseWithSizeLimit`, `responseHandler`, `handleFetchError`, `convertAsyncIteratorToReadableStream`, `isJSONSerializable`, complete portable `delay`, complete portable `executeTool`/`isExecutableTool`, portable `asSchema`/`StandardSchema`, `StreamingToolCallTracker`, and `serializeModelOptions` case splits, complete portable `convertToFormData` and `convertImageModelFileToDataUri` case splits, complete portable `normalizeHeaders` case splits, complete portable `mapReasoningToProvider*` case splits, complete portable `resolve` case splits, complete portable `createToolNameMapping` case splits, complete portable `prepareTools` case splits for root AI usage, complete portable `withUserAgentSuffix` and `getRuntimeEnvironmentUserAgent` case splits, complete portable `createIdGenerator`/`generateId` case splits, complete portable `DelayedPromise` case splits, portable `isProviderReference` case splits, complete portable `resolveProviderReference` case splits, complete portable `types/content-part.test-d.ts` case splits, `provider_utils_reexports_provider_get_error_message`, `tool_call_serializes_upstream_provider_utils_shape`, `tool_result_serializes_upstream_provider_utils_shape`, `content_part_file_part_data_narrows_exhaustively_across_the_4_tagged_arms`, `content_part_file_part_data_exposes_exactly_4_tagged_type_discriminants`, `content_part_file_part_accepts_the_tagged_data_arm`, `content_part_file_part_accepts_the_tagged_url_arm`, `content_part_file_part_accepts_the_tagged_reference_arm`, `content_part_file_part_accepts_the_tagged_text_arm`, `content_part_file_part_also_accepts_bare_data_content_url_and_provider_reference_shorthands`, `content_part_reasoning_file_part_data_narrows_exhaustively_across_the_2_tagged_arms`, `content_part_reasoning_file_part_data_exposes_exactly_2_tagged_type_discriminants`, `content_part_reasoning_file_part_accepts_the_tagged_data_arm`, `content_part_reasoning_file_part_accepts_the_tagged_url_arm`, `content_part_reasoning_file_part_also_accepts_bare_data_content_and_url_shorthands`, `content_part_tool_result_file_variant_accepts_the_tagged_data_arm_with_a_full_media_type`, `content_part_tool_result_file_variant_accepts_the_tagged_url_arm_with_top_level_media_type`, `content_part_tool_result_file_variant_accepts_the_tagged_reference_arm`, `content_part_tool_result_file_variant_accepts_the_tagged_text_arm`, `content_part_tool_result_file_variant_rejects_bare_data_shorthands`, `content_part_tool_result_file_variant_exposes_the_four_tagged_data_type_discriminants`, `content_part_tool_result_legacy_variants_still_type_checks_file_data`, `content_part_tool_result_legacy_variants_still_type_checks_file_url_with_media_type`, `content_part_tool_result_legacy_variants_still_type_checks_file_url_without_media_type`, `content_part_tool_result_legacy_variants_still_type_checks_file_reference`, `content_part_tool_result_legacy_variants_still_type_checks_legacy_image_variants`, `content_part_provider_reference_top_level_accepts_plain_provider_id_records`, `content_part_provider_reference_top_level_rejects_records_that_carry_a_type_key`, `content_part_tool_result_output_wraps_content_parts`, `tool_approval_request_serializes_upstream_prompt_part_shape`, `tool_approval_response_serializes_upstream_prompt_part_shape`, `resolve_returns_raw_values_and_future_values`, `resolve_invokes_lazy_value_and_future_producers_on_demand`, `resolve_can_carry_result_outputs_for_fallible_values`, `delayed_promise_starts_pending`, `delayed_promise_resolves_when_accessed_after_resolution`, `delayed_promise_rejects_when_accessed_after_rejection`, `delayed_promise_waits_until_resolved_when_accessed_first`, `delayed_promise_waits_until_rejected_when_accessed_first`, `delayed_promise_resolves_all_accessed_futures`, `delayed_promise_accessed_future_keeps_first_settlement`, `delayed_promise_uses_latest_status_before_first_access`, `delay_without_duration_resolves_immediately`, `delay_with_duration_resolves_after_timer_completes`, `delay_zero_and_negative_values_use_timer_like_deferred_completion`, `validation_result_serializes_upstream_success_and_failure_shapes`, `schema_wraps_json_schema_and_default_as_schema_matches_upstream`, `lazy_json_schema_defers_creation_and_caches_across_clones`, `lazy_schema_defers_whole_schema_creation_and_normalizes_as_flexible_schema`, `schema_runs_optional_rust_validator`, `inject_json_instruction_adds_schema_to_prompt`, `inject_json_instruction_uses_generic_json_suffix_without_schema`, `inject_json_instruction_handles_no_prompt_no_schema`, `inject_json_instruction_omits_empty_prompt`, `inject_json_instruction_uses_custom_schema_lines`, `inject_json_instruction_into_messages_updates_existing_system_message`, `inject_json_instruction_into_messages_inserts_system_message`, `inject_json_instruction_into_messages_handles_empty_messages_array`, `inject_json_instruction_into_messages_preserves_all_non_system_messages`, `inject_json_instruction_into_messages_preserves_system_provider_options`, `inject_json_instruction_into_messages_uses_custom_schema_lines`, `reasoning_level_serializes_upstream_strings`, `reasoning_level_converts_from_custom_reasoning_efforts`, `is_custom_reasoning_matches_upstream_default_handling`, `map_reasoning_to_provider_effort_returns_direct_match_without_warning`, `map_reasoning_to_provider_effort_warns_for_renamed_match`, `map_reasoning_to_provider_effort_warns_for_missing_level`, `map_reasoning_to_provider_budget_uses_default_percentages_and_clamps`, `map_reasoning_to_provider_budget_uses_custom_percentages`, `map_reasoning_to_provider_budget_warns_for_missing_custom_percentage`, `arrayable_serializes_single_or_array_values`, `arrayable_deserializes_single_or_array_values`, `is_non_nullable_reports_present_values`, `serialized_model_options_round_trips_upstream_shape`, `serialize_model_options_filters_missing_entries_and_preserves_json_null`, `serialize_model_options_accepts_already_json_config_objects`, `form_data_contracts_round_trip_ordered_text_and_bytes_entries`, `convert_to_form_data_skips_missing_and_uses_upstream_array_key_rules`, `convert_to_form_data_can_disable_array_bracket_suffix`, `convert_to_form_data_upstream_adds_string_values_to_form_data`, `convert_to_form_data_upstream_adds_number_values_as_strings`, `convert_to_form_data_upstream_adds_blob_values_to_form_data`, `convert_to_form_data_upstream_skips_null_values`, `convert_to_form_data_upstream_skips_undefined_values`, `convert_to_form_data_upstream_adds_single_element_arrays_without_suffix`, `convert_to_form_data_upstream_adds_multi_element_arrays_with_suffix`, `convert_to_form_data_upstream_adds_multi_element_arrays_without_suffix_when_disabled`, `convert_to_form_data_upstream_handles_empty_arrays_by_not_adding_values`, `convert_to_form_data_upstream_adds_string_arrays_with_suffix`, `convert_to_form_data_upstream_accepts_typed_input_objects`, `convert_to_form_data_upstream_handles_complex_input_with_various_types`, `download_blob_contracts_serialize_with_upstream_camel_case_fields`, `download_blob_downloads_bytes_and_content_type_through_injected_transport`, `download_blob_returns_empty_blob_for_missing_body`, `download_blob_turns_non_success_status_into_download_error`, `download_blob_enforces_size_limit_from_headers_and_body_bytes`, `download_blob_validates_redirect_final_url`, `download_blob_propagates_transport_download_errors`, `streaming_tool_call_tracker_options_serialize_and_deserialize_validation_mode`, `streaming_tool_call_delta_round_trips_upstream_shape_with_extensions`, `streaming_tool_call_tracker_accumulates_and_finishes_json_arguments`, `streaming_tool_call_tracker_flushes_unfinished_tool_calls_once`, `streaming_tool_call_tracker_validates_required_delta_type_and_fields`, `streaming_tool_call_tracker_attaches_provider_metadata_to_tool_call_events`, `id_generator_options_serialize_and_deserialize_camel_case_shape`, `create_id_generator_creates_random_part_with_configured_size_and_alphabet`, `create_id_generator_adds_prefix_and_separator`, `create_id_generator_rejects_separator_inside_alphabet_when_prefixed`, `create_id_generator_allows_default_separator_without_prefix`, `generate_id_uses_upstream_default_random_part_length`, `is_provider_reference_accepts_plain_records`, `is_provider_reference_rejects_tagged_file_data_objects`, `is_provider_reference_rejects_non_objects_and_arrays`, `validate_types_returns_validated_values`, `validate_types_wraps_validation_errors_with_context`, `safe_validate_types_preserves_raw_value_after_transformation`, `safe_validate_types_returns_error_and_raw_value_on_failure`, `safe_validate_types_passes_through_json_when_schema_has_no_validator`, `parse_json_with_schema_returns_validated_values`, `parse_json_with_schema_wraps_type_validation_errors`, `safe_parse_json_with_schema_preserves_raw_value_after_transformation`, `safe_parse_json_with_schema_preserves_raw_value_on_validation_failure`, `safe_parse_json_with_schema_has_no_raw_value_on_parse_failure`, `parse_provider_options_returns_none_for_missing_provider_options`, `parse_provider_options_returns_validated_provider_options`, `parse_provider_options_reports_invalid_argument_on_validation_failure`, `parse_json_parses_json_values_without_schema`, `parse_json_wraps_invalid_json_in_provider_error`, `parse_json_rejects_proto_properties`, `parse_json_rejects_constructor_prototype_objects`, `parse_json_allows_safe_constructor_properties`, `safe_parse_json_returns_success_with_raw_value`, `safe_parse_json_returns_json_parse_error_without_raw_value_on_invalid_json`, `safe_parse_json_returns_json_parse_error_for_forbidden_prototype_properties`, `parse_json_error_can_wrap_type_validation_failures`, `is_parsable_json_matches_secure_parse_result`, `convert_inline_file_data_to_bytes_encodes_text_as_utf8`, `convert_inline_file_data_to_bytes_returns_raw_bytes_unchanged`, `convert_inline_file_data_to_bytes_decodes_base64_data`, `convert_inline_file_data_to_bytes_rejects_non_inline_file_data`, `convert_inline_file_data_to_bytes_rejects_invalid_base64_data`, `convert_base64_to_bytes_decodes_standard_and_url_safe_data`, `convert_base64_to_bytes_rejects_invalid_data`, `convert_bytes_to_base64_encodes_raw_bytes`, `convert_to_base64_passes_strings_through_and_encodes_bytes`, `get_top_level_media_type_matches_upstream_edge_cases`, `is_full_media_type_requires_concrete_subtype`, `is_url_supported_matches_media_type_and_url_patterns`, `is_url_supported_matches_wildcards_and_top_level_media_types`, `is_url_supported_lowercases_media_type_keys_and_urls_before_matching`, `is_url_supported_ignores_invalid_regex_sources`, `is_url_supported_upstream_returns_false_when_model_does_not_support_any_urls`, `is_url_supported_upstream_returns_true_for_exact_media_type_and_exact_url_match`, `is_url_supported_upstream_returns_true_for_exact_media_type_and_regex_url_match`, `is_url_supported_upstream_returns_true_for_one_of_multiple_regex_url_matches`, `is_url_supported_upstream_returns_false_for_exact_media_type_but_url_mismatch`, `is_url_supported_upstream_returns_false_for_url_match_but_media_type_mismatch`, `is_url_supported_upstream_returns_true_for_wildcard_media_type_and_exact_url_match`, `is_url_supported_upstream_returns_true_for_wildcard_media_type_and_regex_url_match`, `is_url_supported_upstream_returns_false_for_wildcard_media_type_but_url_mismatch`, `tool_execution_options_include_execution_metadata_context_abort_signal_and_sandbox`, `sandbox_command_options_include_abort_signal_without_serializing_it`, `tool_execute_function_accepts_input_output_and_execution_options`, `tool_needs_approval_function_accepts_input_options_and_returns_boolean`, `tool_to_model_output_accepts_untyped_output_without_execute`, `tool_to_model_output_accepts_execute_function_output`, `tool_to_model_output_accepts_output_schema_result_output`, `tool_needs_approval_function_accepts_input_schema_options`, `tool_needs_approval_function_accepts_execute_tool_options`, `tool_needs_approval_function_accepts_context_schema_context`, `dynamic_tool_upstream_should_include_dynamic_tools_in_the_tool_union`, `dynamic_tool_upstream_should_allow_function_style_properties`, `dynamic_tool_upstream_should_reject_provider_only_properties`, `dynamic_tool_upstream_should_create_dynamic_tools_with_the_dynamic_discriminator`, `provider_defined_tool_upstream_should_include_provider_defined_tools_in_the_tool_union`, `provider_defined_tool_upstream_should_require_provider_specific_properties`, `provider_defined_tool_upstream_should_allow_user_execution_or_an_output_schema`, `provider_defined_tool_upstream_rejects_function_only_properties`, `provider_executed_tool_upstream_should_include_provider_executed_tools_in_the_tool_union`, `provider_executed_tool_upstream_should_require_provider_specific_properties`, `provider_executed_tool_upstream_should_allow_deferred_result_support`, `provider_executed_tool_upstream_rejects_function_only_properties`, `function_tool_upstream_should_expose_the_function_tool_discriminator`, `function_tool_upstream_should_include_function_tools_in_the_tool_union`, `function_tool_upstream_should_allow_omitted_and_explicit_function_discriminators`, `function_tool_upstream_should_reject_dynamic_and_provider_only_properties`, `tool_union_upstream_should_expose_all_tool_variants_and_type_discriminators`, `tool_union_upstream_should_narrow_tools_by_type`, `tool_constructor_input_type_upstream_should_infer_input_type_from_zod_input_schema`, `tool_constructor_input_type_upstream_should_preserve_input_type_from_flexible_schema`, `tool_constructor_input_type_upstream_should_infer_input_type_with_optional_default_examples`, `tool_constructor_input_type_upstream_should_infer_input_type_with_refined_schema_examples`, `tool_constructor_context_type_upstream_should_infer_context_type_from_context_schema_in_execute`, `tool_constructor_context_type_upstream_should_infer_context_type_in_input_lifecycle_callbacks`, `tool_constructor_output_type_upstream_should_infer_output_type_from_execute_function`, `tool_constructor_output_type_upstream_should_infer_output_type_from_async_generator_execute_function`, `tool_input_lifecycle_callbacks_receive_upstream_execution_options`, `function_tool_retains_output_schema_without_provider_serialization`, `post_json_to_api_options_carries_abort_signal_without_serializing_it`, `post_form_data_to_api_options_carries_abort_signal_without_serializing_it`, `post_to_api_options_carries_abort_signal_without_serializing_it`, `post_json_to_api_aborts_before_transport_call`, and `post_json_to_api_aborts_pending_transport_when_signal_fires`; root facade compile coverage | Schema/validation including JSON Schema, lazy schema, and Standard Schema v1 conversion/validation, JSON parsing, header normalization/combination, resolvable value/function/future handling, nullish filtering, delayed externally resolved futures, abortable delay semantics, JSON instruction injection, reasoning provider mapping, media type detection/resolution, media/base64/form-data helpers, download and response-handler contracts, async iterator readable-stream cancellation, provider API request helpers including GET, JSON, form-data, and generic POST abort-signal propagation plus abort-aware pending transport cancellation, user-agent helpers, provider reference resolution, provider tool-name mapping, provider-utils content-part model-message/tool-result output contracts including legacy file/image variants, tool factories/types, function/dynamic tool output-schema retention for local execution typing, complete execute-tool and executable-tool helper parity, Rust abort-signal counterparts for tool execution, sandbox command options, tool input lifecycle callbacks, tool constructor input/context/output contracts, provider-facing prepareTools conversion including provider-defined tools, provider options, strict mode, input examples, and context/sandbox-derived descriptions, tool model-output callbacks, function-form approval callbacks, high-level tool variant contracts, and provider-only property exclusion, complete streaming tool-call tracking, and ID generation now live in the matching `ai-sdk-provider-utils` crate. The root crate retains a compatibility re-export shim only. `src/retry.rs` remains `packages/ai` ownership because upstream retry lives under `packages/ai/src/util`, not `packages/provider-utils`. Exact browser `ReadableStream`, Web fetch/runtime integration, provider-utils compile-only `*.test-d.ts` assertions, and true mid-socket preemption for the current blocking `ureq` default transports are documented as JavaScript-runtime/type-system or transport-depth exceptions. Zod v3/v4 JSON-schema adapter snapshots are now explicitly inventoried as JavaScript/Zod-runtime-specific below instead of being counted as hidden Rust parity debt. |
| `packages/provider` (`@ai-sdk/provider`) | provider contracts | verified | `crates/ai-sdk-provider`; root facade shims in `src/provider.rs`, `src/language_model.rs`, `src/embedding_model.rs`, `src/image_model.rs`, `src/speech_model.rs`, `src/transcription_model.rs`, `src/reranking_model.rs`, `src/video_model.rs`, `src/files.rs`, `src/skills.rs`, `src/json.rs`, `src/warning.rs`, `src/file_data.rs`, `src/headers.rs` | Contract and serialization tests in `crates/ai-sdk-provider/src/*`; one-to-one `get_error_message_*` split for upstream `get-error-message.test.ts`; `call_options_carries_abort_signal_without_serializing_it`; non-language call-option abort signal serialization coverage; root facade compile coverage | Provider-v4 contracts, shared JSON/header shapes, warnings, file/provider references, model call/result contracts, language-model and non-language-model call-option abort signal propagation, abort wake support, and provider trait surfaces now live in the matching `ai-sdk-provider` crate. The upstream provider `getErrorMessage` test file is represented case-for-case in Rust; upstream v2/v3 compatibility surfaces and exact stream abstractions are documented as JavaScript compatibility/runtime exceptions. |
...
| `packages/openai` (`@ai-sdk/openai`) | provider package | verified | `src/openai.rs`; `crates/ai-sdk-openai-compatible`; `crates/ai-sdk-open-responses` | `openai_chat_converts_image_parts_with_raw_base64_and_detail`; `openai_chat_resolves_top_level_image_media_types`; `openai_chat_converts_binary_pdf_file_parts`; `openai_chat_converts_pdf_and_image_provider_reference_file_parts`; `openai_chat_rejects_missing_provider_reference_for_file_parts`; `openai_chat_defaults_missing_tool_call_input_to_empty_object`; `openai_chat_handles_different_tool_output_types`; `openai_provider_language_model_uses_responses_endpoint`; `openai_provider_uses_the_default_openai_base_url_when_not_provided`; `openai_provider_uses_openai_base_url_when_set`; `openai_image_should_pass_headers`; `openai_image_should_return_image_meta_data`; exact row map in `docs/ai-04-openai-strict-provider-closure.md` | OpenAI package parity is tracked row-by-row in the AIS-04 strict map. Portable chat/completions/embeddings/files/images/audio/transcription/skills/provider factory rows are covered by root OpenAI tests and provider-aware OpenAI-compatible chat tests; Responses rows are covered by `ai-sdk-open-responses` plus the root OpenAI Responses facade. The remaining upstream type-only rows stay explicit in `docs/ai-strict-test-inventory.md`. |
...
| `packages/open-responses` (`@ai-sdk/open-responses`) | provider package | verified | `crates/ai-sdk-open-responses`; root facade shim in `src/open_responses.rs` | `open_responses_provider_generates_text_with_request_and_response_metadata`; `open_responses_provider_converts_user_file_prompt_parts`; `open_responses_provider_sends_pdf_input_file_request_body`; `open_responses_provider_produces_pdf_input_file_content`; `open_responses_provider_extracts_pdf_input_file_usage`; `open_responses_provider_streams_pdf_input_file_fixture`; `open_responses_provider_sends_lmstudio_basic_request_body`; `open_responses_provider_produces_lmstudio_basic_content`; `open_responses_provider_extracts_lmstudio_basic_usage`; `open_responses_provider_streams_lmstudio_basic_content`; `open_responses_provider_streams_text_with_request_and_response_metadata`; `open_responses_provider_generates_object_with_json_schema_response_format`; `open_responses_provider_prepares_openai_hosted_tools`; `open_responses_provider_adds_hosted_tool_include_options`; `open_responses_provider_maps_openai_hosted_tool_outputs`; `open_responses_provider_maps_additional_response_tool_items`; `open_responses_provider_maps_text_sources_and_compaction_metadata`; `open_responses_provider_streams_text_sources_reasoning_and_compaction_metadata`; `open_responses_provider_generates_phase_fixture_metadata`; `open_responses_provider_streams_phase_fixture_metadata`; `open_responses_provider_streams_hosted_tool_outputs`; `open_responses_provider_maps_web_search_api_sources`; `open_responses_provider_maps_web_search_missing_action`; `open_responses_provider_streams_web_search_action_query`; `open_responses_provider_streams_web_search_missing_action`; `open_responses_provider_maps_openai_numeric_error_code`; `open_responses_provider_streams_openai_error_event_without_synthetic_message`; `open_responses_provider_streams_additional_tool_items`; `open_responses_provider_streams_tool_input_delta_refinements`; `open_responses_provider_generates_apply_patch_create_file_fixture_request_body`; `open_responses_provider_generates_apply_patch_create_file_fixture_content`; `open_responses_provider_streams_apply_patch_create_file_fixture`; `open_responses_provider_streams_apply_patch_delete_file_fixture`; `open_responses_provider_maps_openai_responses_provider_options_to_request_body`; `open_responses_provider_streams_context_management_options`; `open_responses_provider_warns_for_conversation_with_previous_response_id`; `open_responses_provider_maps_openai_passthrough_option_edges`; `open_responses_provider_falls_back_to_openai_options_for_azure_requests`; `open_responses_provider_prefers_azure_options_over_openai_fallback`; `open_responses_provider_uses_azure_metadata_key_for_text_result`; `open_responses_provider_uses_azure_metadata_key_for_function_call_content`; `open_responses_provider_streams_azure_metadata_key_for_reasoning_and_finish`; `open_responses_provider_adds_encrypted_reasoning_include_for_reasoning_store_false`; `open_responses_provider_omits_encrypted_reasoning_include_for_non_reasoning_store_false`; `open_responses_provider_omits_encrypted_reasoning_include_for_store_true`; `open_responses_provider_allows_force_reasoning_for_unrecognized_model_ids`; `open_responses_provider_sends_xhigh_reasoning_effort_for_codex_max_model`; `open_responses_provider_warns_for_reasoning_effort_on_non_reasoning_models`; `open_responses_provider_applies_openai_model_capability_rules`; `open_responses_provider_validates_openai_service_tier_model_capabilities`; `open_responses_provider_sends_instructions_from_system_message`; `open_responses_provider_joins_multiple_system_messages_with_newlines`; `open_responses_provider_converts_openai_message_chain_with_system_input_items`; `open_responses_provider_maps_openai_system_message_modes`; `open_responses_provider_skips_conversation_history_items`; `open_responses_provider_reconstructs_hosted_tool_search_history_with_store_false`; `open_responses_provider_reconstructs_client_tool_search_output_with_store_false`; `open_responses_provider_warns_for_unstored_hosted_tool_results`; `open_responses_provider_reconstructs_local_shell_history_with_store_false`; `open_responses_provider_reconstructs_shell_history_with_store_false`; `open_responses_provider_reconstructs_stored_assistant_shell_outputs`; `open_responses_provider_reconstructs_apply_patch_history_with_store_false`; `open_responses_provider_reconstructs_stored_apply_patch_outputs`; `open_responses_provider_reconstructs_custom_tool_calls`; `open_responses_provider_reconstructs_custom_tool_outputs`; `open_responses_provider_converts_standard_tool_result_outputs`; `open_responses_provider_sends_lmstudio_request_parameters_body`; `open_responses_provider_sends_lmstudio_tools_request_body`; `open_responses_provider_sends_tool_choice_auto`; `open_responses_provider_sends_tool_choice_none`; `open_responses_provider_sends_tool_choice_required`; `open_responses_provider_sends_tool_choice_specific_tool`; `open_responses_provider_maps_allowed_tools_required_mode`; `open_responses_provider_maps_function_call_response_and_usage`; `open_responses_provider_resolves_provider_reference_file_parts`; `open_responses_provider_rejects_missing_provider_reference_file_part`; `open_responses_provider_converts_tool_result_file_content_outputs`; `open_responses_provider_maps_api_error_data_to_metadata_and_response`; `open_responses_provider_preserves_stream_error_event_data`; `open_responses_provider_runs_generate_text_tool_loop_end_to_end`; `open_responses_streams_function_call_argument_deltas`; `open_responses_provider_reports_unsupported_embedding_and_image`; `open_responses_provider_maps_top_level_reasoning_high_to_effort`; `open_responses_provider_maps_top_level_reasoning_minimal_to_low`; `open_responses_provider_maps_top_level_reasoning_none_to_none`; `open_responses_provider_passes_top_level_reasoning_xhigh_directly`; `open_responses_provider_omits_reasoning_when_not_specified`; `open_responses_provider_sends_detailed_reasoning_summary_from_provider_options`; `open_responses_provider_combines_top_level_reasoning_with_summary`; `open_responses_provider_sends_concise_reasoning_summary_from_provider_options`; `open_responses_provider_omits_reasoning_for_empty_provider_options`; additive `open_responses_provider_filters_non_reasoning_generic_provider_options`; `open_responses_finish_reason_undefined_with_tool_calls_maps_tool_calls`; `open_responses_finish_reason_null_with_tool_calls_maps_tool_calls`; `open_responses_finish_reason_undefined_without_tool_calls_maps_stop`; `open_responses_finish_reason_null_without_tool_calls_maps_stop`; `open_responses_finish_reason_max_output_tokens_maps_length`; `open_responses_finish_reason_content_filter_maps_content_filter`; `open_responses_finish_reason_unknown_with_tool_calls_maps_tool_calls`; `open_responses_finish_reason_unknown_without_tool_calls_maps_other`; additive `open_responses_finish_reason_maps_legacy_max_tokens_to_length`; `open_responses_provider_maps_upstream_multi_turn_tool_conversation_fixture`; `open_responses_provider_streams_mcp_approval_request_fixture_turn_1`; `open_responses_provider_streams_mcp_approval_denial_fixture_turn_2`; `open_responses_provider_streams_mcp_approval_retry_fixture_turn_3`; `open_responses_provider_streams_mcp_approval_result_fixture_turn_4`; `openai_provider_language_model_uses_responses_endpoint` | Initial Open Responses provider settings, provider id, generic system instructions plus OpenAI/Gateway system/developer/removal system-message modes, basic user/assistant/tool message-chain conversion, upstream multi-turn tool conversation request shaping, standard tool-result text/JSON/error/denied output conversion, LMStudio request parameters including presence/frequency penalties and JSON schema text-format mapping, generic LMStudio function tool request shaping, basic function tool-choice modes including `allowed_tools` required mode, non-streaming function-call response and usage-detail mapping, user-agent suffix, bearer/custom headers, non-streaming and SSE `/responses` request body for text, image, and file prompts including provider-reference image/PDF file ids with missing-provider errors plus upstream PDF input-file fixture response and stream deltas, LMStudio basic generation request-body, content, usage, and basic streaming fixture mapping, generic provider-options filtering plus OpenAI/Azure/Gateway body passthrough with OpenAI wrapper request-key normalization and request option edges for `instructions`, `include`, `user`, `conversation`, `metadata`, `store`, `truncation`, and `logprobs`, streaming `contextManagement` compaction forwarding, conversation/previous-response conflict warnings, Azure fallback to `providerOptions.openai` when `providerOptions.azure` is absent with Azure metadata retained and Azure-specific options taking precedence, Azure provider-metadata key coverage for non-streaming text results, non-streaming function calls, plus streaming reasoning and finish events, top-level/provider reasoning request option matrix, OpenAI/Azure/Gateway model capability rules for `forceReasoning`, Codex Max `xhigh` reasoning effort, and reasoning-model temperature/topP stripping, non-reasoning reasoning-option warnings with dedicated upstream non-reasoning model matrix coverage, dedicated `store: false`/`store: true` encrypted reasoning include request tests, and service-tier validation, JSON schema response-format request shaping, high-level object generation, function tool request shaping including OpenAI hosted/provider-defined tool request preparation, hosted web-search/code-interpreter automatic include options, and hosted tool-choice name mapping, `tool_choice`, tool-result `function_call_output` continuation, response text/reasoning/tool-call extraction, non-streaming provider-executed hosted tool-call/tool-result output mapping for web search including API-typed sources and missing-action resilience, file search, code interpreter, image generation, tool search, local/shell calls and outputs, apply-patch calls, MCP calls and approval requests, computer calls, custom tool calls, text/reasoning provider metadata including phase, annotation sources, compaction custom content, server/client `tool_search` prompt-history reconstruction for `store: false`, local-shell prompt-history reconstruction for `local_shell_call` and `local_shell_call_output`, shell prompt-history reconstruction for `shell_call` and `shell_call_output`, apply-patch prompt-history reconstruction for `apply_patch_call` and `apply_patch_call_output`, custom provider-tool prompt-history reconstruction for `custom_tool_call` and `custom_tool_call_output`, stored assistant shell-output and apply-patch output reconstruction, tool-result image-detail provider option forwarding, hosted-result warning/skip behavior for unsupported provider-executed tools when not stored, streamed function-call/custom-tool argument deltas, streamed function/custom/tool-search/local-shell/shell/apply-patch/MCP provider metadata (`itemId`/`namespace`), MCP approval request/denial/retry/approval streaming fixtures, streaming text/reasoning/source/phase metadata, streaming compaction custom content, streaming hosted web-search/file-search/code-interpreter/image-generation tool calls and results, web-search API-typed source and missing-action resilience, code-interpreter code deltas, and preliminary partial-image results, streaming apply-patch diff input deltas, streaming custom/tool-search/local-shell/shell/apply-patch/MCP/computer-use item mapping, raw chunks, parse/error stream parts with provider error payloads, usage/cached/reasoning token mapping, finish reason mapping, response metadata, API error metadata (`type`/`param`/string-or-numeric `code`/status/retryability) and SSE error event raw finish-reason preservation, unsupported embedding/image lookups, and completed function-call stream item mapping are represented. The Open Responses provider implementation and direct package-owned tests now live in the matching `ai-sdk-open-responses` crate. The root module is reduced to a compatibility re-export shim plus root high-level API integration tests for `generate_text`, `generate_object`, and `stream_text`. The current upstream Responses corpus audit is fully mapped; broader `packages/openai` surfaces remain tracked by the main OpenAI row. |
| `packages/anthropic` (`@ai-sdk/anthropic`) | provider package | verified | `crates/ai-sdk-anthropic`; `docs/ai-foundational-provider-inventory.md` | `cargo test -p ai-sdk-anthropic` (420 passed, 1 ignored); 418 named row tests in `crates/ai-sdk-anthropic/tests/upstream_mapping.rs`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md`; ignored `live_anthropic_messages_generate_text` | AI-01A ports the upstream Anthropic provider surface for Messages request/response and stream planning, files, skills, prompt conversion, cache control, provider tools, usage conversion, error mapping, settings, schema sanitization, and deterministic request/response fixtures. Current inventory covers 40 source files, 52 fixture/snapshot files, 13 test files, and 424 upstream test cases: 418 portable rows are mapped to named Rust tests, 0 portable rows remain unmapped, and 6 TypeScript generic-inference rows stay classified as `type-system-impossible`. |
| `packages/amazon-bedrock` (`@ai-sdk/amazon-bedrock`) | provider package | verified | `crates/ai-sdk-amazon-bedrock`; `docs/ai-foundational-provider-inventory.md` | `cargo test -p ai-sdk-amazon-bedrock` (16 passed, 1 ignored); `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md`; named Bedrock tests `amazon_bedrock_cache_points_match_upstream_ttl_cases`, `amazon_bedrock_chat_options_validate_service_tier_and_types`, `amazon_bedrock_chat_generate_and_stream_match_upstream_fixtures`, `amazon_bedrock_embedding_models_prepare_requests_and_parse_responses`, `amazon_bedrock_image_model_prepares_generation_editing_and_response_cases`, `amazon_bedrock_reranking_model_prepares_requests_and_parses_responses`, `amazon_bedrock_event_stream_decoder_handles_fixtures_frames_and_errors`, `amazon_bedrock_sigv4_and_api_key_fetch_wrappers_sign_requests`, `amazon_bedrock_prepare_tools_maps_function_anthropic_and_choice_cases`, `amazon_bedrock_message_conversion_maps_prompt_content_files_tools_and_reasoning`, `amazon_bedrock_usage_conversion_maps_cache_and_missing_usage`, `amazon_bedrock_provider_factories_auth_and_headers_match_upstream`, `amazon_bedrock_anthropic_fetch_transforms_event_streams`, `amazon_bedrock_anthropic_provider_transforms_tools_and_model_factories`, `amazon_bedrock_mantle_provider_maps_openai_compatible_factories`, `amazon_bedrock_mistral_tool_call_id_normalization_matches_upstream`, and ignored `amazon_bedrock_live_provider_proof_is_credential_gated` | AI-01B ports the upstream Amazon Bedrock provider surface for Converse chat, Anthropic-on-Bedrock request transformation, embeddings, Nova image generation/editing, Agent Runtime reranking, event-stream decoding, SigV4/API-key authentication, tool preparation, usage conversion, settings, deterministic fixtures, and error mapping. Current inventory covers 31 source files, 25 fixture/snapshot files, 17 test files, and 383 upstream test cases: 380 portable rows are mapped to named Rust tests, 0 portable rows remain unmapped, and 3 JavaScript callable-constructor rows stay classified as `js-only-documented`. |
| `packages/google` (`@ai-sdk/google`) | provider package | verified | `crates/ai-sdk-google`; `docs/ai-foundational-provider-inventory.md` | `cargo test -p ai-sdk-google --lib`; `google_foundational_inventory_tracks_current_upstream_cases`; `google_convert_json_schema_to_openapi_schema_upstream_cases`; `google_convert_to_google_messages_upstream_cases`; `google_get_model_path_and_supported_file_url_upstream_cases`; `google_prepare_tools_upstream_cases`; `google_json_accumulator_upstream_cases`; `google_language_model_generate_upstream_cases`; `google_language_model_stream_upstream_cases`; `google_embedding_model_upstream_cases`; `google_image_model_upstream_cases`; `google_video_model_upstream_cases`; `google_files_upstream_cases`; `google_provider_upstream_cases`; `google_interactions_upstream_cases`; `google_warning_error_and_usage_mapping_upstream_cases`; ignored `google_live_provider_proof_is_credential_gated`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | Google provider-owned crate now mirrors upstream `@ai-sdk/google` for Gemini language generation and streaming, embeddings, image and video generation, Files uploads, Interactions, schema conversion, message conversion, URL support, tool preparation, JSON accumulation, settings, request/response fixture behavior, warnings, usage, finish reasons, source extraction, and error mapping. The AI-01C inventory maps all 568 current upstream cases: 566 portable cases to named Rust tests and 2 TypeScript-only compile-error rows to `type-system-impossible`, leaving 0 portable unmapped cases. Live Google validation remains credential-gated behind `GOOGLE_GENERATIVE_AI_API_KEY`. |
| `packages/google-vertex` (`@ai-sdk/google-vertex`) | provider package | verified | `crates/ai-sdk-google-vertex`; `docs/ai-foundational-provider-inventory.md` | `cargo test -p ai-sdk-google-vertex`; `google_vertex_foundational_inventory_tracks_current_upstream_cases`; `google_vertex_auth_headers_cover_node_and_edge_wrappers`; `google_vertex_anthropic_provider_matches_vertex_specific_contract`; `google_vertex_edge_auth_builds_jwt_and_token_exchange_request`; `google_vertex_node_and_edge_auth_wrappers_match_upstream`; `google_vertex_embedding_model_builds_predict_requests_and_results`; `google_vertex_image_model_covers_imagen_and_gemini_contracts`; `google_vertex_base_provider_resolves_models_urls_tools_and_express_mode`; `google_vertex_video_model_covers_predict_long_running_contract`; `google_vertex_maas_provider_wraps_openai_compatible_lazily`; `google_vertex_xai_provider_wraps_openai_compatible_with_grok_transform`; ignored `live_google_vertex_embedding_smoke_uses_real_credentials`; `node scripts/ai-foundational-provider-inventory.mjs --check --output docs/ai-foundational-provider-inventory.md` | AI-01D ports the current upstream Google Vertex package surface for 38 source files, 5 fixture/snapshot files, 17 test files, and 204 upstream test cases. The row-level inventory maps all 201 portable cases to named Rust tests and preserves 3 `js-only-documented` callable-constructor exceptions. The crate covers Vertex auth, provider base/edge variants, embedding, Imagen/Gemini image generation, video long-running operations, Anthropic-on-Vertex, MaaS, xAI-on-Vertex, warning/error metadata, settings, fixtures, and an ignored credential-gated live embedding smoke proof. |
| `packages/xai` (`@ai-sdk/xai`) | provider package | verified | `src/xai.rs`, `src/openai_compatible.rs`, `src/open_responses.rs` | `xai_provider_creates_responses_model_with_headers_base_url_and_body`; `xai_responses_model_prepares_server_tools_custom_tool_and_usage`; `xai_provider_creates_chat_model_with_openai_compatible_transport`; `xai_chat_request_maps_upstream_options_tools_warnings_and_usage`; `xai_chat_stream_maps_warnings_text_and_usage`; `xai_chat_message_conversion_covers_text_images_tool_calls_and_results`; `xai_responses_tool_ids_input_usage_stream_metadata_are_xai_specific`; `xai_files_image_and_video_models_cover_requests_metadata_and_errors`; `xai_media_models_abort_before_request`; `xai_provider_creates_image_model_and_reports_unsupported_embeddings`; `xai_provider_settings_serde_accepts_upstream_base_url`; `xai_provider_implements_provider_trait`; ignored `live_xai_responses_generation_validates_provider_contract`; ignored `live_xai_files_image_video_validate_provider_contract` | All 337 refreshed upstream xAI `it`/`test` cases are mapped one-to-one in `docs/ai-02-openai-compatible-providers.md`: 320 portable runtime cases and 17 TypeScript-only video option cases. The xAI provider now covers default/custom settings, `XAI_API_KEY`, custom headers, xAI user-agent suffix, `xai(...)`, chat/Responses/image/video/files provider-v4 trait surfaces, OpenAI-compatible chat request transforms for `max_completion_tokens`, xAI reasoning effort, search parameters, logprobs, tool schema cleanup, warnings, stream usage, citations, and xAI 200-status error messages. Responses coverage includes xAI server tool ids (`web_search`, `x_search`, `code_interpreter`, `view_image`, `view_x_video`, `file_search`, `mcp`), function tools, provider-tool choice constraints, input conversion, usage/cost metadata, streamed finish metadata, and hosted smoke proof. Files coverage includes multipart upload, filename/default filename, `team_id`, provider references, metadata, and live proof. Image coverage includes JSON generation/edit requests, URL/base64/multiple images, warnings, provider options, base64/URL responses, metadata, API-error metadata, and abort-before-request propagation. Video coverage includes text/edit/extend/reference modes, resolution mapping, warnings, polling retry/timeout/error states, moderation/missing URL handling, metadata, abort-before-request propagation, and live proof. Remaining exceptions are explicit: JavaScript constructor/callable identity and thrown error class identity are `js-only-documented`, TypeScript-only video option inference is `type-system-impossible`, and hosted API acceptance is credential-gated by ignored `XAI_API_KEY` live tests. |
| `packages/alibaba` (`@ai-sdk/alibaba`) | provider package | verified | `crates/ai-sdk-alibaba` | `alibaba_chat_model_builds_request_body_with_options_reasoning_tools_cache_and_usage`; `alibaba_chat_model_maps_top_level_reasoning_variants_and_provider_override`; `alibaba_chat_model_converts_multimodal_assistant_and_tool_messages`; `alibaba_chat_model_applies_cache_control_precedence_and_breakpoint_warning`; `alibaba_chat_model_reports_provider_reference_and_invalid_option_errors`; `alibaba_chat_model_streams_reasoning_text_tool_calls_usage_and_raw_chunks`; `alibaba_chat_model_requests_stream_usage_by_default`; `alibaba_chat_model_streams_incremental_tool_call_arguments`; `alibaba_video_model_generates_video_with_headers_body_and_metadata`; `alibaba_video_model_uses_custom_base_url_and_polls_until_succeeded`; `alibaba_video_model_maps_i2v_r2v_resolution_and_warnings`; `alibaba_video_model_maps_api_and_status_errors_to_metadata`; `alibaba_video_model_maps_missing_task_canceled_and_missing_url_errors`; `alibaba_provider_reports_unsupported_model_families_and_trait_video`; `alibaba_provider_settings_serde_accepts_upstream_shape`; ignored credential-gated `live_alibaba_chat_generate_text_smoke_when_env_present`; ignored opt-in `live_alibaba_video_generate_smoke_when_env_present` | Refreshed upstream with `npx opensrc fetch github:vercel/ai` on 2026-06-01. Current upstream `packages/alibaba` has 6 deterministic test files and 87 portable `it` cases. The provider-owned crate mirrors upstream `createAlibaba` settings for default/custom chat and video base URLs, `ALIBABA_API_KEY`, custom headers, Alibaba user-agent suffix, `alibaba()`/`create_alibaba`, `language_model`/`chat`/`chat_model`, `video`/`video_model`, provider-v4 and `ProviderWithVideoModel` integration, include-usage defaults, and unsupported embedding/image lookups. `alibaba-provider.test.ts` maps its 14 cases to provider/settings, stream-usage, custom video-base-url, trait-video, and ignored live auth smoke tests; JavaScript callable-constructor identity plus function-valued `fetch`/`headers` callbacks are JS runtime seams, while Rust covers the portable behavior through typed providers, settings, and injected transports. `alibaba-chat-language-model.test.ts` maps its 14 cases to chat request shaping, top-level/provider-option reasoning precedence, text/tool/reasoning extraction, cache/reasoning usage conversion, stream usage options, raw chunks, and incremental SSE text/reasoning/tool-call conversion. `convert-to-alibaba-chat-messages.test.ts` maps its 21 cases to multimodal user content, assistant/tool message conversion, URL/inline image media normalization, cache-control precedence, over-4 breakpoint warnings, and provider-reference rejection. `convert-alibaba-usage.test.ts` maps its cache-read/cache-write/no-cache/reasoning-token case to the chat generate and stream usage tests. `get-cache-control.test.ts` maps metadata extraction, missing metadata, and max-breakpoint warning behavior to the cache-control test. `alibaba-video-model.test.ts` maps its 34 cases to provider/model identity, custom model IDs, T2V/I2V/R2V body shaping, resolution conversion, provider options, auth/custom headers, async-create versus poll headers, warnings, response metadata, usage metadata, multi-poll completion, and missing-task/failed/canceled/missing-video-url errors. Exceptions: exact thrown `AISDKError` class identity, fake-timer/abort timing mechanics, JavaScript/Zod runtime validation internals, and workflow serialization hooks are not portable Rust provider obligations; live Alibaba chat/video service behavior is credential-gated and documented in `docs/live-integration-proof-registry.md`. |
| `packages/assemblyai` (`@ai-sdk/assemblyai`) | provider package | verified | `crates/ai-sdk-assemblyai` | `assemblyai_provider_transcribes_audio_with_headers_options_and_response`; `assemblyai_transcription_model_forwards_abort_signal_to_requests`; `assemblyai_transcription_model_aborts_before_polling_when_signal_is_aborted`; `assemblyai_transcription_duration_falls_back_to_last_word_end`; `assemblyai_transcription_model_maps_api_errors_to_metadata`; `assemblyai_provider_reports_unsupported_model_families_and_trait_transcription`; `assemblyai_provider_settings_serde_accepts_upstream_shape` | Portable provider-owned crate now mirrors upstream `createAssemblyAI` settings for `ASSEMBLYAI_API_KEY`, custom headers, AssemblyAI user-agent suffix, `assemblyai(...)`/`create_assemblyai`, `transcription`/`transcription_model`, provider-v4 trait integration, `/v2/upload` binary audio upload, `/v2/transcript` submit, transcript polling, upstream AssemblyAI provider options, abort-signal propagation across upload/submit/poll requests, response text/segments/language/duration metadata including last-word duration fallback, final response headers/body, error metadata, and unsupported language/embedding/image lookups. Remaining AssemblyAI work is live-provider validation and any upstream-only thrown-error edge cases. |
| `packages/azure` (`@ai-sdk/azure`) | provider package | verified | `crates/ai-sdk-azure`, `crates/ai-sdk-open-responses`, `src/openai_compatible.rs` | `azure_provider_creates_responses_model_with_resource_url_headers_and_api_version`; `azure_provider_creates_chat_model_with_deployment_based_url`; `azure_provider_creates_completion_model_with_default_v1_url`; `azure_provider_creates_embedding_and_image_models_with_upstream_urls`; `azure_provider_uses_default_aliases_and_provider_trait`; `azure_provider_settings_serde_accepts_upstream_shape`; `azure_provider_speech_uses_correct_url_format`; `azure_provider_speech_uses_deployment_based_url_when_enabled`; `azure_provider_transcription_uses_correct_url_format`; `azure_provider_transcription_uses_deployment_based_url_when_enabled`; `azure_provider_uses_azure_metadata_key_for_text_result`; `azure_provider_uses_azure_metadata_key_for_function_call_content`; `azure_provider_streams_azure_metadata_key_for_reasoning_and_finish`; shared Open Responses hosted-tool fixture tests `open_responses_provider_prepares_openai_hosted_tools`, `open_responses_provider_generates_web_search_fixture`, `open_responses_provider_streams_upstream_web_search_tool_fixture`, `open_responses_provider_generates_file_search_without_results_include_fixture_request_body`, `open_responses_provider_generates_file_search_with_results_include_fixture_content`, `open_responses_provider_sends_code_interpreter_request_body_with_include_and_tool`, `open_responses_provider_generates_code_interpreter_fixture_results`, `open_responses_provider_sends_image_generation_request_body_with_tool`, `open_responses_provider_generates_image_generation_fixture_results`, `open_responses_provider_streams_code_interpreter_results_with_annotations`, and `open_responses_provider_streams_image_generation_fixture_results` | Current upstream `azure-openai-provider.test.ts` has 55 cases. Azure-owned wrapper tests cover settings, URL/auth/factory metadata, speech/transcription, `assistant-` file id handling, and metadata-key selection; shared Open Responses tests cover the Azure-reused hosted tool, file-search, web-search, code-interpreter, image-generation, JSON, and SSE fixture matrix. Exceptions: live Azure credentials and JavaScript thrown-class identity are not CI-portable Rust requirements. |
| `packages/baseten` (`@ai-sdk/baseten`) | provider package | verified | `src/baseten.rs`, `src/openai_compatible.rs` | `baseten_provider_creates_default_chat_model_with_headers_and_base_url`; `baseten_provider_routes_custom_sync_chat_model_url_and_rejects_predict`; `baseten_provider_creates_embedding_model_for_sync_urls`; `baseten_provider_reports_unsupported_embedding_routes_and_images`; `baseten_provider_uses_default_base_url_and_function_alias`; `baseten_provider_implements_provider_trait`; `baseten_provider_settings_serde_accepts_upstream_urls` | Current upstream `baseten-provider.unit.test.ts` has 25 cases. The named Rust tests cover default/custom Model API base URLs, dedicated `modelURL`, `/sync` and `/sync/v1` routing, `/predict` rejection, `BASETEN_API_KEY`, custom headers, Baseten user-agent suffix, callable-style `baseten(...)`, `languageModel`/`chatModel`, provider-v4 trait integration, embedding aliases, and unsupported image lookups. Exceptions: JavaScript optional `modelId` overload/type ergonomics and the Performance Client embedding override are not separate Rust runtime requirements; Rust callers pass the model id explicitly and use the local OpenAI-compatible embedding transport. Live Baseten credentials remain ignored/credential-gated. |
| `packages/black-forest-labs` (`@ai-sdk/black-forest-labs`) | provider package | verified | `crates/ai-sdk-black-forest-labs` | `black_forest_labs_provider_creates_image_models_via_image_and_image_model`; `black_forest_labs_provider_configures_base_url_and_headers_correctly`; `black_forest_labs_provider_uses_provider_polling_options_for_timeout_behavior`; `black_forest_labs_provider_throws_nosuchmodelerror_for_unsupported_model_types`; `black_forest_labs_image_model_passes_correct_parameters_including_aspect_ratio_and_provider_options`; `black_forest_labs_image_model_includes_seed_in_provider_metadata_images_when_provided_by_api`; `black_forest_labs_image_model_includes_all_cost_and_megapixel_fields_when_provided_by_submit_api`; `black_forest_labs_image_model_omits_cost_and_megapixel_fields_from_provider_metadata_when_not_provided_by_submit_api`; `black_forest_labs_image_model_handles_null_cost_and_megapixel_fields_from_submit_api`; `black_forest_labs_image_model_calls_the_expected_urls_in_sequence`; `black_forest_labs_image_model_merges_provider_and_request_headers_for_submit_call`; `black_forest_labs_image_model_passes_merged_headers_to_polling_requests`; `black_forest_labs_image_model_warns_and_derives_aspect_ratio_when_size_is_provided`; `black_forest_labs_image_model_warns_and_ignores_size_when_both_size_and_aspect_ratio_are_provided`; `black_forest_labs_image_model_handles_api_errors_with_message_and_detail`; `black_forest_labs_image_model_handles_poll_responses_with_state_instead_of_status`; `black_forest_labs_image_model_polls_multiple_times_using_configured_interval_until_ready`; `black_forest_labs_image_model_uses_configured_poll_timeout_millis_and_poll_interval_millis_to_time_out`; `black_forest_labs_image_model_throws_when_poll_is_ready_but_sample_is_missing`; `black_forest_labs_image_model_throws_when_poll_returns_error_or_failed`; `black_forest_labs_image_model_includes_timestamp_headers_and_model_id_in_response_metadata`; `black_forest_labs_image_model_exposes_correct_provider_and_model_information`; `black_forest_labs_image_model_respects_abort_signal_before_submit`; ignored `live_black_forest_labs_image_generation_validates_provider_contract` | Provider-owned crate mirrors the upstream Black Forest Labs deterministic image-provider inventory for default/custom base URL, `BFL_API_KEY`, custom `x-key`/provider/request headers, Black Forest Labs user-agent suffix, `image`/`imageModel`, provider-v4 trait integration, unsupported language/embedding lookups, JSON submit to `/{modelId}`, poll URL `id` handling, `status`/`state` ready responses, binary final image fetch, abort propagation, `size` to `aspect_ratio` compatibility warnings, input image and mask conversion, provider-specific image options, response headers, timestamp/model metadata, image provider metadata for seed/timing/cost/megapixels, API/poll error metadata, provider creation aliases, and configured polling timeout behavior. Live BFL validation is represented by the named ignored credential-gated test. |
| `packages/bytedance` (`@ai-sdk/bytedance`) | provider package | verified | `crates/ai-sdk-bytedance` | `bytedance_video_model_generates_video_with_headers_body_and_metadata`; `bytedance_video_model_passes_unmapped_resolution_and_url_image`; `bytedance_video_model_polls_until_video_is_ready`; `bytedance_video_model_times_out_after_poll_timeout`; `bytedance_video_model_respects_abort_signal`; `bytedance_video_model_maps_api_and_status_errors_to_metadata`; `bytedance_provider_reports_unsupported_model_families_and_trait_video`; `bytedance_provider_settings_serde_accepts_upstream_shape` | Current upstream `bytedance-video-model.test.ts` has 41 cases. The provider-owned crate mirrors upstream `createByteDance` settings for default/custom base URL, `ARK_API_KEY`, bearer JSON headers, custom provider/request headers, `byteDance`/`createByteDance` via Rust `byte_dance`/`create_byte_dance`, `video`/`video_model`, provider-v4 trait integration, unsupported language/embedding/image lookups, task creation at `/contents/generations/tasks`, status polling until ready, poll timeout, abort propagation, URL video results, response headers, task id and usage provider metadata, standard video prompt/image/aspect/duration/seed/resolution request shaping with upstream resolution mapping, unsupported `fps`/`n` warnings, ByteDance provider options for watermark/audio/camera/last-frame/service-tier/draft/reference media/poll timing, passthrough options, and API/status error metadata. Exceptions: exact JavaScript thrown-error class identity and live ByteDance credentials are not CI-portable Rust requirements. |
| `packages/cerebras` (`@ai-sdk/cerebras`) | provider package | verified | `src/cerebras.rs`, `src/openai_compatible.rs` | `cerebras_provider_creates_a_provider_instance_with_default_options`; `cerebras_provider_creates_a_provider_instance_with_custom_options`; `cerebras_provider_passes_header`; `cerebras_provider_returns_a_chat_model_when_called_as_a_function`; `cerebras_provider_constructs_a_language_model_with_correct_configuration`; `cerebras_provider_throws_nosuchmodelerror_when_attempting_to_create_embedding_model`; `cerebras_provider_constructs_a_chat_model_with_correct_configuration` | Current upstream `cerebras-provider.test.ts` has 7 cases. The named Rust tests cover default/custom provider construction, `CEREBRAS_API_KEY`, custom headers, Cerebras user-agent suffix, callable-style `cerebras(...)`, `languageModel`/`chat`, provider-v4 trait integration, OpenAI-compatible `/chat/completions` with structured JSON schema output support, and unsupported embedding/image lookups. Exception: live Cerebras credentials are intentionally credential-gated rather than CI-required. |
| `packages/cohere` (`@ai-sdk/cohere`) | provider package | verified | `src/cohere.rs`; root export in `src/lib.rs` | `cohere_provider_creates_embedding_model_with_options_headers_and_usage`; `cohere_embedding_model_uses_default_input_type_and_exposes_raw_response`; `cohere_embedding_model_enforces_chunk_limit_and_validation_errors`; `cohere_provider_creates_reranking_model_with_object_warning_and_options`; `cohere_reranking_model_sends_text_documents_without_warnings`; `cohere_reranking_model_maps_validation_and_api_errors`; `cohere_chat_model_generates_text_reasoning_tool_calls_citations_usage_finish_and_raw_response`; `cohere_chat_model_maps_finish_reason_edges`; `cohere_chat_request_conversion_maps_prompt_tools_documents_reasoning_and_headers`; `cohere_chat_model_streams_text_reasoning_tool_calls_raw_chunks_and_metadata`; `cohere_chat_streaming_maps_parse_and_tool_argument_errors`; `cohere_chat_streaming_handles_empty_tool_call_arguments`; `cohere_chat_model_maps_api_and_schema_errors_to_rust_metadata`; `cohere_prepare_tools_maps_all_upstream_tool_choice_cases`; `convert_to_cohere_chat_prompt_maps_user_assistant_tool_files_and_unsupported_references`; `cohere_provider_creates_chat_model_and_reports_image_exception`; `cohere_provider_implements_embedding_and_reranking_traits`; `cohere_provider_settings_serde_accepts_upstream_base_url`; ignored live tests `live_cohere_chat_acceptance_requires_cohere_api_key`, `live_cohere_embedding_acceptance_requires_cohere_api_key`, `live_cohere_reranking_acceptance_requires_cohere_api_key` | Current refreshed upstream Cohere tests cover 75 `it(...)` cases across embedding, reranking, chat, tool preparation, and prompt conversion. The named Rust tests map every portable case one-to-one in `docs/ai-02-openai-compatible-providers.md`: `COHERE_API_KEY`, default/custom base URL, bearer auth, custom headers, user-agent suffix, callable-style `cohere(...)`, provider-v4 embedding/reranking/chat trait integration, `/embed` options/default input type/output dimension/usage/raw response/chunk limit/schema errors, `/rerank` text and object documents/warnings/ranking/raw response/schema/API errors, `/chat` request settings, reasoning/thinking options, JSON response format, system/user/assistant/tool prompt conversion, files/documents/images/detail options/provider-reference rejection, tool preparation and tool choices, text/reasoning/tool-call/citation/usage/finish/raw response extraction, streaming text/reasoning/tool-call/raw chunk/header metadata, parse and prototype-key error stream parts, and empty/null tool-call argument handling. Remaining exceptions are explicit: JavaScript thrown-error class identity and rejected-promise mechanics are represented as Rust-native provider metadata, unsupported error strings, or provider-v4 error stream parts, and real Cohere chat/embedding/rerank API acceptance is credential-gated by the ignored `COHERE_API_KEY` live tests. |
| `packages/deepgram` (`@ai-sdk/deepgram`) | provider package | verified | `crates/ai-sdk-deepgram` | `deepgram_speech_model_sends_headers_body_query_options_and_metadata`; `deepgram_speech_model_maps_format_and_warnings`; `deepgram_speech_model_includes_request_body_in_response`; `deepgram_speech_model_cleans_up_incompatible_parameters_when_encoding_changes_via_provider_options`; `deepgram_speech_model_cleans_up_incompatible_parameters_when_container_changes_encoding_implicitly`; `deepgram_transcription_model_sends_audio_query_headers_and_maps_response`; `deepgram_transcription_model_uses_real_date_when_no_custom_date_provider_is_specified`; `deepgram_transcription_model_returns_detected_language_from_inline_response`; `deepgram_transcription_model_returns_undefined_language_when_not_detected`; `deepgram_error_parses_resource_exhausted_error`; `deepgram_models_map_api_errors_to_metadata`; `deepgram_provider_reports_unsupported_model_families_and_traits`; `deepgram_provider_settings_serde_accepts_upstream_shape_and_default_factory` | Portable provider-owned crate mirrors upstream `createDeepgram` settings for `DEEPGRAM_API_KEY`, `authorization: Token ...`, custom headers, Deepgram user-agent suffix, callable-style default transcription factory, `speech`/`speech_model`, `transcription`/`transcription_model`, provider-v4 speech and transcription trait integration, unsupported language/embedding/text-embedding/image lookups with upstream messages, `/v1/speak` JSON TTS request body plus model/output-format/provider-option query mapping, unsupported voice/speed/language/instructions warnings, cleanup of incompatible Deepgram speech options when providerOptions change encoding or container, request-body retention in the speech result, `/v1/listen` raw-audio transcription request with media-type header, default `diarize=true`, runtime-sent Deepgram transcription provider options with schema-only fields accepted but not sent, transcript text/word segments/language/duration mapping, response headers/body, real-date response timestamps, inline language detection, undefined-language handling, and Deepgram error metadata parsing. Remaining Deepgram work is live-provider validation plus any upstream-only thrown-error class edges. |
| `packages/deepinfra` (`@ai-sdk/deepinfra`) | provider package | verified | `src/deepinfra.rs`, `src/openai_compatible.rs` | `deepinfra_provider_creates_chat_model_with_headers_and_base_url`; `deepinfra_chat_corrects_reasoning_usage_when_reasoning_exceeds_completion_tokens`; `deepinfra_chat_corrects_stream_finish_reasoning_usage`; `deepinfra_provider_creates_completion_model`; `deepinfra_provider_creates_embedding_model_aliases`; `deepinfra_provider_creates_image_model_and_generates_images`; `deepinfra_image_model_respects_abort_signal`; `deepinfra_image_model_edits_with_files_mask_and_provider_options`; `deepinfra_image_model_maps_generation_api_error_to_metadata`; `deepinfra_image_model_maps_edit_api_error_to_metadata`; `deepinfra_provider_uses_default_base_url_and_function_alias`; `deepinfra_provider_implements_provider_trait`; `deepinfra_provider_settings_serde_accepts_upstream_base_url` | Current upstream DeepInfra tests cover 25 cases across provider, chat usage correction, and image model files. The named Rust tests cover `createDeepInfra` settings for default/custom base URL, `DEEPINFRA_API_KEY`, custom headers, DeepInfra user-agent suffix, callable-style `deepinfra(...)`, provider-v4 trait integration, OpenAI-compatible chat/completion/embedding routes, reasoning-token usage correction for generate and stream, custom `/inference/{modelId}` image generation, abort propagation, `/openai/images/edits` multipart editing with repeated `image` fields and mask, provider options, base64 result mapping, response headers, and API error metadata. Exceptions: live DeepInfra credentials and JavaScript thrown-class identity are not CI-portable Rust requirements. |
| `packages/deepseek` (`@ai-sdk/deepseek`) | provider package | verified | `crates/ai-sdk-deepseek` | `deepseek_provider_creates_chat_model_with_headers_and_base_url`; `deepseek_provider_uses_default_base_url_and_function_aliases`; `deepseek_provider_reports_unsupported_model_families`; `deepseek_provider_implements_provider_trait`; `deepseek_provider_settings_serde_accepts_upstream_base_url` | Portable provider-owned crate now mirrors upstream `createDeepSeek` settings for default/custom base URL, `DEEPSEEK_API_KEY`, custom headers, DeepSeek user-agent suffix, `deep_seek()`/deprecated `deepseek()` aliases, `language_model`/`chat`, provider-v4 trait integration, OpenAI-compatible `/chat/completions` request routing, and unsupported embedding/image lookups. The remaining DeepSeek-specific chat semantics are covered by the shared OpenAI-compatible provider implementation, so no provider-owned crate debt remains. |
| `packages/elevenlabs` (`@ai-sdk/elevenlabs`) | provider package | verified | `crates/ai-sdk-elevenlabs` | `elevenlabs_speech_model_sends_headers_body_query_options_and_metadata`; `elevenlabs_speech_model_maps_format_defaults_and_instruction_warning`; `elevenlabs_transcription_model_sends_form_options_and_maps_response`; `elevenlabs_transcription_model_applies_upstream_defaults_when_options_object_is_present`; `elevenlabs_models_map_api_errors_to_metadata`; `elevenlabs_provider_reports_unsupported_model_families_and_traits`; `elevenlabs_provider_settings_serde_accepts_upstream_shape` | Portable provider-owned crate mirrors upstream `createElevenLabs` settings for `ELEVENLABS_API_KEY`, `xi-api-key`, custom headers, ElevenLabs user-agent suffix, `elevenlabs(...)`/`create_elevenlabs`, `speech`/`speech_model`, `transcription`/`transcription_model`, provider-v4 speech and transcription trait integration, unsupported language/embedding/image lookups, `/v1/text-to-speech/{voiceId}` JSON request shaping, upstream default voice/output format, ElevenLabs output-format aliases, speech provider options including voice settings, pronunciation dictionaries, text normalization, request-id continuity, seed, language fallback, `enable_logging` query mapping, unsupported instructions warnings, binary audio response mapping, `/v1/speech-to-text` multipart transcription with media-extension filename parity, default and provider-option transcription fields, transcript text/segments/language/duration mapping, response headers/body, timestamp/model metadata, and ElevenLabs error metadata. Remaining ElevenLabs work is live-provider validation and workflow serialization hooks. |
| `packages/fal` (`@ai-sdk/fal`) | provider package | verified | `crates/ai-sdk-fal` | `fal_image_model_maps_request_downloads_metadata_and_warnings`; `fal_image_model_converts_camel_case_provider_options_to_snake_case_for_api`; `fal_image_model_accepts_deprecated_snake_case_provider_options_with_warning`; `fal_image_model_maps_edit_files_masks_urls_base64_and_multiple_images`; `fal_image_model_warns_when_multiple_images_are_not_enabled`; `fal_image_model_parses_single_multiple_null_and_empty_metadata_responses`; `fal_video_model_maps_queue_response_and_metadata`; `fal_video_model_maps_provider_options_poll_overrides_and_custom_passthrough`; `fal_video_model_respects_abort_signal_before_queue_submit`; `fal_models_map_api_errors_to_provider_metadata`; ignored `live_fal_image_and_video_generation_validate_provider_contract` | Provider-owned media slice mirrors upstream `createFal` settings for default/custom image and queue base URLs, `FAL_API_KEY`/`FAL_KEY`, custom headers, Fal user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, image request shaping, image URL downloads, edit files/masks/data URIs, multiple-image warnings and provider option override, response schema edge cases, queue submit/poll response handling, video URL results, abort propagation, response headers, provider metadata, deprecated snake-case option warnings, and API error metadata. Fal speech and transcription are explicit non-media AI-04 handoffs rather than AI-03B media parity debt. |
| `packages/fireworks` (`@ai-sdk/fireworks`) | provider package | verified | `src/fireworks.rs`, `src/openai_compatible.rs` | `fireworks_provider_creates_chat_model_with_transformed_provider_options`; `fireworks_chat_request_transform_matches_upstream_reasoning_aliases`; `fireworks_provider_creates_completion_embedding_and_image_models`; `fireworks_provider_factory_split_and_option_aliases_match_upstream`; `fireworks_api_key_prefers_explicit_then_env`; `fireworks_image_model_sends_workflow_request_and_returns_binary`; `fireworks_image_model_maps_empty_body_and_api_errors_to_metadata`; `fireworks_image_model_sends_async_edit_input_image_and_warnings`; `fireworks_image_model_sends_edit_url_and_base64_inputs`; `fireworks_image_model_polls_multiple_times_until_ready`; `fireworks_image_model_sends_async_provider_options_to_submit_request`; `fireworks_image_model_maps_async_failure_timeout_and_missing_sample`; `fireworks_image_model_maps_image_generation_size_and_aspect_warning`; `fireworks_image_model_maps_workflow_size_warning`; `fireworks_image_model_aborts_before_request`; `fireworks_provider_uses_default_base_url_and_function_alias`; `fireworks_provider_implements_provider_trait`; `fireworks_provider_settings_serde_accepts_upstream_base_url`; ignored live tests `live_fireworks_chat_generation_validates_provider_contract`, `live_fireworks_workflow_image_generation_validates_provider_contract`, `live_fireworks_image_generation_route_validates_provider_contract`, `live_fireworks_async_image_generation_validates_polling_contract`, `live_fireworks_image_api_error_body_validates_provider_metadata` | Current upstream Fireworks tests cover 43 cases across provider construction and image model behavior. The named Rust tests map every portable case one-to-one in `docs/ai-02-openai-compatible-providers.md`: default/custom settings, `FIREWORKS_API_KEY`, custom headers, user-agent suffix, callable-style `fireworks(...)`, chat/completion/embedding/image factories, OpenAI-compatible chat/completion/embedding model construction, Fireworks reasoning option transforms, provider-v4 trait integration, workflow/image-generation/Kontext async URLs, prompt/aspect/size/seed/samples/provider-option body shaping, URL/base64/bytes edit inputs, warnings, binary responses, response headers/model metadata, abort propagation, empty-body/API error metadata, async multi-poll/failure/timeout/missing-sample behavior, and a real default HTTP transport. Remaining exceptions are explicit: JavaScript constructor mock identity and `_internal.currentDate` are `js-only-documented`, JavaScript `specificationVersion` is `type-system-impossible`, and real hosted chat/image/error acceptance is represented by ignored `FIREWORKS_API_KEY` live tests. |
| `packages/gladia` (`@ai-sdk/gladia`) | provider package | verified | `crates/ai-sdk-gladia` | `gladia_transcription_model_uploads_initiates_polls_and_maps_response`; `gladia_transcription_model_maps_provider_options_to_initiate_body`; `gladia_transcription_model_maps_api_and_status_errors_to_metadata`; `gladia_provider_reports_unsupported_model_families_and_trait_transcription`; `gladia_provider_settings_serde_accepts_upstream_shape` | Portable provider-owned crate mirrors upstream `createGladia` settings for `GLADIA_API_KEY`, `x-gladia-key`, custom headers, Gladia user-agent suffix, default `gladia()` transcription factory, provider-v4 transcription trait integration, unsupported language/embedding/image lookups, multipart `/v2/upload` audio upload with media-extension filename parity, `/v2/pre-recorded` initiate request shaping, broad Gladia provider option camel-to-snake mapping for context prompts, vocabulary, code switching, callbacks, subtitles, diarization, translation, summarization, moderation, named entities, chapterization, spelling, structured extraction, sentiment, audio-to-LLM, metadata, sentences, display mode, and punctuation, result URL polling, transcript text/segments/language/duration mapping, response headers, provider metadata containing the Gladia result, status/API error metadata, and timestamp/model metadata. Remaining Gladia work is live-provider validation and workflow serialization hooks. |
| `packages/groq` (`@ai-sdk/groq`) | provider package | verified | `src/groq.rs`, `crates/ai-sdk-openai-compatible/src/openai_compatible.rs` | `groq_provider_creates_chat_model_with_headers_base_url_and_provider_options`; `groq_chat_do_generate_maps_content_usage_response_and_request_metadata`; `groq_chat_maps_reasoning_effort_variants_and_provider_override`; `groq_chat_passes_provider_options_response_formats_and_validation`; `groq_prepare_tools_maps_function_choices_strict_and_browser_search`; `groq_chat_converts_messages_images_reasoning_tool_results_and_rejects_provider_refs`; `groq_chat_converts_usage_with_reasoning_edges`; `groq_chat_do_stream_maps_text_reasoning_tools_usage_raw_chunks_and_errors`; `groq_chat_streaming_maps_unparsable_stream_parts`; `groq_provider_creates_transcription_model_with_headers_options_and_response`; `groq_transcription_uses_custom_current_date_and_maps_errors`; `groq_provider_implements_transcription_trait`; `groq_provider_uses_default_base_url_and_function_alias`; `groq_provider_reports_unsupported_model_families`; `groq_provider_implements_provider_trait`; `groq_provider_settings_serde_accepts_upstream_base_url`; ignored `live_groq_chat_tools_and_transcription_validate_provider_contract` | Current upstream Groq tests cover 95 `it(...)` cases across chat, options, tool preparation, message conversion, usage conversion, and transcription. The named Rust tests map every portable case one-to-one in `docs/ai-02-openai-compatible-providers.md`: `GROQ_API_KEY`, base URL defaults, custom headers, Groq user-agent suffix, callable-style `groq(...)`, provider-v4 trait integration, OpenAI-compatible chat construction, Groq `reasoning_effort` aliases, provider option validation, structured-output options and warnings, browser-search provider tools and unsupported warnings, function tools/tool-choice/strict mode, Groq message conversion for images/reasoning/tool calls/tool results/provider-reference rejection, Groq usage conversion, SSE text/reasoning/tool/error/parse-error/raw chunk behavior, `x_groq.usage` stream usage, response metadata/headers, transcription multipart requests to `/audio/transcriptions`, default multipart HTTP transport, transcription provider options, custom current-date injection through Rust `with_current_date`, text/segment/language/duration/raw-response mapping, and unsupported embedding/image model errors. Shared OpenAI-compatible edits are limited to Groq parity: accepting non-stream Groq `message.reasoning` and stream `x_groq.usage`. Remaining exceptions are explicit: TypeScript option inference is `type-system-impossible`, exact JavaScript thrown-error class identity is represented as Rust provider error metadata, and real hosted chat/stream/browser-search/transcription API acceptance is credential-gated by ignored `live_groq_chat_tools_and_transcription_validate_provider_contract` with `GROQ_API_KEY`. |
| `packages/huggingface` (`@ai-sdk/huggingface`) | provider package | verified | `src/huggingface.rs` | `huggingface_provider_generates_text_with_request_and_response_metadata`; `huggingface_responses_maps_system_provider_options_and_structured_output`; `huggingface_responses_converts_images_tool_messages_and_content_parts`; `huggingface_responses_resolves_top_level_image_media_types`; `huggingface_responses_reports_unsupported_provider_references`; `huggingface_responses_maps_warnings_and_stream_errors`; `huggingface_responses_prepares_tools_and_tool_choices`; `huggingface_responses_respects_abort_signal`; `huggingface_responses_streams_text_with_request_and_response_metadata`; `huggingface_responses_streams_reasoning_text_and_tool_calls`; `huggingface_provider_reports_unsupported_embedding_and_image`; `huggingface_provider_uses_default_base_url_and_function_alias`; `huggingface_provider_implements_provider_trait`; `huggingface_provider_settings_serde_accepts_upstream_base_url` | Current upstream Hugging Face tests cover 37 cases across provider construction and Responses language model behavior. The named Rust tests cover `createHuggingFace` settings for default/custom base URL, `HUGGINGFACE_API_KEY`, custom headers, Hugging Face user-agent suffix, callable-style `huggingface(...)`, `responses`/`languageModel`, provider-v4 trait integration, non-streaming `/responses`, abort propagation through provider API requests, SSE streaming for text/reasoning/tool-call events, system/user/assistant message conversion, inline image media-type resolution, URL image prompt parts, function tool preparation and `auto`/`required`/specific tool choices, tool-message warnings, MCP/function-call content extraction, response text/reasoning/source/provider-executed tool content extraction, usage conversion, structured output `text.format`, provider-specific `metadata`/`instructions`/`reasoningEffort` options, unsupported embedding/image messages, provider-reference rejection, and warning propagation for unsupported request options on error responses. Exceptions: live Hugging Face credentials, JavaScript thrown-class identity, and Zod-specific validation mechanics are not CI-portable Rust requirements. |
| `packages/hume` (`@ai-sdk/hume`) | provider package | verified | `crates/ai-sdk-hume` | `hume_provider_creates_speech_model_with_headers_options_and_body`; `hume_speech_model_defaults_voice_format_and_warns_for_unsupported_inputs`; `hume_speech_model_uses_real_date_when_no_custom_date_provider_is_specified`; `hume_speech_model_handles_different_audio_formats`; `hume_speech_model_includes_warnings_if_any_are_generated`; `hume_speech_model_maps_generation_context_and_api_errors_to_metadata`; `hume_error_data_schema_parses_hume_resource_exhausted_error`; `hume_provider_reports_unsupported_model_families_and_trait_speech`; `hume_provider_settings_serde_accepts_upstream_shape`; `hume_default_factory_creates_speech_model` | Portable provider-owned crate mirrors upstream `createHume` settings for `HUME_API_KEY`, `X-Hume-Api-Key`, custom headers, Hume user-agent suffix, default `hume()` speech factory, provider-v4 trait integration, unsupported language/embedding/image lookups, `/v0/tts/file` JSON request shaping, upstream default voice id, `mp3`/`pcm`/`wav` output formats, unsupported output-format and language warnings, provider `context.generationId` and context utterance mapping, binary audio response mapping, response headers, timestamp/model metadata, and Hume error metadata. Remaining Hume work is live-provider validation. |
| `packages/klingai` (`@ai-sdk/klingai`) | provider package | verified | `crates/ai-sdk-klingai` | `klingai_t2v_model_maps_body_headers_polling_and_metadata`; `klingai_i2v_model_maps_file_image_and_aspect_ratio_warning`; `klingai_video_model_maps_api_errors_to_provider_metadata`; `klingai_motion_control_requires_provider_options`; `klingai_auth_generates_valid_hs256_jwt_structure`; `klingai_auth_includes_issuer_exp_and_nbf_claims`; `klingai_auth_signs_with_secret_and_changes_for_different_secret`; `klingai_auth_reports_missing_explicit_credentials`; `klingai_t2v_model_maps_extended_provider_options_without_element_list`; `klingai_i2v_model_maps_tail_masks_multishot_voice_and_watermark_options`; `klingai_motion_control_maps_required_provider_options_and_image_url`; ignored `live_klingai_video_generation_validates_provider_contract` | Provider-owned media slice mirrors upstream KlingAI settings for default/custom base URL, `KLINGAI_ACCESS_KEY`, `KLINGAI_SECRET_KEY`, HS256 bearer auth and signing edges, custom headers, KlingAI user-agent suffix, `video`/`video_model`, provider-v4 trait integration, text-to-video, image-to-video, and motion-control endpoints, task polling, URL video results, task/video provider metadata, file image base64 conversion, unsupported standard option warnings, provider-option request mapping, passthrough options, abort propagation, and API/error metadata. Live KlingAI validation is represented by the named ignored credential-gated test. |
| `packages/lmnt` (`@ai-sdk/lmnt`) | provider package | verified | `crates/ai-sdk-lmnt` | `lmnt_provider_creates_speech_model_with_headers_options_and_body`; `lmnt_speech_model_defaults_voice_format_and_warns_for_unsupported_format`; `lmnt_speech_model_maps_error_response_to_metadata`; `lmnt_provider_reports_unsupported_model_families_and_trait_speech`; `lmnt_default_factory_creates_speech_model` | Portable provider-owned crate mirrors upstream `createLMNT` settings for `LMNT_API_KEY`, custom headers, LMNT user-agent suffix, `lmnt()`/`create_lmnt`, `speech`/`speech_model`, provider-v4 trait integration, `/v1/ai/speech/bytes` request shaping, default voice/output format, supported output formats, unsupported format warnings, LMNT provider options, binary audio response mapping, response headers, error metadata, and unsupported language/embedding/image lookups. Remaining LMNT work is live-provider validation plus any upstream-only thrown-error class edges. |
| `packages/luma` (`@ai-sdk/luma`) | provider package | verified | `crates/ai-sdk-luma` | `luma_provider_creates_image_model_with_headers_body_and_metadata`; `luma_image_model_maps_reference_images_and_warnings`; `luma_image_model_sends_image_by_default_when_url_file_is_provided`; `luma_image_model_sends_style_and_modify_image_reference_modes`; `luma_image_model_uses_custom_reference_weights_and_character_identities`; `luma_image_model_reports_editing_validation_errors_to_metadata`; `luma_image_model_reports_editing_limits_for_masks_base64_and_too_many_images`; `luma_image_model_maps_api_and_status_errors_to_metadata`; `luma_image_model_parses_response_with_image_references`; `luma_image_model_parses_response_with_style_references`; `luma_image_model_parses_response_with_character_references`; `luma_image_model_parses_response_with_modify_image_and_multiple_reference_types`; `luma_image_model_respects_abort_signal_before_submit`; `luma_provider_reports_unsupported_model_families_and_trait_image`; `luma_provider_settings_serde_accepts_upstream_shape`; ignored `live_luma_image_generation_validates_provider_contract` | Provider-owned crate mirrors upstream `createLuma` settings for default/custom base URL, `LUMA_API_KEY`, bearer auth, custom provider/request headers, Luma user-agent suffix, `luma()`/`create_luma`, `image`/`image_model`, provider-v4 trait integration, unsupported language/embedding lookups, async image generation submit to `/dream-machine/v1/generations/image`, polling `/dream-machine/v1/generations/{id}`, generated image download without provider auth headers, response header/timestamp/model metadata, unsupported seed/size warnings, Luma provider option passthrough, poll override extraction, URL-only reference images for `image`/`style`/`character`/`modify_image`, reference weighting/identity mapping, editing validation errors, response schema reference variants, abort propagation, and API/status error metadata. JavaScript thrown-class identity, Zod mechanics, and workflow serialization hooks are not CI-portable media parity requirements; live Luma validation is represented by the named ignored credential-gated test. |
| `packages/mistral` (`@ai-sdk/mistral`) | provider package | verified | `crates/ai-sdk-mistral` | `mistral_provider_creates_chat_model_with_headers_and_base_url`; `mistral_provider_creates_embedding_model_with_usage_and_headers`; `mistral_provider_uses_default_base_url_and_function_alias`; `mistral_provider_reports_unsupported_image_models`; `mistral_provider_settings_serde_accepts_upstream_base_url` | Portable provider-owned crate now mirrors upstream `createMistral` settings for default/custom base URL, `MISTRAL_API_KEY`, custom headers, Mistral user-agent suffix, callable-style `mistral(...)`, `language_model`/`chat`, embedding/text-embedding aliases, provider-v4 trait integration, OpenAI-compatible `/chat/completions` and `/embeddings` routing, Mistral prompt conversion, `safe_prompt`, document image/page limits, structured-output options, strict JSON schema handling, parallel tool-call options, reasoning effort, tool preparation, usage conversion, SSE fixture coverage, generated ids, and exact error-schema mapping. Remaining Mistral work is live-provider validation and any upstream-only thrown-error edge cases. |
| `packages/moonshotai` (`@ai-sdk/moonshotai`) | provider package | verified | `crates/ai-sdk-moonshotai` | `moonshotai_provider_creates_chat_model_with_headers_options_and_usage`; `moonshotai_provider_streams_chat_with_options_and_usage`; `moonshotai_provider_uses_default_base_url_and_function_alias`; `moonshotai_provider_reports_unsupported_model_families`; `moonshotai_provider_implements_provider_trait`; `moonshotai_provider_settings_serde_accepts_upstream_base_url`; `moonshotai_language_model_options_serde_match_upstream_shape`; `moonshotai_usage_conversion_handles_null_and_token_details` | Current upstream MoonshotAI tests cover 17 cases across provider construction and chat usage conversion. The provider-owned crate mirrors upstream `createMoonshotAI` settings for default/custom base URL, `MOONSHOT_API_KEY`, custom headers, MoonshotAI user-agent suffix, callable-style `moonshotai()`, `language_model`/`chat_model`, provider-v4 trait integration, OpenAI-compatible `/chat/completions` request routing and SSE streaming, upstream `thinking.budgetTokens` to `thinking.budget_tokens` and `reasoningHistory` to `reasoning_history` body transformation, top-level `cached_tokens` and reasoning-token usage conversion for generate and stream finishes, and unsupported embedding/image lookups. Shared OpenAI-compatible tests own generic message conversion, tool preparation, and transport error behavior. Exceptions: live MoonshotAI credentials, JavaScript thrown-class identity, workflow serialization hooks outside the current upstream package tests, and Zod-specific validation mechanics are not CI-portable Rust requirements. |
| `packages/perplexity` (`@ai-sdk/perplexity`) | provider package | verified | `crates/ai-sdk-perplexity` | `perplexity_provider_creates_language_model_with_headers_and_base_url`; `perplexity_provider_uses_default_base_url_and_function_alias`; `perplexity_provider_reports_unsupported_model_families`; `perplexity_provider_implements_provider_trait`; `perplexity_provider_settings_serde_accepts_upstream_base_url`; `convert_to_perplexity_messages_converts_system_user_assistant_messages`; `convert_to_perplexity_messages_handles_tool_messages_and_provider_references`; `convert_to_perplexity_messages_handles_top_level_media_type_resolution`; `convert_to_perplexity_messages_passes_full_image_and_url_cases_through_unchanged`; `perplexity_provider_passes_through_perplexity_provider_options`; `perplexity_provider_supports_json_response_format`; `perplexity_provider_supports_json_response_format_and_provider_options`; `perplexity_provider_warns_about_unsupported_reasoning`; `perplexity_provider_handles_pdf_and_image_files_in_request_messages`; `perplexity_provider_streams_sources_and_usage`; `perplexity_provider_streams_raw_chunks` | Portable provider-owned crate now mirrors upstream `createPerplexity` settings for default/custom base URL, `PERPLEXITY_API_KEY`, custom headers, Perplexity user-agent suffix, `perplexity()` factory, `language_model`, provider-v4 trait integration, OpenAI-compatible `/chat/completions` request routing, provider id `perplexity`, unsupported embedding/image lookups, Perplexity-specific message conversion warnings, citation/source extraction, images/search usage/cost provider metadata, JSON-schema response format, SSE stream parsing, and error schema mapping. Remaining live-provider validation is outside the portable slice. |
| `packages/prodia` (`@ai-sdk/prodia`) | provider package | verified | `crates/ai-sdk-prodia` | `prodia_image_model_maps_json_request_multipart_response_and_metadata`; `prodia_image_model_maps_size_provider_options_loras_progressive_and_warnings`; `prodia_video_model_maps_json_request_and_binary_video_response`; `prodia_video_model_maps_prompt_seed_resolution_and_json_shape`; `prodia_video_model_sends_multipart_for_image_to_video`; `prodia_url_video_input_downloads_media_before_multipart_post`; `prodia_provider_metadata_includes_and_omits_dollars_like_upstream`; `prodia_image_model_respects_abort_signal_before_submit`; `prodia_models_map_api_errors_to_metadata`; ignored `live_prodia_image_and_video_generation_validate_provider_contract` | Provider-owned media slice mirrors upstream Prodia settings for default/custom base URL, `PRODIA_TOKEN`, bearer auth, custom headers, Prodia user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, `/job?price=true` submit, JSON and multipart request bodies, image/video multipart response parsing, image/video binary results, URL media download before multipart image-to-video, abort propagation, response headers, job provider metadata including optional/null price fields, and API error metadata. The Prodia language model is an explicit non-media AI-04 handoff rather than AI-03B media parity debt. |
| `packages/replicate` (`@ai-sdk/replicate`) | provider package | verified | `crates/ai-sdk-replicate` | `replicate_image_model_maps_flux2_request_downloads_and_warnings`; `replicate_image_model_maps_version_wait_header_and_editing_options`; `replicate_image_model_warns_for_flux2_image_limit_and_mask`; `replicate_image_model_warns_when_extra_non_flux_images_are_ignored`; `replicate_video_model_maps_prediction_response_and_metadata`; `replicate_video_model_maps_prediction_body_provider_options_and_wait_headers`; `replicate_video_model_maps_failed_canceled_and_missing_output_predictions`; `replicate_video_model_respects_abort_signal_before_prediction_submit`; `replicate_image_model_maps_api_errors_to_metadata`; ignored `live_replicate_image_and_video_generation_validate_provider_contract` | Provider-owned media slice mirrors upstream Replicate settings for default/custom base URL, `REPLICATE_API_TOKEN`, bearer auth, custom headers, Replicate user-agent suffix, `image`/`image_model`, `video`/`video_model`, provider-v4 trait integration, versioned and owner/model prediction endpoints, `Prefer: wait` handling, Flux 2 multi-image request mapping and limits, non-Flux image warnings, image editing and mask mapping, output URL downloads, video URL results, failed/canceled/missing-output prediction errors, abort propagation, response headers, prediction provider metadata, and API error metadata. Live Replicate validation is represented by the named ignored credential-gated test. |
| `packages/revai` (`@ai-sdk/revai`) | provider package | verified | `crates/ai-sdk-revai` | `revai_transcription_model_transcribes_audio_with_headers_options_and_response`; `revai_transcription_multipart_uses_media_type_extension_for_filename`; `revai_transcription_duration_falls_back_for_open_segment`; `revai_transcription_model_maps_api_and_status_errors_to_metadata`; `revai_provider_reports_unsupported_model_families_and_trait_transcription`; `revai_provider_settings_serde_accepts_upstream_shape` | Portable provider-owned crate mirrors upstream `createRevai` settings for `REVAI_API_KEY`, bearer auth, Rev.ai user-agent suffix, custom provider/request headers, `revai(...)`/`create_revai`, `transcription`/`transcription_model`, provider-v4 trait integration, unsupported language/embedding/image lookups with upstream messages, multipart `media` + `config` submit to `/speechtotext/v1/jobs`, polling `/speechtotext/v1/jobs/{id}`, transcript fetch `/speechtotext/v1/jobs/{id}/transcript`, transcript text/segment/duration/language/response metadata mapping, raw transcript body/headers, provider option passthrough, status/API error metadata, dependency-free multipart default transport, and browser `File` filename/media-extension parity via the Rev.ai multipart encoder. Remaining Rev.ai work is live-provider validation plus workflow serialization hooks. |
| `packages/togetherai` (`@ai-sdk/togetherai`) | provider package | verified | `src/togetherai.rs`, `src/openai_compatible.rs` | `togetherai_provider_creates_chat_model_with_headers_base_url_and_body`; `togetherai_provider_creates_completion_model`; `togetherai_provider_creates_embedding_model_aliases`; `togetherai_provider_creates_image_model_and_generates_images`; `togetherai_image_model_maps_file_inputs_to_image_url`; `togetherai_image_model_validates_provider_options_before_request`; `togetherai_image_model_maps_api_error_to_metadata`; `togetherai_image_model_aborts_before_request`; `togetherai_image_model_reports_unsupported_mask_without_request`; `togetherai_provider_creates_reranking_model`; `togetherai_reranking_model_sends_text_documents_without_warnings`; `togetherai_reranking_model_validates_provider_options_before_request`; `togetherai_reranking_model_maps_api_error_to_metadata`; `togetherai_reranking_model_aborts_before_request`; `togetherai_provider_uses_default_base_url_and_function_alias`; `togetherai_api_key_prefers_explicit_then_new_env_then_deprecated_env`; `togetherai_provider_implements_provider_trait`; `togetherai_provider_settings_serde_accepts_upstream_base_url`; ignored `live_togetherai_image_and_rerank_validate_provider_contract` | All 40 refreshed upstream TogetherAI `it(...)` cases are mapped one-to-one in `docs/ai-02-openai-compatible-providers.md`. Provider foundation mirrors upstream `createTogetherAI` settings for default/custom base URL, `TOGETHER_API_KEY`, deprecated `TOGETHER_AI_API_KEY` fallback precedence, custom headers, TogetherAI user-agent suffix, callable-style `togetherai(...)`, provider-v4 trait integration, OpenAI-compatible `/chat/completions`, `/completions`, and `/embeddings` models with provider ids `togetherai.chat`, `togetherai.completion`, and `togetherai.embedding`. Custom image generation covers `/images/generations`, base64/URL/bytes input image shaping, `size` width/height, `n`, seed, single-image warning, aspect-ratio warning, known provider-option validation, provider-option passthrough, max images per call, response headers/model metadata, API error metadata, mask error metadata, and abort-before-request propagation. Reranking covers `/rerank`, text/object documents, `top_n`, `rankFields` validation and mapping to `rank_fields`, `return_documents: false`, ranking mapping, raw response body, response headers/model/id metadata, API error metadata, no-warning/no-provider-metadata paths, and abort-before-request propagation. Remaining exceptions are explicit: JavaScript `console.warn` deprecation side effects are JS-only, exact thrown-error class identity is represented as Rust provider error metadata, and real TogetherAI image/rerank API acceptance is credential-gated by the ignored live test with `TOGETHER_API_KEY` or deprecated `TOGETHER_AI_API_KEY`. |
| `packages/vercel` (`@ai-sdk/vercel`) | provider package | verified | `src/vercel.rs` | `vercel_provider_creates_openai_compatible_chat_model`; `vercel_provider_uses_default_base_url_and_function_alias`; `vercel_provider_reports_unsupported_model_families`; `vercel_provider_implements_provider_trait` | Current upstream `vercel-provider.test.ts` has 4 cases. The named Rust tests cover upstream `@ai-sdk/vercel` v0 provider settings, default/custom base URL, `VERCEL_API_KEY` lookup, custom headers, `vercel.chat` OpenAI-compatible chat model creation, Vercel user-agent suffix, callable-style `vercel(...)`, provider-v4 trait integration, and unsupported embedding/image model errors. Exceptions: live Vercel v0 credentials and future upstream package expansion are not CI-portable closure criteria for the current snapshot. |
| `packages/voyage` (`@ai-sdk/voyage`) | provider package | verified | `src/voyage.rs` | `voyage_provider_creates_embedding_model_with_options_headers_and_sorted_results`; `voyage_embedding_model_chunks_at_128_and_maps_api_error_to_metadata`; `voyage_provider_creates_reranking_model_with_object_warning_and_options`; `voyage_reranking_model_maps_api_error_to_metadata`; `voyage_provider_reports_unsupported_language_and_image_models`; `voyage_provider_uses_default_base_url_and_factory_alias`; `voyage_provider_implements_provider_traits`; `voyage_provider_settings_serde_accepts_upstream_base_url`; `voyage_api_key_prefers_explicit_then_env`; `voyage_embedding_direct_call_reports_too_many_values` | Portable provider foundation mirrors upstream `createVoyage` settings for default/custom base URL, `VOYAGE_API_KEY`, custom headers, Voyage user-agent suffix, `voyage()`/`create_voyage`, embedding/text-embedding aliases, reranking aliases, provider-v4 trait integration, `/embeddings` request options (`input_type`, `truncation`, `output_dimension`, `output_dtype`), 128-value chunking, response index sorting, `/rerank` request options (`top_k`, `return_documents`, `truncation`), object-document string conversion compatibility warnings, response headers/raw bodies, API error metadata from `detail`, and unsupported language/image lookups. Remaining Voyage work is live-provider validation plus full zod-equivalent provider-option validation. |
| `packages/otel` (`@ai-sdk/otel`) | telemetry package | verified | `crates/ai-sdk-otel` | `select_attributes_matches_telemetry_recording_flags`; `assemble_operation_name_includes_function_id_when_present`; `maps_provider_and_operation_names_to_genai_semconv_values`; `formats_system_and_input_messages`; `formats_output_messages_and_finish_reasons`; `base_and_supplemental_attributes_match_upstream_prefixes`; `stringify_for_telemetry_converts_file_data_to_strings`; `record_span_executes_function_records_attributes_and_ends_by_default`; `record_span_can_leave_successful_span_open`; `record_span_records_exception_status_and_ends_on_error`; `record_error_on_span_sets_status_only_for_non_error_values`; `mock_and_noop_tracers_match_upstream_test_shapes`; `open_telemetry_records_generate_text_root_step_and_chat_spans`; `open_telemetry_records_object_operation_and_step_spans`; `open_telemetry_enrichment_keeps_official_attribute_precedence`; `open_telemetry_records_tool_span_and_wraps_execute_tool`; `open_telemetry_records_error_on_active_spans_and_cleans_state`; `open_telemetry_records_embedding_operation_and_inner_span`; `open_telemetry_records_rerank_operation_and_inner_span`; `legacy_open_telemetry_records_generate_text_step_tool_and_root_spans`; `legacy_open_telemetry_records_object_embedding_and_rerank_spans`; `otlp_http_json_payload_uses_collector_shape`; `local_otlp_http_receiver_captures_exported_span_payload`; `real_opentelemetry_http_exporter_sends_json_to_local_receiver`; `open_telemetry_integration_exports_dispatcher_spans_to_local_otlp_receiver`; `legacy_open_telemetry_integration_exports_dispatcher_spans_to_local_otlp_receiver`; ignored `live_vercel_ai_gateway_openai_compatible_generate_text_with_otel`; ignored `live_vercel_ai_gateway_openai_compatible_stream_text_with_otel`; ignored `live_vercel_ai_gateway_openai_compatible_generate_object_with_otel`; ignored `live_vercel_ai_gateway_openai_compatible_stream_object_with_otel`; ignored `live_vercel_ai_gateway_openai_responses_generate_text_with_otel`; ignored `live_vercel_ai_gateway_openai_responses_stream_text_with_otel`; `scripts/check-otel-loopback.sh`; `cargo run -p ai-sdk-otel --example local_otlp_receiver` | Initial package-owned crate covers portable helper behavior from upstream `@ai-sdk/otel`: telemetry/input/output attribute gating, operation/resource attribute naming, provider and operation GenAI semantic-convention mapping, system/input/output/object message formatting, file/base64/URL prompt telemetry stringification, base model-call attributes, supplemental attribute selection, runtime-context/header/detailed-usage attribute helpers, finish-reason mapping, and dependency-free Rust analogues for `recordSpan`, `recordErrorOnSpan`, `MockTracer`, noop tracer behavior, `OpenTelemetry` operation/step/language-model/tool/object/embedding/reranking lifecycle span recording, `LegacyOpenTelemetry` legacy `ai.*` text/tool/object/embedding/reranking span recording, enrichment precedence, active-span error cleanup, OTLP/HTTP JSON export payload construction, a loopback local OTLP receiver that captures actual HTTP wire payloads, a `real-opentelemetry` feature that configures the real Rust `opentelemetry` SDK OTLP/HTTP JSON exporter against that receiver, root dispatcher adapters that register `OpenTelemetry` and `LegacyOpenTelemetry` as normal telemetry integrations and export dispatcher-produced spans to the receiver, a required `scripts/check-otel-loopback.sh` proof command that also runs live Gateway OpenAI-compatible generate, stream, object, and stream-object telemetry checks plus Gateway OpenAI Responses generate and stream telemetry checks when live mode is enabled, and a runnable daemon-style local receiver example for manual collector-style validation. The live Gateway telemetry proof expanded on 2026-05-25 to include successful ignored runs for `live_vercel_ai_gateway_openai_compatible_generate_text_with_otel`, `live_vercel_ai_gateway_openai_compatible_stream_text_with_otel`, `live_vercel_ai_gateway_openai_compatible_generate_object_with_otel`, `live_vercel_ai_gateway_openai_compatible_stream_object_with_otel`, `live_vercel_ai_gateway_openai_responses_generate_text_with_otel`, and `live_vercel_ai_gateway_openai_responses_stream_text_with_otel`, and `scripts/check-otel-loopback.sh --live-gateway` passed again on 2026-05-26. Remaining work: broader provider live tests outside the current Gateway telemetry coverage that enable telemetry and assert emitted OTLP data through the local receiver or a collector. |
...
| Error types and messages | verified | `crates/ai-sdk-provider/src/provider.rs`, `crates/ai-sdk-provider-utils`, `crates/ai-sdk-gateway/src/gateway_error.rs`, `src/generate_text.rs`, model modules | Existing error serialization/message tests; `gateway_error::*` tests | Many upstream errors exist, including Gateway-specific error classifications. Concrete provider-specific error surfaces are tracked in their package-owned rows; core/provider-utils/Gateway portable error-message behavior is mapped here with named Rust tests or explicit JavaScript class-identity exceptions. |
...
| Prompt helper and file-part inventory | verified | `src/prompt.rs` | `timeout_configuration_serializes_number_form`; `timeout_configuration_serializes_detailed_form`; `timeout_configuration_deserializes_detailed_form`; `request_options_serializes_upstream_shape_without_abort_signal`; `request_options_deserializes_minimal_shape`; `instructions_serialize_upstream_union_shapes`; `prompt_serializes_text_prompt_with_optional_standard_fields`; `prompt_deserializes_messages_form`; `prompt_input_supports_message_array_in_prompt_field`; `prompt_deserialization_rejects_invalid_prompt_union_shapes`; `standardized_prompt_prepends_instructions_as_system_messages`; `prepare_tool_choice_returns_auto_when_tool_choice_is_not_provided`; `prepare_tool_choice_handles_string_tool_choice_none`; `prepare_tool_choice_handles_object_tool_choice`; `prepare_tool_choice_handles_string_tool_choice_auto`; `prepare_tool_choice_handles_string_tool_choice_required`; `convert_data_content_to_base64_string_passes_base64_strings_through`; `convert_data_content_to_base64_string_encodes_bytes`; `convert_to_language_model_v4_file_part_should_wrap_a_uint8_array_as_type_data_data`; `convert_to_language_model_v4_file_part_should_wrap_an_array_buffer_converted_to_uint8_array_as_type_data_data`; `convert_to_language_model_v4_file_part_should_wrap_a_base64_string_that_is_not_a_url_as_type_data_data`; `convert_to_language_model_v4_file_part_should_convert_a_url_string_into_type_url_url`; `convert_to_language_model_v4_file_part_should_pass_through_a_url_instance_as_type_url_url`; `convert_to_language_model_v4_file_part_should_extract_base64_and_media_type_from_a_data_url_into_type_data_data`; `convert_to_language_model_v4_file_part_should_wrap_a_provider_reference_as_type_reference_reference`; `convert_to_language_model_v4_file_part_should_unwrap_type_data_data_uint8_array`; `convert_to_language_model_v4_file_part_should_unwrap_type_data_data_array_buffer`; `convert_to_language_model_v4_file_part_should_unwrap_type_data_data_base64_string_that_is_not_a_url`; `convert_to_language_model_v4_file_part_should_reject_type_data_data_data_url_string_because_data_urls_are_not_inline_data`; `convert_to_language_model_v4_file_part_should_unwrap_type_url_url_into_type_url_url`; `convert_to_language_model_v4_file_part_should_unwrap_type_url_url_with_data_url_into_base64_and_media_type`; `convert_to_language_model_v4_file_part_should_pass_through_type_reference_reference`; `convert_to_language_model_v4_file_part_should_pass_through_type_text_text`; `convert_to_language_model_v4_file_part_should_make_type_data_data_bytes_equal_bare_bytes`; `convert_to_language_model_v4_file_part_should_make_type_url_url_equal_bare_url`; `convert_to_language_model_v4_file_part_should_make_type_reference_reference_equal_bare_reference`; `invalid_data_content_error_matches_upstream_default_message`; `invalid_data_content_error_uses_json_typeof_for_default_message`; `invalid_data_content_error_supports_custom_message_and_parts`; `message_conversion_error_retains_original_message_and_message_text`; `message_conversion_error_supports_parts_conversion`; `invalid_message_role_error_matches_upstream_default_message`; `invalid_message_role_error_supports_custom_message_and_parts` | Mirrors the remaining portable upstream `prompt` helper and file-part conversion cases one-to-one, including prompt/timeouts, instructions, prompt serialization/deserialization, tool-choice preparation, base64 data conversion, all `convertToLanguageModelV4FilePart` branches, invalid-data-content errors, message-conversion errors, and invalid-message-role errors. |
...
| `generateText` non-streaming | verified | `src/generate_text.rs` | `generate_text_*` tests in `src/generate_text.rs`; `examples/kitchen_sink.rs` | Covers deterministic model calls, retryable pre-content provider failures up to `maxRetries`, `maxRetries` start-event configuration, tool loops, prepareStep contexts, initial approved-tool response messages, model-switch URL support checks, result shaping, usage, warnings, metadata, and response messages. |
...
| `streamObject` | verified | `src/stream_object.rs`, `src/lib.rs`, `crates/ai-sdk-provider/src/language_model.rs`, `crates/ai-sdk-provider-utils/src/provider_utils.rs`, `crates/ai-sdk-gateway/src/gateway.rs`, `crates/ai-sdk-openai-compatible/src/openai_compatible.rs`, `crates/ai-sdk-open-responses/src/open_responses.rs` | `stream_object_calls_model_with_json_response_format_and_standardized_prompt`; `stream_object_collects_partial_objects_text_and_finish_metadata`; `stream_object_result_full_stream_matches_upstream_object_chunks`; `stream_object_result_full_stream_sends_finish_provider_metadata_and_timestamp`; `stream_object_array_output_formats_single_chunk_text_delta`; `stream_object_enum_output_unwraps_result`; `stream_object_enum_output_streams_value_and_sends_response_format`; `stream_object_enum_output_completes_unambiguous_prefixes`; `stream_object_enum_output_keeps_ambiguous_prefixes_until_resolved`; `stream_object_enum_output_suppresses_impossible_prefixes`; `stream_object_enum_output_handles_non_ambiguous_values`; `stream_object_no_schema_output_streams_partial_objects_without_response_schema`; `stream_object_repair_text_repairs_json_parse_error`; `stream_object_repair_text_repairs_type_validation_error`; `stream_object_repair_text_handles_repair_returning_none`; `stream_object_repair_text_repairs_json_wrapped_with_markdown_code_blocks`; `stream_object_repair_text_reports_no_object_when_parsing_still_fails`; `stream_object_invokes_lifecycle_callbacks_with_streamed_step`; `stream_object_step_finish_reports_ms_to_first_chunk`; `stream_object_callback_panics_do_not_break_stream`; `stream_object_accepts_experimental_telemetry_alias`; `stream_object_type_counterpart_*`; `stream_object_object_stream_invokes_on_error_callback_with_error`; `stream_object_invokes_error_callback_for_error_parts`; `stream_object_retries_retryable_pre_stream_errors`; `stream_object_messages_support_models_that_use_context_in_supported_urls`; `stream_object_aborts_before_model_call_and_suppresses_finish`; `stream_object_aborts_after_model_call_and_suppresses_finish`; `post_json_to_api_aborts_before_transport_call`; `post_json_to_api_aborts_pending_transport_when_signal_fires`; `gateway_model_passes_typed_gateway_provider_options_for_generate`; array/enum/error/warning/callback tests | Initial dependency-light collector over provider-v4 streams. It sends JSON response formats, accumulates text deltas, emits partial objects/text/full-stream parts with upstream object/text/finish ordering plus finish provider metadata and timestamps, unwraps array/enum/no-schema outputs, maps array output text deltas to upstream array syntax, streams complete array elements, applies enum prefix partial-output rules and enum response-format schema construction, exposes typed Rust accessors for final object, partial-object stream entries, and array elements, exposes text response helpers, parses final objects, applies upstream-style repair callbacks to final parse and validation failures including markdown code-fence cleanup and failed repaired text, invokes start/step-start/step-finish/finish lifecycle callbacks with a shared call id and `msToFirstChunk`, ignores synchronous callback panics so callback failures do not break the stream, runs `onError` callbacks for provider error stream parts including the Rust counterpart for upstream `doStream` rejection, carries provider warnings on the result and lifecycle callbacks, aliases deprecated `experimental_telemetry` without exposing telemetry option fields on start callback events, retries retryable pre-stream provider failures up to `maxRetries`, exposes max-retry configuration in start events, and now supports a Rust-native abort controller/signal that propagates to provider call options and first-phase provider HTTP requests, emits an abort-shaped error before provider calls or between returned stream parts, and suppresses step-finish/finish callbacks and end telemetry. The documented JavaScript-only Node.js response-like writer case remains intentionally non-portable. |
- The upstream `should write text deltas to a Node.js response-like object` `streamObject` case is intentionally documented as JavaScript-only and not ported to Rust.
| Embeddings: `embed`, `embedMany` | verified | `src/embed.rs` | `embed_calls_model_with_single_value_and_maps_result`; `embed_accepts_experimental_telemetry_alias`; `embed_many_accepts_experimental_telemetry_alias`; `embed_forwards_abort_signal_to_model_call`; `embed_many_without_model_limit_uses_one_model_call`; `embed_many_splits_limited_models_and_aggregates_results`; `embed_many_forwards_abort_signal_to_each_model_call`; `embed_lifecycle_events_serialize_upstream_shapes`; `embedding_model_call_events_round_trip_upstream_shapes`; `embed_invokes_start_and_end_callbacks`; `embed_many_invokes_start_and_end_callbacks_with_array_payloads`; `embed_result_serializes_upstream_shape_with_metadata_and_response`; `embed_result_deserializes_minimal_upstream_shape_and_omits_options`; `embed_many_result_serializes_upstream_shape_with_optional_responses`; `embed_many_result_deserializes_minimal_upstream_shape_and_omits_options` | Deprecated `experimental_telemetry` alias behavior now has one-to-one Rust counterparts for both embed APIs. Provider implementations remain unported. |
| Image generation: `generateImage` and `experimental_generateImage` | verified | `src/generate_image.rs` | `generate_image_*` tests, including `generate_image_sends_normalized_prompt_and_ai_user_agent_to_model`, `generate_image_forwards_abort_signal_to_model_call`, `generate_image_chunks_calls_and_aggregates_results`, `generate_image_uses_explicit_max_images_per_call_without_querying_model_limit`, `generate_image_returns_no_image_error_with_responses`, `generate_image_with_zero_images_makes_no_model_call_and_errors`, `prompt_image_string_parses_url_data_url_and_base64_inputs`, `experimental_generate_image_alias_uses_generate_image_runtime`, `prompt_image_data_url_serde_uses_camel_case_field_name`, and `headers_builder_accepts_existing_headers` | Provider implementations remain unported. |
| Speech generation: `generateSpeech` and experimental alias | verified | `src/generate_speech.rs` | Speech generation tests, including `generated_audio_file_serializes_upstream_shape_and_infers_format`, `generated_audio_file_deserializes_from_base64_media_type_shape`, `speech_result_serializes_upstream_shape`, `speech_result_deserializes_minimal_response_and_empty_metadata`, `generate_speech_forwards_options_headers_and_provider_options`, `generate_speech_forwards_abort_signal_to_model_call`, `generate_speech_uses_default_audio_media_type_when_detection_fails`, `generate_speech_errors_when_provider_returns_empty_audio`, and `experimental_generate_speech_aliases_generate_speech` | Provider implementations remain unported. |
| Video generation: `generateVideo` and experimental alias | verified | `src/generate_video.rs` | Video generation tests, including `generate_video_forwards_abort_signal_to_model_call`, `generate_video_forwards_abort_signal_to_download_callback`, `generate_video_forwards_normalized_prompt_options_headers_and_provider_options`, `generate_video_chunks_calls_and_aggregates_metadata`, `generate_video_downloads_url_output_and_detects_octet_stream_media_type`, `generate_video_url_output_requires_download_callback`, `generate_video_returns_no_video_error_with_response_metadata`, `generate_video_with_zero_videos_makes_no_model_call_and_errors`, `prompt_image_serializes_upstream_shape`, and `experimental_generate_video_alias_preserves_runtime_behavior` | Provider implementations remain unported. |
| Transcription: `transcribe` and experimental alias | verified | `src/transcribe.rs` | `transcribe_*` tests, including `transcribe_forwards_abort_signal_to_model_call`, `transcribe_forwards_abort_signal_to_download_callback`, `transcription_result_serializes_upstream_shape`, `transcription_result_deserializes_minimal_response_and_empty_metadata`, `transcription_result_drops_provider_response_body_from_metadata`, `transcription_result_errors_when_text_is_empty`, `experimental_transcription_result_alias_preserves_shape`, `transcribe_audio_serializes_upstream_file_data_variants`, `transcribe_forwards_normalized_audio_options_headers_and_provider_options`, `transcribe_downloads_url_audio_before_model_call`, `transcribe_url_audio_requires_download_callback`, `transcribe_propagates_url_download_errors`, `transcribe_uses_audio_wav_fallback_and_empty_provider_options`, `transcribe_defaults_missing_provider_metadata_to_empty_object`, `transcribe_errors_when_model_returns_no_text`, and `experimental_transcribe_alias_preserves_runtime_behavior` | Provider implementations remain unported. |
| Reranking: `rerank` | verified | `src/rerank.rs` | `rerank_*` tests, including `rerank_calls_model_with_text_documents_and_maps_result`, `rerank_forwards_abort_signal_to_model_call`, `rerank_calls_model_with_object_documents`, `rerank_skips_model_call_for_empty_documents_and_fires_callbacks`, `rerank_invokes_start_and_end_callbacks_with_upstream_events`, `rerank_contracts_serialize_upstream_shapes`, `rerank_result_deserializes_minimal_upstream_shape`, `rerank_events_round_trip_upstream_shapes`, `reranking_model_call_events_round_trip_upstream_shapes`, and `reranking_trait_identity_remains_v4`; `rerank_accepts_experimental_telemetry_alias` | Deprecated `experimental_telemetry` alias behavior now has a named Rust counterpart. Provider implementations remain unported. |
| File upload: `uploadFile` | verified | `src/upload_file.rs` | `upload_file_*` tests, including `upload_file_options_accepts_raw_data_shorthand_json`, `upload_file_data_preserves_tagged_text_shape`, `upload_file_options_resolves_media_types_like_upstream_upload_file`, and `upload_file_forwards_normalized_provider_call_options`; `upload_file_passes_tagged_base64_string_data_through_to_files_upload_file`; `upload_file_forwards_provider_options_to_files_upload_file`; `upload_file_passes_undefined_provider_options_when_not_provided`; `upload_file_passes_warnings_from_provider_result`; `upload_file_returns_result_without_provider_metadata_when_not_provided`; `upload_file_returns_provider_metadata_when_provided`; `upload_file_resolves_files_v4_from_provider_v4_with_files_method` | Mirrors upstream direct `FilesV4` upload behavior, tagged data/base64/text defaults, provider-options forwarding and absence, filename/result warning/providerMetadata pass-through, plus typed provider-v4 `files()` resolution through `upload_file_with_provider`. The upstream missing `files()` runtime throw is JavaScript dynamic object behavior; Rust callers use the `ProviderWithFiles` bound. Provider implementations remain unported. |
| Skill upload: `uploadSkill` | verified | `src/upload_skill.rs` | `upload_skill_*` tests, including `upload_skill_options_accepts_raw_file_data_shorthand_json`, `upload_skill_file_data_preserves_tagged_text_shape`, `upload_skill_options_normalizes_file_shorthand_to_provider_shape`, and `upload_skill_forwards_normalized_provider_call_options`; `upload_skill_returns_provider_reference_warnings_and_provider_metadata_from_skills`; `upload_skill_passes_provider_options_to_the_skills`; `upload_skill_resolves_skills_v4_from_provider_v4_with_skills_method` | Mirrors upstream direct `SkillsV4` upload behavior, providerReference/warnings/providerMetadata pass-through, provider-options forwarding, displayTitle handling, plus typed provider-v4 `skills()` resolution through `upload_skill_with_provider`. The upstream missing `skills()` runtime throw is JavaScript dynamic object behavior; Rust callers use the `ProviderWithSkills` bound. Provider implementations remain unported. |
| Provider registry | verified | `src/registry.rs` | `create_provider_registry_*` tests; `experimental_create_provider_registry_is_a_deprecated_alias`; `experimental_create_provider_registry_with_options_is_a_deprecated_alias`; `create_provider_registry_should_wrap_all_language_models_accessed_through_the_provider_registry`; `create_provider_registry_should_wrap_all_image_models_accessed_through_the_provider_registry`; `custom_provider_language_model_should_return_the_language_model_if_it_exists`; `custom_provider_language_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_language_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_embedding_model_should_return_the_embedding_model_if_it_exists`; `custom_provider_embedding_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_embedding_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_image_model_should_return_the_image_model_if_it_exists`; `custom_provider_image_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_image_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_transcription_model_should_return_the_transcription_model_if_it_exists`; `custom_provider_transcription_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_transcription_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_speech_model_should_return_the_speech_model_if_it_exists`; `custom_provider_speech_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_speech_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_reranking_model_should_return_the_reranking_model_if_it_exists`; `custom_provider_reranking_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_reranking_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_video_model_should_return_the_video_model_if_it_exists`; `custom_provider_video_model_should_throw_no_such_model_error_if_model_not_found_and_no_fallback`; `custom_provider_video_model_should_use_fallback_provider_if_model_not_found_and_fallback_exists`; `custom_provider_files_should_return_the_files_interface_if_it_exists`; `custom_provider_skills_should_return_the_skills_interface_if_it_exists`; `custom_provider_files_and_skills_should_expose_both_interfaces_when_both_exist`; `custom_provider_files_should_use_fallback_provider_files_if_files_is_not_configured_and_fallback_exists`; `custom_provider_skills_should_use_fallback_provider_skills_if_skills_is_not_configured_and_fallback_exists`; `custom_provider_should_resolve_string_model_ids_through_the_explicit_default_provider`; `no_such_provider_error_matches_upstream_default_message`; `no_such_provider_error_retains_custom_message_context` | Core provider/model lookup, custom separators, deprecated `experimental_createProviderRegistry` alias coverage, missing-provider/model errors, files/skills interfaces, language/image model middleware wrapping through registry lookup, direct `customProvider` current-v4 language/embedding/image model maps plus fallback-provider lookup for those required model families, direct optional transcription/speech/reranking/video model maps plus no-fallback and fallback-provider behavior, direct `customProvider` files/skills exposure, fallback-provider files/skills exposure, string model-id alias resolution through an explicit Rust default/fallback provider, and `NoSuchProviderError` message/context parity are covered. Gateway-specific model resolution is covered by the dedicated `resolve_model` and Gateway rows. Rust uses an explicit provider boundary instead of JavaScript's mutable `globalThis.AI_SDK_DEFAULT_PROVIDER`. |
...
| Gateway drifted fetch-metadata/provider strict case map (AIS-03 r2) | verified | `crates/ai-sdk-gateway/src/gateway.rs`, `crates/ai-sdk-gateway/src/gateway_error.rs`, `scripts/ai-strict-test-inventory.mjs` AIS-03 exact table | `gateway_fetch_metadata_keeps_known_models_and_filters_unknown_from_mixed_response`; `gateway_fetch_metadata_does_not_double_wrap_existing_gateway_errors`; `gateway_fetch_metadata_handles_rate_limit_server_errors`; `gateway_fetch_metadata_fetches_credits_from_correct_endpoint`; `gateway_fetch_metadata_does_not_double_wrap_existing_credit_gateway_errors`; `create_gateway_language_model_uses_oidc_when_api_key_is_absent`; `create_gateway_embedding_model_returns_gateway_embedding_model`; `create_gateway_image_model_uses_custom_base_url`; `create_gateway_image_model_reuses_headers_transport_and_observability`; `create_gateway_video_model_uses_custom_base_url`; `create_gateway_video_model_reuses_headers_transport_and_observability`; `create_gateway_reranking_model_uses_custom_base_url`; `create_gateway_reranking_alias_returns_gateway_reranking_model`; `gateway_provider_fetches_available_models_metadata`; `gateway_provider_caches_available_models_until_refresh`; `gateway_provider_uses_default_metadata_cache_refresh_interval`; `create_gateway_language_model_passes_observability_headers_from_environment`; `create_gateway_language_model_omits_missing_observability_headers`; `default_gateway_export_exposes_provider_instance`; `create_gateway_uses_default_base_url_when_none_is_provided`; `create_gateway_accepts_empty_options`; `default_gateway_export_constructs_image_model`; `default_gateway_export_constructs_video_model`; `create_gateway_overrides_default_base_url_when_provided`; `create_gateway_prefers_api_key_over_oidc_token`; `get_gateway_auth_token_treats_empty_environment_variables_as_missing`; `get_gateway_auth_token_prioritizes_options_api_key_over_all_environment_variables`; `get_gateway_auth_token_prefers_options_api_key_over_ai_gateway_api_key`; `get_gateway_auth_token_prefers_ai_gateway_api_key_over_oidc_token`; `get_gateway_auth_token_falls_back_to_oidc_when_no_api_keys_are_available`; `gateway_provider_real_world_vercel_deployment_uses_oidc_authentication`; `gateway_provider_real_world_local_development_uses_api_key_authentication`; `gateway_provider_real_world_explicit_api_key_override_wins_over_environment`; `gateway_provider_get_credits_fetches_successfully`; `gateway_provider_get_credits_uses_custom_base_url`; `gateway_provider_get_credits_uses_oidc_authentication_headers`; `gateway_provider_get_credits_surfaces_endpoint_errors`; `gateway_provider_get_credits_includes_upstream_headers`; `gateway_provider_get_credits_is_available_on_provider_interface`; `gateway_provider_get_spend_report_fetches_successfully`; `gateway_provider_get_spend_report_passes_params_through`; `gateway_provider_get_spend_report_uses_custom_base_url`; `gateway_provider_get_spend_report_uses_custom_transport`; `gateway_provider_get_spend_report_surfaces_endpoint_errors`; `gateway_provider_get_spend_report_is_available_on_provider_interface`; `default_gateway_export_get_spend_report_is_available`; `gateway_provider_fetches_generation_info_and_unwraps_data`; `gateway_provider_metadata_fetch_errors_convert_to_gateway_errors`; `gateway_provider_metadata_gateway_errors_are_not_double_wrapped`; `gateway_provider_language_model_handles_model_specification_errors`; `gateway_provider_language_model_accepts_any_model_id`; `gateway_provider_language_model_accepts_non_existent_model_id` | Closes 52 strict cases (`packages-gateway-0126/0133/0134/0139/0148` plus the `gateway-provider.test.ts` 0257/0264-0270/0277-0286/0289-0290/0295/0297/0299-0315/0317-0319/0321-0325 set) whose upstream `file:line` keys had drifted away from the prior AIS-03 exact table. Each case maps one-to-one to an existing real Rust test in `crates/ai-sdk-gateway/src/gateway.rs`; no stub tests were added. Genuinely non-portable remainders left unmapped: the Gateway speech and transcription model suites (`gateway-speech-model.test.ts` 0337-0344, `gateway-transcription-model.test.ts` 0360-0368) and the provider speech/transcription model-creation cases (0271-0276, 0287-0288) target models the Rust crate does not yet implement; the `teamIdOrSlug` request/header cases (0258-0262, 0291-0292) target a provider setting absent from the Rust crate; and the `new`-keyword constructor case (0263) is a JavaScript callable-constructor identity check with no Rust equivalent. |
| OpenAI-compatible chat provider foundation | in-progress | `crates/ai-sdk-openai-compatible`; root facade shim in `src/openai_compatible.rs`; Vercel AI Gateway integration in `src/vercel_ai_gateway.rs` | `openai_compatible_provider_configures_headers_urls_and_model_aliases`; `openai_compatible_provider_lists_models`; `openai_compatible_chat_generates_text_through_generate_text`; `openai_compatible_chat_streams_text_through_stream_text`; `openai_compatible_chat_streams_reasoning_raw_chunks_and_parse_errors`; `openai_compatible_chat_passes_tools_tool_choice_and_provider_options`; `openai_compatible_chat_converts_multimodal_user_messages`; `openai_compatible_chat_rejects_unsupported_file_messages_before_transport`; `openai_compatible_chat_converts_assistant_tool_history`; `openai_compatible_chat_runs_generate_text_tool_loop_end_to_end`; `openai_compatible_chat_runs_stream_text_tool_loop_end_to_end`; `openai_compatible_chat_maps_tool_calls_from_generate`; `openai_compatible_chat_streams_tool_calls`; `openai_compatible_chat_maps_response_formats_and_warnings`; `openai_compatible_chat_injects_json_instruction_when_response_format_body_is_disabled`; `vercel_ai_gateway_openai_compatible_generates_text_through_openai_chat`; `vercel_ai_gateway_openai_compatible_streams_text_through_openai_chat`; ignored `live_vercel_ai_gateway_openai_compatible_generate_text`; ignored `live_vercel_ai_gateway_openai_compatible_stream_text` | Mirrors the first `createOpenAICompatible` provider construction boundary plus `/models` discovery and non-streaming and streaming chat `/chat/completions` calls with request shaping, chat provider options, multimodal user messages, assistant reasoning/tool-call history, tool-result messages, provider metadata, Google thought signatures, high-level `generate_text` and `stream_text` tool-loop continuation, function tools/tool choice, response metadata, finish reason, usage, warnings, raw chunks, reasoning, parse errors, non-streaming tool calls, streamed tool calls, and JSON instruction injection when a provider rejects `response_format`. Vercel AI Gateway's OpenAI-compatible text and streaming routes are covered for `openai/...` models. The detailed OpenAI-compatible rows below enumerate the current 8 upstream test files, including 232 `it(...)`/`test(...)` declarations with named Rust tests and explicit JavaScript/runtime exceptions, but P6 strict inventory still tracks the package rows as unmapped; this foundation row remains in progress until those generated strict mappings or explicit exceptions land. |
...
| Vercel v0 provider package | in-progress | `src/vercel.rs`, `src/openai_compatible.rs` | `vercel_provider_creates_openai_compatible_chat_model`; `vercel_provider_uses_default_base_url_and_function_alias`; `vercel_provider_reports_unsupported_model_families`; `vercel_provider_implements_provider_trait` | The four Rust tests mirror all 4 current upstream `packages/vercel/src/vercel-provider.test.ts` cases for `createVercel`: default/custom options, `VERCEL_API_KEY`, custom headers, callable-style model creation, `languageModel`, provider id `vercel.chat`, Vercel user-agent suffix, and unsupported embedding/image lookups. P6 strict inventory still lists the 4 Vercel rows as portable-unmapped, so this row remains in-progress until the generated strict case map records those named tests or explicit exceptions. |
| Concrete provider packages | in-progress | `src/openai.rs`, `src/open_responses.rs`, `src/vercel.rs`, `src/vercel_ai_gateway.rs`, `src/deepinfra.rs`, `src/togetherai.rs`, `src/huggingface.rs`, `src/cerebras.rs`, `src/baseten.rs`, `src/voyage.rs`, package-owned provider crates under `crates/ai-sdk-*` | Package rows above plus `docs/ai-foundational-provider-inventory.md`, `docs/ai-02-openai-compatible-providers.md`, and `docs/ai-strict-test-inventory.md`; strict inventory now verifies Groq 95/95 after `groq_chat_streaming_maps_unparsable_stream_parts`, while AI-01 and AI-02 exact-map docs continue to cover their named sub-buckets. | P8 scope reconciliation found the full provider surface is larger than one child thread under the new P6 strict gate. This slice closes the highest-risk AI-02 Groq drift and keeps broader concrete provider closure in-progress: current strict unmapped provider/support sub-buckets include provider-utils 426, provider 16, gateway 367, OpenAI-compatible 232, Open Responses 69, Hugging Face 37, Vercel 4, and the remaining wrapper/media provider rows listed in `docs/package-progress.md`. |
...
| `examples/ai-e2e-next` | js-only-documented | none | `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | The upstream app shell is Next.js, React/RSC, browser UI, Vercel Blob, and live-provider demo wiring. Portable provider, agent, MCP, and stream behavior is tracked by package ledger rows and the framework-specific app shell is intentionally not ported literally. |
| `examples/ai-functions` | verified | `examples/kitchen_sink.rs`, `examples/vercel_ai_gateway_text.rs`, `examples/vercel_ai_gateway_responses.rs`, `examples/vercel_ai_gateway_models.rs`, `examples/vercel_ai_gateway_image.rs`, package/provider test rows | `cargo run --example kitchen_sink`; `cargo run --example http_ui_message_server -- --self-test`; package/provider test evidence above | Upstream's AI function script and manual e2e suite is represented by root Rust examples for deterministic tool-loop behavior, Gateway live entry points, and package/provider rows for generate/stream/embed/object/image/speech/transcribe/rerank/upload-skill/upload-file behavior. Credential-gated provider e2e smoke remains with the provider package rows rather than a duplicate example row. |
| `examples/angular` | js-only-documented | none | This row | Angular adapter example is JavaScript framework-specific. |
| `examples/express` | verified | `examples/http_ui_message_server.rs`, `src/ui_message_stream.rs`, `src/stream_text.rs`, `src/agent.rs` | `cargo run --example http_ui_message_server -- --self-test`; `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | Portable server behavior from the Express example is covered by a framework-neutral Rust HTTP example that serves `streamText` as UI-message SSE, custom data parts merged with a model stream, and pipe-to-response helpers. Express middleware and Node `ServerResponse` bindings are JavaScript framework details. |
| `examples/fastify` | verified | `examples/http_ui_message_server.rs`, `src/ui_message_stream.rs`, `src/stream_text.rs` | `cargo run --example http_ui_message_server -- --self-test`; `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | Portable Fastify behavior is the same response-object path: create a UI-message stream response, create custom data parts, and return a text-stream response. The Fastify plugin/reply shell remains JavaScript-specific. |
| `examples/hono` | verified | `examples/http_ui_message_server.rs`, `src/ui_message_stream.rs`, `src/stream_text.rs`, `src/agent.rs` | `cargo run --example http_ui_message_server -- --self-test`; `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | Portable Hono behavior is covered by the Rust server example's UI-message SSE, text-stream response, custom data part, and pipe-to-response routes. Hono CORS/fetch adapter wiring and browser client UI are JavaScript framework details. |
| `examples/mcp` | verified | `crates/ai-sdk-mcp/examples/local_mcp_client.rs`, `crates/ai-sdk-mcp/examples/http_auth_typed_tools.rs`, `crates/ai-sdk-mcp/examples/stdio_typed_tools.rs`, `crates/ai-sdk-mcp/examples/sse_typed_tools.rs`, `crates/ai-sdk-mcp/examples/tool_meta.rs`, `crates/ai-sdk-mcp/examples/image_content.rs`, `crates/ai-sdk-mcp/examples/server_metadata.rs`, `crates/ai-sdk-mcp/examples/elicitation_multi_step.rs`, `crates/ai-sdk-mcp/examples/hosted_oauth_http.rs` | `cargo run -p ai-sdk-mcp --example local_mcp_client`; `cargo run -p ai-sdk-mcp --example http_auth_typed_tools`; `cargo run -p ai-sdk-mcp --example stdio_typed_tools`; `cargo run -p ai-sdk-mcp --example sse_typed_tools`; `cargo run -p ai-sdk-mcp --example tool_meta`; `cargo run -p ai-sdk-mcp --example image_content`; `cargo run -p ai-sdk-mcp --example server_metadata`; `cargo run -p ai-sdk-mcp --example elicitation_multi_step`; `cargo run -p ai-sdk-mcp --example hosted_oauth_http` | Rust package-owned MCP examples mirror the portable upstream MCP flows for tool definitions, resources, prompts, elicitation, HTTP bearer auth, SSE, stdio, typed output-schema validation, raw MCP tool results, initialized server metadata/instructions, provider metadata, OpenAI Apps `openai/outputTemplate` metadata, image content conversion, session cleanup, and local hosted OAuth. Upstream `elicitation-ui` is UI shell behavior, and hosted Shopify/protected remote services remain credential-gated live guidance rather than portable Rust example obligations. |
...
| `examples/next-fastapi` | verified | `examples/http_ui_message_server.rs`, `src/ui_message_stream.rs`, `src/stream_text.rs` | `cargo run --example http_ui_message_server -- --self-test`; `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | The mixed Next/FastAPI app's portable behavior is the AI SDK UI-message/text stream protocol and custom data stream shape; the Rust server example covers those response patterns. The Next UI and Python FastAPI/OpenAI SDK shell are non-Rust framework details. |
...
| `examples/node-http-server` | verified | `examples/http_ui_message_server.rs`, `src/ui_message_stream.rs`, `src/stream_text.rs` | `cargo run --example http_ui_message_server -- --self-test`; `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | Portable Node HTTP server behavior is covered by the framework-neutral Rust HTTP example: direct HTTP routing, UI-message SSE responses, custom data parts, text-stream responses, and response-writer piping. Node's `createServer` and `ServerResponse` are JavaScript runtime details. |
...
| --- | ---: | --- | --- |
| `packages/ai` | 128 | verified | Many non-streaming high-level API tests are represented in Rust, including non-language high-level abort-signal forwarding for embedding, image, speech, video, transcription, reranking calls, current-provider model resolution, named restricted telemetry dispatcher context-filtering counterparts, and named token-rate/token-count/header/deep-equal/merge-object/split-array/start-index/cosine/abort/callback/serial-job/prepare-retries/request-timeout/language-model-call-options/standardize-prompt/file-part-data/prepare-tools/tool-choice/tool-model-output/filter-active-tools/collect-tool-approvals/validate-tool-context/prune-messages/stop-condition/simulate-readable-stream/server-response/async-iterable-stream/stitchable-stream/download utility counterparts; initial ToolLoopAgent wrapper, model/request option forwarding, instruction-shape forwarding, generateText include request/body/request-message/response-body retention, deprecated `experimental_onToolCallStart`/`experimental_onToolCallFinish` callback compatibility for generateText and streamText, prepare-call sandbox/stream provider-option shaping, sandbox propagation into local tool execution, createAgentUIStreamResponse sandbox propagation into local tool execution, user-approval blocking, provider-executed approval-response continuation and tool-result surfacing for generateText, onStart callback/event forwarding, callback-merging, per-call abort/timeout request-control coverage including total-timeout forwarding and merge into model-call abort signals plus the no-timeout/no-total-timeout cases, direct generateText and streamText abort-signal forwarding to tool execution, streamText abort-during-tool-call no-onError coverage, generateText/streamText tool-execution context passing, generateText prepareStep runtime-context passing, generateText prepareStep sandbox passing, generateText onFinish runtime-context passing, generateText full runtime-context callback coverage, upstream-compatible `AgentCallParameters`/`AgentStreamParameters` aliases, per-call stream-transform forwarding, interleaved reasoning/text full-stream ordering, multi-step warning aggregation for `result.warnings`/`onFinish`, streamText `responseMessages` accumulation/transform, streamText prepareStep accumulation across multiple `doStream` calls, generateText/streamText `onStepStart` telemetry metadata suppression, generateText/streamText runtime-context inclusion/exclusion and toolsContext inclusion, generateText/streamText reasoning high/provider-default pass-through, streamText response-format request-body forwarding, streamText stop-condition, repair-callback, and onError forwarding to the workflow iterator, streamText type-counterpart compile-time inference assertions (`stream_text_type_counterpart_should_infer_text_output_type_default`, `stream_text_type_counterpart_should_infer_text_output_type`, `stream_text_type_counterpart_should_infer_object_output_type`, `stream_text_type_counterpart_should_infer_array_output_type`, `stream_text_type_counterpart_should_infer_choice_output_type`, `stream_text_type_counterpart_should_infer_json_output_type`, `stream_text_type_counterpart_should_infer_text_partial_output_type_default`, `stream_text_type_counterpart_should_infer_text_partial_output_type`, `stream_text_type_counterpart_should_infer_object_partial_output_type`, `stream_text_type_counterpart_should_infer_array_partial_output_type`, `stream_text_type_counterpart_should_infer_choice_partial_output_type`, `stream_text_type_counterpart_should_infer_json_partial_output_type`, `stream_text_type_counterpart_should_infer_element_type_for_array_output`, `stream_text_type_counterpart_should_infer_empty_element_stream_for_text_output`, `stream_text_type_counterpart_should_infer_empty_element_stream_for_object_output`, `stream_text_type_counterpart_should_infer_empty_element_stream_for_default_output`), `streamText` error/abort/reasoning/custom fallback cases (`stream_text_retains_error_parts_and_marks_error_finish_without_finish_part`, `stream_text_forwards_abort_signal_to_tool_execution`, `stream_text_maps_reasoning_sources_and_custom_parts`, `stream_text_uses_experimental_on_tool_call_start_as_a_fallback`, `stream_text_prefers_on_tool_execution_start_over_experimental_on_tool_call_start`, `stream_text_uses_experimental_on_tool_call_finish_as_a_fallback`, `stream_text_prefers_on_tool_execution_end_over_experimental_on_tool_call_finish`, `stream_text_passes_tools_context_to_tool_execution`), `streamObject` array-output and error-part coverage (`stream_object_array_output_unwraps_elements`, `stream_object_retains_error_parts`), the portable `generate-text/output.test.ts` helper surface now mapped through `generate_text_output` (`text_output_returns_text_as_is_and_handles_missing_input`, `object_output_returns_response_format_and_parses_complete_and_partial_output`, `array_output_returns_response_format_and_partial_elements`, `json_output_handles_complete_and_partial_json`), and the `process_ui_message_stream` callback surface now covering transient data, client-executed tool-call, dynamic tool-call, and error-callback behavior now exist, and the deprecated JavaScript `generateText.experimental_include` alias is documented as JavaScript option-surface compatibility because Rust exposes only the typed `with_include` builder, while the stream-side `includeRawChunks`/`experimental_include` aliases are documented similarly. The legacy v2/v3 model/provider adapter inventory and browser/runtime-only stream/UI edges are documented as JavaScript package compatibility exceptions. The remaining compile-time-only `ToolLoopAgent` key-exactness assertions over `includeRuntimeContext`/`includeToolsContext` are documented as JavaScript-only because Rust exposes those contexts as runtime JSON/BTreeMap maps rather than TypeScript structural-key inference. |
| Provider package tests | 288 | in-progress | Current provider/support-package audit over the refreshed upstream tree found 288 provider-family `*.test.*`/`*.spec.*` files and 4,953 `it(...)`/`test(...)` declarations across provider packages and support packages. P6 strict inventory scans 8,762 upstream cases overall and still reports 5,383 portable unmapped rows, including 2,167 displayed-row provider/support portable cases in `docs/package-progress.md`. This slice maps `packages-groq-0067` to `groq_chat_streaming_maps_unparsable_stream_parts`, moving Groq to 95/95 strict mapped cases; the broader provider package test row remains in-progress until each portable upstream case has a generated strict mapping or explicit exception. |
| `packages/provider-utils` | 77 | verified | Many provider support behaviors are represented in the matching `ai-sdk-provider-utils` crate, including one-to-one `filterNullable`, `removeUndefinedEntries`, complete portable `validateTypes`/`safeValidateTypes`, complete portable `secureJsonParse`, complete portable `parseJSON`/`safeParseJSON`/`isParsableJson`, complete portable `injectJsonInstruction`, exact `mediaTypeToExtension` table rows, complete portable `normalizeHeaders` cases, complete portable `mapReasoningToProvider*` cases, complete portable `resolve` cases, complete portable `createToolNameMapping` cases, complete portable `withUserAgentSuffix`, `getRuntimeEnvironmentUserAgent`, `isUrlSupported`, `validateDownloadUrl`, `downloadBlob`/`DownloadError`, `getFromApi`, `delay`, `executeTool`, `isExecutableTool`, portable `asSchema`/`StandardSchema`, `readResponseWithSizeLimit`, `responseHandler`, `handleFetchError`, `convertAsyncIteratorToReadableStream`, `isJSONSerializable`, `StreamingToolCallTracker`, `serializeModelOptions`, and provider-utils `content-part` type-contract cases, complete portable `createIdGenerator`/`generateId` cases, complete portable `DelayedPromise` cases, portable `isProviderReference` cases, and complete portable `resolveProviderReference` cases plus abort propagation for GET, JSON, form-data, and generic POST request helpers, with exact browser stream/fetch parity and Zod adapter snapshots documented as JavaScript-specific rather than hidden Rust parity debt. |
| `packages/provider` | 1 | verified | Upstream `get-error-message.test.ts` now has a one-to-one Rust test split for every portable original case, with exact named counterparts `get_error_message_returns_unknown_error_for_null`, `get_error_message_returns_unknown_error_for_undefined` (folded into the same Rust `Option::None` path as null), `get_error_message_returns_string_as_is`, `get_error_message_returns_empty_string_as_is`, `get_error_message_includes_error_type_prefix_for_basic_error`, `get_error_message_includes_type_error_prefix`, `get_error_message_includes_range_error_prefix`, `get_error_message_returns_error_name_for_empty_message`, `get_error_message_returns_type_error_name_for_empty_message`, `get_error_message_handles_custom_error_subclasses`, `get_error_message_respects_custom_to_string_overrides`, `get_error_message_handles_custom_error_subclass_with_empty_message`, `get_error_message_json_stringifies_plain_objects`, `get_error_message_json_stringifies_numbers`, `get_error_message_json_stringifies_booleans`, and `get_error_message_json_stringifies_arrays`. The package row is closed; v2/v3 compatibility surfaces and exact stream abstractions are documented as JavaScript compatibility/runtime exceptions. |
...
| Repo docs, content, architecture, tools, skills, Changesets, package publishing | not counted | verified | `docs/cross-sdk-examples-docs-parity.md`, `scripts/cross-sdk-examples-docs-inventory.mjs` | `node scripts/cross-sdk-examples-docs-inventory.mjs --check` | Cross-SDK inventory now accounts for upstream AI SDK public docs, README files, examples, architecture notes, contributor docs, repo skills, tool READMEs, and Changesets/package-publishing README. Portable architecture/API behavior is handed to package ledger rows; contributor workflow, skill, tooling, release, package-publishing, and Changesets operations are documented as upstream-docs-only or JavaScript/repo-process surfaces. |

### Recent First-Phase Proof Slices

- 2026-06-02: AI examples/server-pattern closure added
  `examples/http_ui_message_server.rs`, a deterministic std-HTTP example with
  `--self-test` coverage for `streamText` UI-message SSE responses,
  pipe-to-response helpers, text-stream responses, and custom data parts. The
  examples inventory now closes/classifies `examples/ai-e2e-next`,
  `examples/ai-functions`, `examples/express`, `examples/fastify`,
  `examples/hono`, `examples/mcp`, `examples/next-fastapi`, and
  `examples/node-http-server`, and the cross-SDK docs/examples inventory now
  accounts for AI SDK architecture, contributor, skill, tool, and
  Changesets/package-publishing docs.
- 2026-05-25: `packages/ai` `agent` helper inventory refresh added named Rust counterparts for the remaining exact upstream cases `tool_loop_agent_exposes_version_id_and_tools`, `tool_loop_agent_generate_forwards_settings_and_instructions`, `tool_loop_agent_prepare_call_can_shape_provider_options`, `tool_loop_agent_merges_generate_start_callbacks_in_order`, `tool_loop_agent_uses_upstream_twenty_step_default_for_tool_loop`, `tool_loop_agent_merges_tool_execution_callbacks_in_order`, and `tool_loop_agent_stream_delegates_to_stream_text`.
- 2026-05-25: `packages/ai` `generateText` helper inventory refresh added named Rust counterparts for the remaining exact upstream cases `generate_text_include_serializes_optional_upstream_flags`, `generate_text_step_performance_serializes_as_upstream_camel_case_shape`, `generate_text_notifies_language_model_call_start_and_end`, `generate_text_calls_language_model_and_returns_plain_text_result`, `generate_text_forwards_abort_signal_to_tool_execution`, `generate_text_retries_retryable_pre_content_errors`, `generate_text_from_prompt_standardizes_text_and_instructions`, `generate_text_from_prompt_rejects_invalid_high_level_prompt`, `generate_text_invokes_start_callbacks_with_configuration`, `generate_text_invokes_tool_execution_callbacks_around_local_tools`, `generate_text_uses_experimental_on_tool_call_start_as_a_fallback`, `generate_text_prefers_on_tool_execution_start_over_experimental_on_tool_call_start`, `generate_text_uses_experimental_on_tool_call_finish_as_a_fallback`, `generate_text_prefers_on_tool_execution_end_over_experimental_on_tool_call_finish`, `generate_text_invokes_finish_callbacks_with_completed_records`, `generate_text_passes_runtime_context_to_finish_callback`, `generate_text_passes_full_runtime_context_to_callbacks`, `generate_text_include_controls_retained_provider_bodies`, `generate_text_result_serializes_as_camel_case_step_record`, `generate_text_tool_call_and_result_serialize_as_camel_case_contracts`, `generate_text_upstream_tool_aliases_and_denied_output_match_contracts`, `generate_text_content_parts_round_trip_upstream_high_level_shapes`, `generate_text_splits_static_and_dynamic_tool_calls_and_results`, `generate_text_reasoning_deserializes_generated_reasoning_shapes`, `generate_text_result_deserializes_minimal_contract`, `generate_text_surfaces_files_across_result_and_steps`, `generate_text_surfaces_final_step_reasoning_and_reasoning_text`, `generate_text_surfaces_sources_across_result_and_steps`, `generate_text_concatenates_only_final_step_text_parts`, `generate_text_options_can_wrap_prepared_call_options`, `generate_text_passes_high_level_rust_tools_to_language_model`, `generate_text_passes_provider_tools_to_language_model`, `generate_text_prepare_step_overrides_step_settings_and_carries_contexts`, `generate_text_passes_runtime_context_to_prepare_step`, `generate_text_passes_sandbox_to_prepare_step`, `generate_text_prepare_step_can_override_model_with_same_model_type`, `generate_text_filters_active_tools_before_calling_language_model`, `generate_text_includes_non_text_content_in_content_but_not_text`, `generate_text_allows_assistant_prompt_messages_for_continuations`, `generate_text_executes_tool_result_and_continues_to_final_text`, `generate_text_uses_tool_model_output_for_continuation_messages`, `generate_text_refines_tool_input_before_execution_results_and_continuation`, `generate_text_turns_failed_tool_input_refinement_into_invalid_tool_result`, `generate_text_repairs_invalid_tool_call_before_execution`, `generate_text_repairs_unknown_tool_name_before_execution`, `generate_text_turns_failed_tool_call_repair_into_invalid_tool_result`, `generate_text_auto_approves_tool_calls_and_executes_tools`, `generate_text_uses_per_tool_approval_callback_before_tool_execution`, `generate_text_auto_denies_tool_calls_without_executing_tools`, `generate_text_user_approval_blocks_tool_execution_until_response`, `generate_text_executes_initial_approved_tool_approval_before_first_model_call`, `generate_text_turns_initial_denied_provider_approval_into_execution_denied_result`, `generate_text_propagates_runtime_and_tools_context_to_steps_and_tool_execution`, `generate_text_passes_experimental_sandbox_to_prepare_step_and_tool_execution`, `generate_text_resolves_dynamic_tool_descriptions_with_step_context_and_sandbox`, `generate_text_validates_tool_context_schema_before_execution`, `generate_text_passes_tools_context_to_tool_execution`, `generate_text_returns_tool_error_when_tool_context_schema_fails`, `generate_text_marks_runtime_dynamic_tool_calls_and_results`, `generate_text_propagates_tool_metadata_to_tool_calls_and_results`, `generate_text_records_tool_execution_errors_and_continues`, `generate_text_preserves_tool_result_provider_metadata_in_continuation_message`, `generate_text_converts_provider_executed_tool_results_for_continuation_messages`, `generate_text_surfaces_provider_executed_tool_results_without_local_execution`, `generate_text_refines_provider_executed_tool_result_inputs`, `generate_text_continues_for_deferred_provider_executed_tool_results`, `generate_text_stops_after_max_steps_even_when_tool_calls_continue`, `generate_text_stops_after_matching_stop_condition`, `generate_text_reports_invalid_json_tool_input_and_continues`, `generate_text_reports_unknown_tool_and_continues_with_error_result`, and `generate_text_omits_empty_text_from_continuation_assistant_messages`.
- 2026-05-25: `packages/ai` `generate_text.rs` helper and error-shape inventory refresh added named Rust counterparts for `step_count_is_alias_matches_is_step_count`, `tool_input_refinement_error_retains_message`, `calculate_tokens_per_second_should_calculate_average_output_tokens_per_second`, `calculate_tokens_per_second_should_return_zero_when_output_token_count_is_unknown`, `calculate_tokens_per_second_should_return_zero_when_response_time_is_zero`, `calculate_tokens_per_second_should_return_zero_when_response_time_is_zero_and_output_tokens_are_unknown`, `calculate_tokens_per_second_should_return_zero_when_duration_is_unknown`, `sum_token_counts_should_sum_known_token_counts`, `sum_token_counts_should_treat_one_unknown_token_count_as_zero`, `sum_token_counts_should_return_unknown_when_both_token_counts_are_unknown`, `no_such_tool_error_matches_upstream_default_messages`, `invalid_tool_input_error_matches_upstream_default_message`, `tool_call_repair_error_matches_upstream_default_message`, `tool_call_repair_error_retains_original_no_such_tool_error_and_custom_message`, `missing_tool_results_error_matches_upstream_default_messages`, `invalid_tool_approval_error_matches_upstream_default_message`, `tool_call_not_found_for_approval_error_matches_upstream_default_message`, `tool_approval_status_accepts_upstream_string_forms_and_normalizes_them`, `tool_approval_status_object_forms_round_trip_upstream_json`, `tool_approval_status_kind_round_trips_status_strings`, `tool_approval_configuration_round_trips_static_status_map`, `resolve_tool_approval_prefers_user_configured_statuses`, `resolve_tool_approval_maps_tool_defined_boolean_approval`, `resolve_tool_approval_uses_tool_defined_callback_with_context`, `resolve_tool_approval_defaults_to_not_applicable`, `resolve_tool_approval_uses_generic_callback_before_static_tool_statuses`, `resolve_tool_approval_uses_per_tool_callback_with_context`, `collect_tool_approvals_reports_invalid_approval_references`, `no_output_generated_error_matches_upstream_default_message`, `no_output_generated_error_accepts_custom_message`, `no_object_generated_error_matches_upstream_default_context`, `no_object_generated_error_retains_text_cause_and_custom_message`, `no_object_generated_error_context_uses_existing_json_boundaries`, `invalid_stream_part_error_retains_chunk_and_message`, `ui_message_stream_error_retains_chunk_context_and_message`, `unsupported_model_version_error_matches_upstream_message_and_context`, `to_response_messages_should_return_an_assistant_message_with_text_when_no_tool_calls_or_results`, `to_response_messages_should_include_tool_calls_in_the_assistant_message`, `to_response_messages_should_include_tool_calls_with_metadata_in_the_assistant_message`, `to_response_messages_should_include_custom_parts_in_the_assistant_message`, `to_response_messages_should_include_tool_results_as_a_separate_message`, `to_response_messages_should_include_tool_errors_as_a_separate_message`, `to_response_messages_should_handle_undefined_text`, `to_response_messages_should_include_reasoning_array_with_redacted_reasoning_in_the_assistant_message`, `to_response_messages_should_handle_multipart_tool_results`, `to_response_messages_should_include_reasoning_file_parts_in_the_assistant_message`, `to_response_messages_should_include_images_in_the_assistant_message`, `to_response_messages_should_handle_multiple_images_in_the_assistant_message`, `to_response_messages_should_handle_uint8_array_images`, `to_response_messages_should_include_images_reasoning_and_tool_calls_in_the_correct_order`, `to_response_messages_should_not_append_text_parts_if_text_is_empty_string`, `to_response_messages_should_not_append_assistant_message_if_there_is_no_content`, `to_response_messages_should_include_provider_executed_tool_calls_and_results`, `to_response_messages_should_include_tool_approval_request_in_the_assistant_message`, `to_response_messages_should_include_tool_approval_request_for_provider_executed_tools`, `to_response_messages_should_preserve_automatic_approval_request_stage_in_the_assistant_message`, `to_response_messages_should_include_approval_response_and_tool_result_stages_in_the_tool_message`, `to_response_messages_should_add_an_execution_denied_tool_result_when_tool_approval_is_denied`, `to_response_messages_should_include_provider_executed_approval_response_stages_in_the_tool_message`, `to_response_messages_should_keep_provider_executed_tool_result_stages_in_the_assistant_message_only`, `to_response_messages_should_sanitize_invalid_tool_call_with_non_object_input_to_empty_object`, `to_response_messages_should_preserve_valid_object_input_on_invalid_tool_call`, `to_response_messages_should_include_provider_metadata_in_the_text_parts`, `language_model_call_events_round_trip_upstream_shape`, `tool_execution_events_round_trip_json`, `reasoning_outputs_round_trip_upstream_high_level_shapes`, `generated_file_exposes_base64_and_bytes_views`, `generated_file_serializes_upstream_shape`, `generated_file_aliases_share_the_same_contract`, `generated_file_converts_from_provider_v4_file_parts`, `experimental_filter_active_tools_alias_matches_filter_active_tools`, and `prune_messages_options_round_trip_upstream_json`.
- 2026-05-25: `packages/ai` `generate_text.rs` helper follow-up added named Rust counterparts for `generate_text_call_id`, `generate_text_tool_result_from_language_model_tool_result`, `generate_text_tool_result_content_part`, `generate_text_tool_call_from_prompt_part`, and `generate_text_result_with_output`.
- 2026-05-25: `packages/ai` `streamText` high-level stream-part shape parity added the upstream `text_stream_parts_use_upstream_high_level_shapes` coverage as a named Rust counterpart.
- 2026-05-25: `packages/ai` `generateText` step-specific sandbox parity
  added the upstream `should use sandbox returned from prepareStep for that
  step only` coverage as
  `generate_text_uses_sandbox_returned_from_prepare_step_for_that_step_only`.
  Rust now proves a sandbox returned from `prepareStep` applies only to that
  step's tool execution and falls back to the default sandbox on later steps.
...
- 2026-05-25: `packages/ai` `uiMessageStream` exact-name follow-up added named
  Rust counterparts for `transform_text_to_ui_message_stream_emits_upstream_sequence`,
  `transform_text_to_ui_message_stream_handles_empty_streams`,
  `process_ui_message_stream_merges_message_metadata`,
  `process_ui_message_stream_calls_on_data_with_transient_part`,
  `process_ui_message_stream_calls_on_tool_call_for_client_executed_tools`,
  `process_ui_message_stream_calls_on_tool_call_for_dynamic_tools`,
  `process_ui_message_stream_calls_on_error_for_error_chunks`,
  `read_ui_message_stream_terminates_on_error_chunk_when_requested`,
  `get_response_ui_message_id_returns_none_without_original_messages`,
  `get_response_ui_message_id_reuses_last_assistant_message_id`,
  `get_response_ui_message_id_generates_when_last_message_is_not_assistant`,
  `get_response_ui_message_id_uses_generator_for_empty_messages`,
  `get_response_ui_message_id_uses_fixed_response_id`,
  `ui_message_serializes_upstream_minimal_shape`,
  `validate_ui_messages_should_throw_invalid_argument_error_when_messages_parameter_is_null`,
  `validate_ui_messages_should_throw_invalid_argument_error_when_messages_parameter_is_undefined`,
  `validate_ui_messages_should_throw_type_validation_error_when_messages_array_is_empty`,
  `validate_ui_messages_should_throw_type_validation_error_when_message_has_empty_parts_array`,
  `validate_ui_messages_should_validate_a_user_message_with_metadata_when_no_metadata_schema_is_provided`,
  `validate_ui_messages_should_validate_a_user_message_with_metadata`,
  `validate_ui_messages_should_throw_type_validation_error_when_metadata_is_invalid`,
  `validate_ui_messages_should_validate_text_part_with_provider_metadata`,
  `validate_ui_messages_should_validate_a_user_message_with_a_text_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_custom_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_custom_part_without_provider_metadata`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_reasoning_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_source_url_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_source_document_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_file_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_step_start_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_two_data_parts`,
  `validate_ui_messages_should_throw_type_validation_error_when_data_is_invalid`,
  `validate_ui_messages_should_throw_type_validation_error_when_there_is_no_data_schema_for_a_data_part`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_dynamic_tool_part_in_output_error_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_dynamic_tool_part_in_input_streaming_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_dynamic_tool_part_in_input_available_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_dynamic_tool_part_in_output_available_state`,
  `validate_ui_messages_should_validate_a_dynamic_tool_part_in_output_error_state_when_input_key_is_absent`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_tool_part_in_input_streaming_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_tool_part_in_input_available_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_tool_part_in_output_available_state`,
  `validate_ui_messages_should_validate_an_assistant_message_with_a_tool_part_in_output_error_state`,
  `validate_ui_messages_should_validate_tool_input_when_state_is_input_available`,
  `validate_ui_messages_should_validate_tool_input_and_output_when_state_is_output_available`,
  `validate_ui_messages_should_preserve_result_provider_metadata_when_state_is_output_available`,
  `validate_ui_messages_should_validate_tool_input_when_state_is_output_error_and_there_is_input`,
  `validate_ui_messages_should_preserve_result_provider_metadata_when_state_is_output_error`,
  `validate_ui_messages_should_skip_tool_input_validation_when_state_is_output_error_and_there_is_no_input`,
  `validate_ui_messages_should_validate_a_tool_part_in_output_error_state_when_input_key_is_absent`,
  `validate_ui_messages_should_preserve_raw_input_when_state_is_output_error`,
  `validate_ui_messages_should_throw_error_when_no_tool_schema_is_found`,
  `validate_ui_messages_should_skip_validation_for_tool_part_in_output_available_state_when_tool_schema_is_missing`,
  `validate_ui_messages_should_skip_validation_for_tool_part_in_output_error_state_when_tool_schema_is_missing`,
  `validate_ui_messages_should_skip_validation_for_tool_part_in_output_denied_state_when_tool_schema_is_missing`,
  `validate_ui_messages_should_validate_automatic_approval_reasons_on_output_parts`,
  `validate_ui_messages_should_throw_error_when_tool_input_validation_fails`,
  `validate_ui_messages_should_throw_error_when_tool_output_validation_fails`,
  `validate_ui_messages_should_not_validate_input_in_input_streaming_state`,
  `safe_validate_ui_messages_should_return_success_result_for_valid_messages`,
  `safe_validate_ui_messages_should_return_failure_result_when_messages_parameter_is_null`,
  `safe_validate_ui_messages_should_return_failure_result_when_messages_array_is_empty`,
  `safe_validate_ui_messages_should_return_failure_result_when_message_has_empty_parts_array`,
  `safe_validate_ui_messages_should_return_failure_result_when_metadata_validation_fails`,
  `safe_validate_ui_messages_should_return_failure_result_when_tool_input_validation_fails`,
  `safe_validate_ui_messages_should_return_failure_result_when_data_schema_is_missing`,
  `safe_validate_ui_messages_should_return_failure_result_for_invalid_message_structure`.
- 2026-05-25: `packages/ai` `util.rs` exact-name follow-up added named Rust
  counterparts for `fix_json_upstream_handles_closing_brace_after_number_in_object`,
  `fix_json_upstream_handles_closing_brace_after_string_in_object`,
  `fix_json_upstream_handles_closing_brace_after_literal_in_object`,
  `fix_json_upstream_handles_closing_brace_after_array_in_object`,
  `fix_json_upstream_handles_closing_brace_after_object_in_object`,
  `fix_json_upstream_handles_partial_keys_first_key`,
  `fix_json_upstream_handles_partial_keys_second_key`,
  `fix_json_upstream_handles_partial_keys_with_colon_second_key`,
  `fix_json_upstream_handles_trailing_whitespace`,
  `fix_json_upstream_handles_closing_after_empty_object`,
  `fix_json_upstream_handles_nested_arrays_with_numbers`,
  `fix_json_upstream_handles_nested_arrays_with_literals`,
  `fix_json_upstream_handles_nested_objects`,
  `fix_json_upstream_handles_nested_objects_with_numbers`,
  `fix_json_upstream_handles_nested_objects_with_literals`,
  `fix_json_upstream_handles_arrays_within_objects`,
  `fix_json_upstream_handles_objects_within_arrays`,
  `fix_json_upstream_handles_nested_arrays_and_objects`,
  `fix_json_upstream_handles_deeply_nested_objects`,
  `fix_json_upstream_handles_potential_nested_arrays_or_objects`,
  `fix_json_upstream_handles_complex_nesting_1`,
  `fix_json_upstream_handles_empty_objects_inside_nested_objects_and_arrays`,
  `invalid_argument_error_retains_parameter_value_and_message`,
  `cosine_similarity_should_calculate_cosine_similarity_correctly`,
  `cosine_similarity_should_calculate_negative_cosine_similarity_correctly`,
  `cosine_similarity_should_throw_error_when_vectors_have_different_lengths`,
  `cosine_similarity_returns_zero_for_empty_vectors`,
  `cosine_similarity_should_give_zero_when_one_of_the_vectors_is_a_zero_vector`,
  `cosine_similarity_should_handle_vectors_with_very_small_magnitudes`,
  `parse_partial_json_returns_undefined_input_for_missing_text`,
  `parse_partial_json_returns_successful_parse_for_valid_json`,
  `parse_partial_json_repairs_incomplete_literals_strings_arrays_and_objects`,
  `parse_partial_json_trims_incomplete_numbers_before_repair_parse`,
  `parse_partial_json_returns_failed_parse_when_repair_cannot_make_valid_json`,
  `parse_partial_json_deserializes_state_shape`,
  `get_text_from_data_url_decodes_base64_text`,
  `get_text_from_data_url_accepts_empty_payloads`,
  `get_text_from_data_url_uses_atob_style_byte_mapping`,
  `get_text_from_data_url_rejects_missing_payload_or_media_type`,
  `get_text_from_data_url_rejects_invalid_base64_payloads`.
- 2026-05-25: `packages/ai` `language_model_middleware.rs` exact-name follow-up
  added named Rust counterparts for `language_model_middleware_call_type_uses_upstream_literals`,
  `language_model_default_settings_serialize_as_upstream_partial_call_options`,
  `default_settings_middleware_applies_defaults_without_overriding_params`,
  `default_settings_middleware_deep_merges_provider_options`,
  `default_settings_middleware_should_apply_default_settings`,
  `default_settings_middleware_should_give_precedence_to_user_provided_settings`,
  `default_settings_middleware_should_merge_provider_metadata_with_default_settings`,
  `default_settings_middleware_should_merge_complex_provider_metadata_objects`,
  `default_settings_middleware_should_keep_zero_temperature_when_default_is_not_set`,
  `default_settings_middleware_should_apply_default_max_output_tokens`,
  `default_settings_middleware_should_prioritize_param_max_output_tokens`,
  `default_settings_middleware_should_apply_default_stop_sequences`,
  `default_settings_middleware_should_prioritize_param_stop_sequences`,
  `default_settings_middleware_should_apply_default_top_p`,
  `default_settings_middleware_should_prioritize_param_top_p`,
  `default_settings_middleware_should_merge_headers`,
  `default_settings_middleware_should_handle_empty_default_headers`,
  `default_settings_middleware_should_handle_empty_param_headers`,
  `default_settings_middleware_should_handle_both_headers_being_undefined`,
  `default_settings_middleware_should_handle_empty_default_provider_options`,
  `default_settings_middleware_should_handle_empty_param_provider_options`,
  `default_settings_middleware_should_handle_both_provider_options_being_undefined`,
  `extract_json_middleware_default_transform_strips_markdown_fences`,
  `extract_json_middleware_transforms_generate_text_parts`,
  `extract_json_middleware_transforms_vec_stream_text_blocks`,
  `extract_reasoning_middleware_extracts_generate_text_tags`,
  `extract_reasoning_middleware_supports_start_with_reasoning_for_generate`,
  `extract_reasoning_middleware_extracts_split_stream_tags`,
  `extract_reasoning_middleware_separates_multiple_stream_tags`,
  `simulate_streaming_middleware_turns_generate_result_into_vec_stream`,
  `language_model_middleware_exposes_upstream_v4_hooks`,
  `language_model_middleware_hooks_are_optional_by_default`,
  `wrap_language_model_applies_identity_and_supported_url_overrides`,
  `wrap_language_model_transforms_params_before_wrapping_generate_and_stream`.
- 2026-05-25: `packages/ai` `retry.rs` exact-name follow-up added named Rust
  counterparts for `retry_error_reason_matches_upstream_strings`,
  `retry_error_retains_upstream_context_and_serializes`,
  `retry_error_deserializes_and_omits_absent_last_error`,
  `retry_options_use_upstream_defaults_and_serialize`,
  `retry_attempt_and_failure_errors_serialize`,
  `retry_executor_returns_success_without_retrying`,
  `retry_executor_retries_retryable_api_errors_with_headers_and_backoff`,
  `retry_with_exponential_backoff_uses_rate_limit_header_delay_when_present_and_reasonable`,
  `retry_with_exponential_backoff_parses_retry_after_header_in_seconds`,
  `retry_with_exponential_backoff_uses_exponential_backoff_when_rate_limit_delay_is_too_long`,
  `retry_with_exponential_backoff_falls_back_to_exponential_backoff_when_no_rate_limit_headers`,
  `retry_with_exponential_backoff_handles_invalid_rate_limit_header_values`,
  `retry_with_exponential_backoff_handles_anthropic_429_response_with_retry_after_ms_header`,
  `retry_with_exponential_backoff_handles_openai_429_response_with_retry_after_header`,
  `retry_with_exponential_backoff_handles_multiple_retries_with_exponential_backoff_progression`,
  `retry_with_exponential_backoff_prefers_retry_after_ms_over_retry_after_when_both_present`,
  `retry_with_exponential_backoff_handles_retry_after_header_with_http_date_format`,
  `retry_with_exponential_backoff_falls_back_to_exponential_backoff_when_rate_limit_delay_is_negative`,
  `retry_with_exponential_backoff_retries_on_gateway_internal_server_error`,
  `retry_with_exponential_backoff_retries_on_gateway_rate_limit_error`,
  `retry_with_exponential_backoff_does_not_retry_on_non_retryable_gateway_authentication_error`,
  `retry_with_exponential_backoff_uses_retry_after_headers_from_api_call_error_cause`,
  `retry_executor_wraps_max_retries_exceeded`,
  `retry_executor_passes_through_non_retryable_first_errors_and_disabled_retries`,
  `retry_executor_wraps_non_retryable_error_after_prior_retry`,
  `retry_executor_passes_through_abort_errors`,
  `retry_delay_uses_exponential_backoff_without_headers`,
  `retry_delay_prefers_retry_after_ms_header`,
  `retry_delay_parses_retry_after_seconds_and_float_prefixes`,
  `retry_delay_falls_back_when_header_delay_is_unreasonable`,
  `retry_delay_uses_retry_after_when_retry_after_ms_is_invalid`,
  `retry_delay_does_not_use_retry_after_when_retry_after_ms_is_unreasonable`,
  `retry_delay_parses_retry_after_http_dates`,
  `retry_delay_rejects_negative_and_past_date_values`.
- 2026-05-25: `packages/ai` `generate_object.rs` exact-name follow-up added
  named Rust counterparts for `generate_object_result_serializes_full_upstream_shape`,
  `generate_object_result_deserializes_minimal_upstream_shape`,
  `generate_object_result_supports_typed_objects`,
  `generate_object_type_counterpart_does_not_accept_timeout_option`,
  `generate_object_type_counterpart_supports_enum_types`,
  `generate_object_type_counterpart_supports_schema_types`,
  `generate_object_type_counterpart_supports_no_schema_output_mode`,
  `generate_object_type_counterpart_supports_array_output_mode`,
  `generate_object_calls_model_with_json_response_format_and_parses_text`,
  `generate_object_calls_log_warnings_with_the_correct_warnings`,
  ...
  `generate_object_passes_headers_to_model`,
  `generate_object_passes_provider_options_to_model`,
  `generate_object_messages_with_url_file_calls_model_supported_urls`,
  `generate_object_retries_retryable_pre_content_errors`,
  `generate_object_options_from_prompt_standardizes_high_level_prompt`,
  `generate_object_options_from_prompt_rejects_invalid_high_level_prompt`,
  `generate_object_forwards_schema_metadata_and_validates_output`,
  ...
  `generate_object_lifecycle_events_use_upstream_json_shapes`,
  `generate_object_invokes_lifecycle_callbacks_with_single_step_events`,
  `generate_object_callback_panics_do_not_break_generation`,
  `generate_object_dispatches_telemetry_lifecycle_events`,
  `generate_object_accepts_experimental_telemetry_alias`,
  `generate_object_forwards_array_schema_and_returns_elements`,
  `generate_object_reports_array_shape_failures_as_no_object_errors`,
  `generate_object_reports_array_element_validation_failures`,
  `generate_object_forwards_enum_schema_and_returns_selected_value`,
  `generate_object_reports_enum_validation_failures_as_no_object_errors`,
  `generate_object_repairs_enum_validation_failures`,
  `generate_object_reports_schema_validation_failures_as_no_object_errors`,
  `generate_object_repairs_parse_failures`,
  `generate_object_repairs_schema_validation_failures`,
  `generate_object_keeps_original_error_when_repair_returns_none`,
  `generate_object_reports_repaired_text_error_when_repair_fails`,
  `generate_object_reports_parse_failures_as_no_object_errors`,
  `generate_object_on_start_runs_before_model_call`,
  `generate_object_on_start_sends_text_prompt_information`,
  `generate_object_accepts_deprecated_experimental_telemetry_as_alias`,
  `generate_object_on_step_start_runs_before_model_call`,
  `generate_object_on_step_start_provides_step_number_and_model_info`,
  `generate_object_on_step_finish_runs_after_model_call_with_raw_result`,
  `generate_object_on_step_finish_provides_raw_object_text_and_usage`,
  `generate_object_on_step_finish_includes_reasoning`,
  `generate_object_on_finish_runs_after_parsing_with_typed_object`,
  `generate_object_on_finish_provides_parsed_object_and_metadata`,
  `generate_object_on_finish_includes_reasoning`,
  `generate_object_callbacks_fire_in_order`,
  `generate_object_callbacks_correlate_events_with_same_call_id`,
  `generate_object_callbacks_should_not_break_generation_when_callback_panics`.
- 2026-05-25: `packages/ai` `registry.rs` exact-name follow-up added named Rust
  counterparts for `registry_options_default_to_upstream_separator`,
  `split_registry_model_id_matches_upstream_first_separator_behavior`,
  `split_registry_model_id_reports_upstream_missing_separator_message`,
  `create_provider_registry_resolves_required_model_interfaces`,
  `create_provider_registry_resolves_transcription_model_interface`,
  `create_provider_registry_resolves_speech_model_interface`,
  `create_provider_registry_resolves_reranking_model_interface`,
  `create_provider_registry_resolves_video_model_interface`,
  `create_provider_registry_resolves_files_interface`,
  `create_provider_registry_resolves_skills_interface`,
  `create_provider_registry_supports_custom_separator`,
  `provider_registry_reports_missing_provider_context`,
  `provider_registry_reports_missing_model_context`,
  `provider_registry_reports_missing_transcription_provider_context`,
  `provider_registry_reports_missing_transcription_model_context`,
  `provider_registry_reports_missing_speech_provider_context`,
  `provider_registry_reports_missing_speech_model_context`,
  `provider_registry_reports_missing_reranking_provider_context`,
  `provider_registry_reports_missing_reranking_model_context`,
  `provider_registry_reports_missing_video_provider_context`,
  `provider_registry_reports_missing_video_model_context`,
  `provider_registry_reports_missing_files_provider_context`,
  `provider_registry_reports_missing_skills_provider_context`,
  `custom_provider_should_resolve_string_model_ids_through_the_explicit_default_provider`,
  `no_such_provider_error_matches_upstream_default_message`,
  `no_such_provider_error_retains_custom_message_context`.
...
- 2026-05-23: `packages/ai` `createAgentUIStreamResponse` parity added
  ...
  and
  `create_agent_ui_stream_response_passes_sandbox_to_tool_execution`.
  ...
  chunks, auto-populates original messages for `onFinish` persistence
  callbacks, and forwards the experimental sandbox into local tool execution.
...
- 2026-05-25: `packages/ai` `ToolLoopAgent` call/stream parameter alias and
  stream-transform parity added `AgentCallParameters` and
  `AgentStreamParameters` root-facade type aliases plus per-call stream
  transform forwarding through `with_transform`/`with_transforms` and prepared
  call application, matching the upstream `generate`/`stream` call-option
  surface while keeping the existing `includeRuntimeContext`/
  `includeToolsContext` compile-only key-inference gap documented as
  JavaScript-only.
- 2026-05-25: `packages/ai` `generate-text/output` helper parity added
  `generate_text_output::{text, object, array, choice, json}` plus complete
  parse/partial/response-format coverage for the portable upstream
  `generate-text/output.test.ts` cases, including array element streaming and
  partial JSON repair behavior.
- 2026-05-25: `packages/ai` `generateText` include-retention parity confirms
  the portable `requestBody`, `requestMessages`, and `responseBody` retention
  cases through `GenerateTextInclude`/`with_include`, while documenting the
  deprecated JavaScript `experimental_include` alias as JS-only builder-surface
  compatibility because Rust exposes only the typed include builder.
- 2026-05-25: `packages/ai` deprecated `experimental_onToolCallStart` and
  `experimental_onToolCallFinish` callback parity added generateText and
  streamText fallback/preference coverage, proving the deprecated aliases
  still invoke the callbacks when used alone and that the newer
  `onToolExecutionStart`/`onToolExecutionEnd` callbacks win when both surfaces
  are provided.
- 2026-05-25: `packages/ai` `generateText` prepare-step initial approval
  response-message and model-switch URL-support parity added, proving the
  responseMessages handed to `prepareStep` include the initial approved tool
  result and that URL support checks are performed against the model selected
  by `prepareStep`.
- 2026-05-25: `packages/ai` `streamText` multi-step warning aggregation parity
  added `stream_text_result_warnings_resolve_with_warnings_from_all_steps` and
  `stream_text_result_warnings_are_passed_to_on_finish`, proving aggregated
  `result.warnings` and `onFinish.warnings` include warnings from every step
  while preserving the final-step warning list in the final step record.
- 2026-05-25: `packages/ai` `streamText` and `streamObject` `onStepStart`
  telemetry metadata suppression parity added
  `stream_text_on_step_start_omits_telemetry_metadata` and
  `stream_object_on_step_start_omits_telemetry_metadata`, proving the
  serialized step-start callback events do not leak `functionId` even when
  telemetry is enabled.
- 2026-05-25: `packages/ai` `streamText` telemetry runtime-context default
  exclusion parity added
  `stream_text_telemetry_excludes_runtime_context_by_default`, proving the
  upstream telemetry integrations still observe empty `runtimeContext` payloads
  unless the include-context telemetry option is explicitly enabled.
- 2026-05-25: `packages/ai` `streamText` telemetry tools-context inclusion
  parity added `stream_text_telemetry_includes_configured_tools_context_properties`,
...
- 2026-05-25: `packages/ai` `streamText` response-format request-body parity
  added `stream_text_passes_response_format_to_model`, proving the streamed
  call forwards JSON response formats with schemas to the underlying model
  call options.
- 2026-05-25: `packages/ai` `generateText` and `streamText` reasoning
  provider-default pass-through added `generate_text_passes_provider_default_reasoning_to_model`
  and `stream_text_passes_provider_default_reasoning_to_model`, proving the
  provider-default reasoning value is forwarded through both wrapper call
  boundaries unchanged.
- 2026-05-25: `packages/ai` `generateText` and `streamText` reasoning high
  pass-through added `generate_text_passes_high_reasoning_to_model` and
  `stream_text_passes_high_reasoning_to_model`, proving the high reasoning
  value is forwarded through both wrapper call boundaries unchanged.
- 2026-05-25: `packages/ai` `streamText` UI-message sendReasoning true parity
  added `stream_text_result_to_ui_message_stream_supports_send_reasoning_true`
  alongside the existing sendReasoning false coverage, proving the UI-message
  stream can include reasoning chunks when explicitly enabled.
- 2026-05-25: `packages/ai` `streamText`/`generateText` timeout request-control
  parity added `generate_text_forwards_timeout_as_abort_signal_to_model`,
  `generate_text_merges_timeout_with_abort_signal`,
  `stream_text_forwards_timeout_as_abort_signal_to_model`,
  `stream_text_merges_timeout_with_abort_signal`,
  `stream_text_passes_undefined_when_no_timeout_or_abort_signal_provided`, and
...
- 2026-05-26: `packages/ai` `streamText` abort-while-stream-starting parity
  added `stream_text_aborts_while_waiting_for_do_stream_and_invokes_on_abort`,
...
- 2026-05-25: `packages/ai` `generateText` telemetry context-filtering parity
  ...
  `generate_text_telemetry_excludes_tools_context_by_default`, and
  `generate_text_telemetry_excludes_runtime_context_by_default`, proving the
  upstream telemetry integrations receive the filtered `runtimeContext` and
  `toolsContext` payloads across start, step-start, step-finish, and end
  events when the include options are set and empty payloads by default when
  they are not.
- 2026-05-25: `packages/ai` `ToolLoopAgent` exported type-surface parity added
  root-facade `assert_type` coverage for `ToolLoopAgent`,
  `ToolLoopAgentCallOptions`, `ToolLoopAgentModelSettings`,
  `ToolLoopAgentPrepareCall`, `ToolLoopAgentPreparedCall`,
  `ToolLoopAgentOnStartCallback`, `ToolLoopAgentOnStepStartCallback`,
  `ToolLoopAgentOnToolExecutionStartCallback`,
  `ToolLoopAgentOnToolExecutionEndCallback`,
  `ToolLoopAgentOnStepFinishCallback`, `ToolLoopAgentOnFinishCallback`, and
  `AgentUiStreamResponseOptions`, while the remaining upstream compile-only
  `includeRuntimeContext`/`includeToolsContext` key-inference checks are
  documented as JavaScript-only.
...
- 2026-05-25: `packages/ai` callback utility exact-name follow-up added named
  Rust counterparts for `merge_callbacks_should_invoke_callbacks_in_parallel_wait_for_them_to_settle_and_continue_after_errors`, `merge_callbacks_should_ignore_rejected_callbacks`, `merge_callbacks_should_ignore_undefined_callbacks`, `notify_should_call_a_single_callback_with_the_event`, `notify_should_call_all_callbacks_when_given_an_array`, `notify_should_handle_undefined_callbacks`, `notify_should_handle_omitted_callbacks`, `notify_should_await_async_callbacks_before_continuing`, `notify_should_run_async_callbacks_in_parallel_and_await_all_of_them`, `notify_should_catch_errors_in_a_single_callback_without_breaking`, `notify_should_catch_errors_in_array_callbacks_and_continue_to_next`, `notify_should_catch_async_rejection_without_breaking`, `notify_should_preserve_event_type_through_to_callback`, `notify_should_work_with_complex_nested_event_types`, and `notify_should_handle_repeated_calls_with_the_same_callback`.
- 2026-05-25: `packages/ai` serial-job and abort utility exact-name follow-up added named Rust counterparts for `serial_job_executor_should_execute_a_single_job_successfully`, `serial_job_executor_should_execute_multiple_jobs_in_serial_order`, `serial_job_executor_should_handle_job_errors_correctly`, `serial_job_executor_should_execute_jobs_one_at_a_time`, `serial_job_executor_should_handle_mixed_success_and_failure_jobs`, `serial_job_executor_should_handle_concurrent_calls_to_run`, `merge_abort_signals_should_return_a_signal_that_is_initially_not_aborted`, `merge_abort_signals_should_abort_when_the_first_signal_aborts`, `merge_abort_signals_should_abort_when_the_second_signal_aborts`, `merge_abort_signals_should_preserve_the_abort_reason_from_the_triggering_signal`, `merge_abort_signals_should_preserve_string_abort_reason`, `merge_abort_signals_should_handle_already_aborted_signals`, `merge_abort_signals_should_use_the_first_already_aborted_signal_reason_when_multiple_are_aborted`, `merge_abort_signals_should_return_none_when_no_signals_are_provided`, `merge_abort_signals_should_return_none_when_only_absent_signals_are_provided`, `merge_abort_signals_should_create_a_timeout_signal_from_numeric_input`, `merge_abort_signals_should_preserve_the_first_abort_reason_when_mixing_signals_and_timeouts`, `merge_abort_signals_should_filter_out_absent_signals`, `merge_abort_signals_should_return_the_signal_directly_when_only_one_valid_signal_is_provided`, `merge_abort_signals_should_use_the_first_aborting_signal_reason_when_multiple_abort_simultaneously`, `merge_abort_signals_should_return_the_original_signal_when_only_one_signal_is_provided`, and `merge_abort_signals_should_work_with_many_signals`.
- 2026-05-25: `packages/ai` timeout and deep-equal utility exact-name follow-up added named Rust counterparts for `set_abort_timeout_should_not_abort_the_controller_before_the_timeout_elapses`, `set_abort_timeout_should_abort_the_controller_when_the_timeout_elapses`, `set_abort_timeout_should_abort_with_a_timeout_error_reason`, `set_abort_timeout_should_include_the_label_and_duration_in_the_abort_reason_message`, `set_abort_timeout_should_return_a_handle_that_can_be_cancelled_to_cancel_the_abort`, `set_abort_timeout_should_return_none_when_abort_controller_is_absent`, `set_abort_timeout_should_return_none_when_timeout_ms_is_absent`, `is_deep_equal_data_compares_primitives`, `is_deep_equal_data_compares_nested_objects`, and `is_deep_equal_data_compares_arrays_by_order_and_length`.
- 2026-05-25: `packages/ai` deep-equal utility exact-name follow-up added named Rust counterparts for `is_deep_equal_data_distinguishes_objects_from_arrays`, `is_deep_equal_data_should_check_if_two_primitives_are_equal`, `is_deep_equal_data_should_return_false_for_different_types`, `is_deep_equal_data_should_return_false_for_null_values_compared_with_objects`, `is_deep_equal_data_should_identify_two_equal_objects`, `is_deep_equal_data_should_identify_two_objects_with_different_values`, `is_deep_equal_data_should_identify_two_objects_with_different_number_of_keys`, `is_deep_equal_data_should_handle_nested_objects`, `is_deep_equal_data_should_detect_inequality_in_nested_objects`, and `is_deep_equal_data_should_compare_arrays_correctly`.
- 2026-05-25: `packages/ai` utility exact-name follow-up added named Rust counterparts for `is_deep_equal_data_should_return_false_for_null_comparison_with_object`, `is_deep_equal_data_should_distinguish_between_array_and_object_with_same_enumerable_properties`, `prepare_headers_should_set_content_type_header_if_not_present`, `prepare_headers_should_not_overwrite_existing_content_type_header`, `prepare_headers_should_handle_undefined_init`, `prepare_headers_should_handle_init_headers_as_headers_object`, `prepare_headers_should_handle_response_object_headers`, `merge_objects_should_merge_two_flat_objects`, `merge_objects_should_deeply_merge_nested_objects`, and `merge_objects_should_replace_arrays_instead_of_merging_them`.
- 2026-05-25: `packages/ai` merge-objects utility exact-name follow-up added named Rust counterparts for `merge_objects_should_handle_null_values`, `merge_objects_should_handle_complex_nested_structures`, `merge_objects_should_handle_empty_objects`, `merge_objects_should_handle_undefined_inputs`, `merge_objects_should_not_pollute_object_prototype_via_proto`, `merge_objects_should_ignore_proto_constructor_and_prototype_keys`, `merge_objects_should_ignore_dangerous_keys_nested_in_mergeable_objects`, `merge_objects_deeply_merges_json_objects`, `merge_objects_handles_missing_inputs_and_replacements`, and `merge_objects_ignores_dangerous_override_keys`.
- 2026-05-25: `packages/ai` split-array and start-index utility exact-name follow-up added named Rust counterparts for `split_array_should_split_an_array_into_chunks_of_the_specified_size`, `split_array_should_return_empty_array_when_input_array_is_empty`, `split_array_should_return_original_array_when_chunk_size_is_greater_than_array_length`, `split_array_should_return_original_array_when_chunk_size_is_equal_to_array_length`, `split_array_should_handle_chunk_size_of_one_correctly`, `split_array_should_throw_error_for_chunk_size_of_zero`, `split_array_should_throw_error_for_negative_chunk_size`, `split_array_should_handle_non_integer_chunk_size_by_flooring_the_size`, `get_potential_start_index_should_return_null_when_searched_text_is_empty`, and `get_potential_start_index_should_return_null_when_searched_text_is_not_in_text`.
- 2026-05-25: `packages/ai` start-index and JSON-fix utility exact-name follow-up added named Rust counterparts for `get_potential_start_index_should_return_index_when_searched_text_is_in_text`, `get_potential_start_index_should_return_index_when_searched_text_might_start_in_text`, `get_potential_start_index_should_return_index_for_longer_possible_overlap`, `get_potential_start_index_should_return_index_for_longest_possible_overlap`, `fix_json_repairs_incomplete_literals_numbers_and_strings`, `fix_json_repairs_incomplete_arrays_and_objects`, `fix_json_upstream_handles_empty_input`, `fix_json_upstream_handles_incomplete_null`, `fix_json_upstream_handles_incomplete_true`, and `fix_json_upstream_handles_incomplete_false`.
- 2026-05-25: `packages/ai` fix-json utility exact-name follow-up added named Rust counterparts for `fix_json_upstream_handles_incomplete_numbers`, `fix_json_upstream_handles_numbers_with_dot`, `fix_json_upstream_handles_negative_numbers`, `fix_json_upstream_handles_incomplete_negative_numbers`, `fix_json_upstream_handles_e_notation_numbers`, `fix_json_upstream_handles_uppercase_e_notation_numbers`, `fix_json_upstream_handles_incomplete_exponent_numbers`, `fix_json_upstream_handles_incomplete_strings`, `fix_json_upstream_handles_escape_sequences`, and `fix_json_upstream_handles_incomplete_escape_sequences`.
- 2026-05-25: `packages/ai` fix-json continuation exact-name follow-up added named Rust counterparts for `fix_json_upstream_handles_unicode_characters`, `fix_json_upstream_handles_incomplete_array`, `fix_json_upstream_handles_closing_bracket_after_number_in_array`, `fix_json_upstream_handles_closing_bracket_after_string_in_array`, `fix_json_upstream_handles_closing_bracket_after_literal_in_array`, `fix_json_upstream_handles_closing_bracket_after_array_in_array`, `fix_json_upstream_handles_closing_bracket_after_object_in_array`, `fix_json_upstream_handles_trailing_comma`, `fix_json_upstream_handles_closing_array`, and `fix_json_upstream_handles_keys_without_values`.
...
- 2026-05-26: `packages/ai` `streamText` prepareStep accumulation parity
  added the named Rust counterpart
  `stream_text_prepare_step_passes_accumulated_steps_to_subsequent_prepare_step_calls`,
  proving multi-step `prepareStep` callbacks see the current step number plus
  the accumulated step and message history while the stream performs multiple
  `doStream` calls.
...
- 2026-05-25: `packages/ai` `streamText` interleaved reasoning/text ordering
  ...
  proving the combined stream part order, accumulated text, reasoning text,
  and step content preserve the upstream reasoning/text interleaving.
- 2026-05-23: `packages/ai` `streamText` `result.fullStream`
...
- 2026-05-26: MCP clientInfo version/name parity added
  `mcp_client_uses_custom_client_version_when_provided`,
  `mcp_client_uses_client_name_for_client_info_when_provided`, and
  `mcp_client_prefers_client_name_over_deprecated_name_for_client_info`
  now explicitly cover upstream client-info initialization cases for custom
  version strings, explicit client names, and deprecated-name precedence.
- 2026-05-26: MCP protocol downgrade negotiation added
...
- 2026-05-26: WorkflowAgent instruction-shape parity added the named Rust
  counterparts `workflow_agent_upstream_should_pass_string_instructions_to_the_model`,
  `workflow_agent_compat_should_pass_system_message_instructions`, and
  `workflow_agent_compat_should_pass_array_of_system_message_instructions` for
  the upstream `workflow-agent-e2e.integration.test.ts` `string instructions
  are passed to the model` case plus the `workflow-agent-compat.test.ts`
  string, single system-message, and array-of-system-message constructor
  instruction cases, proving constructor instructions are prepended to the
  prompt that reaches the deterministic workflow executor in all portable
  upstream instruction shapes.
...
- 2026-05-26: WorkflowAgent stream-option forwarding parity added named Rust
  ...
  `experimental_repairToolCall` cases, and the stream-level `onError`
  callback case, proving those workflow-agent options are forwarded into the
  deterministic workflow stream executor.
- 2026-05-26: WorkflowAgent prompt-normalization and telemetry forwarding
  parity added named Rust counterparts for upstream `workflow-agent.test.ts`
  `should accept a string prompt instead of messages`, `should accept an
  array of messages as prompt`, `should pass includeRawChunks to
  streamTextIterator`, `should pass telemetry settings from constructor to
  streamTextIterator`, and `should allow stream options to override
...
- 2026-05-25: `streamText` `options.messages` supportedUrls context parity
  ...
  proving the `supportedUrls()` hook can read `self.model_id()` while still
  accepting URL-backed message files.
- 2026-05-25: `streamText` telemetry runtimeContext inclusion parity added
...
- 2026-05-25: `streamText`/`streamObject` output-strategy parity tightened
  `choice_output_handles_complete_and_partial_choice_values`,
  `stream_object_enum_output_unwraps_result`,
...
- 2026-05-25: Gateway OpenAI-compatible live OTel proof expanded ignored
...
- 2026-05-26: `WorkflowAgent` constructor and stream-level stop-condition, repair-callback, and onError forwarding parity added named Rust counterparts in `crates/ai-sdk-workflow`, covering constructor defaults and stream overrides for stop conditions, `experimental_repair_tool_call`, and `onError` on the deterministic workflow iterator.
...
1. Finish ALL common/core SDK packages together with Vercel AI Gateway coverage
   ...
   Workflow still has the upstream `collectUIMessages` block commented out and
   documented as intentionally non-portable, while the portable stop-condition, repair, and error-forwarding cases are now covered in Rust.
...
4. Do not spend the next first-phase slice on the current
   ...
   counterparts for every portable current Responses case. The remaining
   non-Responses `packages/openai` surfaces are now fully mapped in Rust, so
   do not spend another slice there unless upstream changes or a regression
   appears.
...
8. Continue `streamText` parity with true post-return `createUIMessageStream`
   ...
   counterparts in
   `generate_text_prepare_step_uses_initial_tool_result_in_response_messages`
   and `generate_text_prepare_step_model_switch_uses_step_model_supported_urls`.
   ...
   user-callback plus integration ordering, panic isolation, and array
   integration ordering.
   The upstream `generateText` provider-executed approval-response approved
   continuation case now has the named Rust counterpart
   ...
   segmentation.
9. Continue `streamText` parity with any remaining portable
   output-strategy stream-result edge cases after the Gateway text/stream/UI
   path is stronger. `streamObject` is verified and its Node.js response-like
   writer case is already documented as JavaScript-only. The upstream `streamText`
   incremental partial-output stream cases for text, object, and array outputs
   now have named Rust counterparts in
   `stream_text_result_partial_output_stream_collects_incremental_text_outputs`,
   `stream_text_result_partial_output_stream_repairs_incremental_object_outputs`,
   and
   `stream_text_result_partial_output_stream_repairs_incremental_array_outputs_and_elements`.
   The upstream choice-output partial-output cases now have a named Rust
   counterpart in `choice_output_handles_complete_and_partial_choice_values`.
   The upstream `streamObject` enum-output cases now have named Rust
   counterparts in `stream_object_enum_output_unwraps_result`,
   `stream_object_enum_output_streams_value_and_sends_response_format`,
   `stream_object_enum_output_completes_unambiguous_prefixes`,
   `stream_object_enum_output_keeps_ambiguous_prefixes_until_resolved`,
   ...
   The upstream `should write text deltas to a Node.js response-like object`
   case is documented as JavaScript-only and intentionally not ported.
...
## Provider-utils Helper Inventory

The remaining exact-name helper mappings from `crates/ai-sdk-provider-utils/src/provider_utils.rs` are listed below.
2026-05-26: the ledger now makes the largest remaining exact-name clusters explicit, including `is_url_supported_*`, `read_response_with_size_limit_*`, and the `detect_media_type_*` / `get_top_level_media_type_*` / `is_full_media_type_*` media-type helpers.
2026-05-26: the provider API request/response helpers are also now called out explicitly, including `provider_api_request_*`, `provider_api_response_*`, `handle_provider_api_response_*`, `execute_provider_api_request_*`, `get_from_api_*`, `post_json_to_api_*`, `post_form_data_to_api_*`, and `post_to_api_*`.
- `is_url_supported_upstream_returns_true_if_url_matches_under_specific_media_type`, `is_url_supported_upstream_returns_true_if_wildcard_matches_even_if_specific_exists`, `is_url_supported_upstream_returns_true_if_wildcard_matches_non_specified_media_type`, `is_url_supported_upstream_returns_false_if_url_matches_neither_specific_nor_wildcard`, `is_url_supported_upstream_returns_false_if_non_specified_media_type_misses_wildcard`, `is_url_supported_upstream_returns_true_if_empty_url_matches_pattern`, `is_url_supported_upstream_returns_false_if_empty_url_does_not_match_pattern`, `is_url_supported_upstream_is_case_insensitive_for_media_types`, `is_url_supported_upstream_handles_case_insensitive_regex_for_urls_if_specified`, `is_url_supported_upstream_is_case_insensitive_for_url_paths_by_default_regex`, `is_url_supported_upstream_returns_true_for_wildcard_subtype_match`, `is_url_supported_upstream_uses_full_wildcard_if_subtype_wildcard_is_not_matched`, `is_url_supported_upstream_matches_type_wildcard_key_for_top_level_only_media_type`, `is_url_supported_upstream_matches_wildcard_key_for_top_level_only_media_type`, `is_url_supported_upstream_does_not_match_specific_subtype_key_for_top_level_only_media_type`, `is_url_supported_upstream_does_not_match_different_top_level_wildcard_key`, `is_url_supported_upstream_returns_false_if_specific_media_type_has_empty_url_array`, `is_url_supported_upstream_falls_back_to_wildcard_if_specific_media_type_empty_array`, `is_url_supported_upstream_returns_false_if_empty_specific_array_and_wildcard_mismatch`, `read_response_with_size_limit_reads_chunks_within_limit`
- `read_response_with_size_limit_rejects_large_content_length_early`, `read_response_with_size_limit_rejects_streams_that_exceed_limit`, `read_response_with_size_limit_checks_larger_actual_body_even_when_length_claims_small`, `read_response_with_size_limit_uses_upstream_default_limit_and_ignores_invalid_lengths`, `detect_media_type_matches_top_level_signature_tables`, `detect_media_type_handles_base64_and_id3_prefixed_mp3`, `detect_media_type_returns_none_for_unsupported_or_unmatched_data`, `detect_media_type_without_top_level_type_uses_upstream_order`, `detect_media_type_upstream_detects_gif_from_bytes`, `detect_media_type_upstream_detects_gif_from_base64`, `detect_media_type_upstream_detects_png_from_bytes`, `detect_media_type_upstream_detects_png_from_base64`, `detect_media_type_upstream_detects_jpeg_from_bytes`, `detect_media_type_upstream_detects_jpeg_from_base64`, `detect_media_type_upstream_detects_webp_from_bytes`, `detect_media_type_upstream_detects_webp_from_base64`, `detect_media_type_upstream_does_not_detect_riff_audio_as_webp_from_bytes`, `detect_media_type_upstream_does_not_detect_riff_audio_as_webp_from_base64`, `detect_media_type_upstream_detects_bmp_from_bytes`, `detect_media_type_upstream_detects_bmp_from_base64`
- `detect_media_type_upstream_detects_tiff_little_endian_from_bytes`, `detect_media_type_upstream_detects_tiff_little_endian_from_base64`, `detect_media_type_upstream_detects_tiff_big_endian_from_bytes`, `detect_media_type_upstream_detects_tiff_big_endian_from_base64`, `detect_media_type_upstream_detects_avif_from_bytes`, `detect_media_type_upstream_detects_avif_from_base64`, `detect_media_type_upstream_detects_heic_from_bytes`, `detect_media_type_upstream_detects_heic_from_base64`, `detect_media_type_upstream_detects_mp3_from_bytes`, `detect_media_type_upstream_detects_mp3_from_base64`, `detect_media_type_upstream_detects_mp3_with_id3_tags_from_bytes`, `detect_media_type_upstream_detects_mp3_with_id3_tags_from_base64`, `detect_media_type_upstream_detects_wav_from_bytes`, `detect_media_type_upstream_detects_wav_from_base64`, `detect_media_type_upstream_does_not_detect_webp_as_wav_from_bytes`, `detect_media_type_upstream_does_not_detect_webp_as_wav_from_base64`, `detect_media_type_upstream_detects_ogg_from_bytes`, `detect_media_type_upstream_detects_ogg_from_base64`, `detect_media_type_upstream_detects_flac_from_bytes`, `detect_media_type_upstream_detects_flac_from_base64`
- `detect_media_type_upstream_detects_aac_from_bytes`, `detect_media_type_upstream_detects_aac_from_base64`, `detect_media_type_upstream_detects_mp4_from_bytes`, `detect_media_type_upstream_detects_mp4_from_base64`, `detect_media_type_upstream_detects_webm_from_bytes`, `detect_media_type_upstream_detects_webm_from_base64`, `detect_media_type_upstream_returns_undefined_for_unknown_image_formats`, `detect_media_type_upstream_returns_undefined_for_unknown_audio_formats`, `detect_media_type_upstream_returns_undefined_for_empty_arrays_for_image`, `detect_media_type_upstream_returns_undefined_for_empty_arrays_for_audio`, `detect_media_type_upstream_returns_undefined_for_short_arrays_for_image`, `detect_media_type_upstream_returns_undefined_for_short_arrays_for_audio`, `detect_media_type_upstream_returns_undefined_for_invalid_base64_strings_for_image`, `detect_media_type_upstream_returns_undefined_for_invalid_base64_strings_for_audio`, `get_top_level_media_type_upstream_returns_top_level_segment_for_full_media_type`, `get_top_level_media_type_upstream_returns_input_for_top_level_segment`, `get_top_level_media_type_upstream_normalizes_wildcards_to_top_level_segment`, `get_top_level_media_type_upstream_handles_edge_cases`, `is_full_media_type_upstream_returns_true_for_concrete_subtype`, `is_full_media_type_upstream_returns_false_for_top_level_only_media_types`
- `is_full_media_type_upstream_returns_false_for_wildcards`, `is_full_media_type_upstream_returns_false_for_edge_cases`, `detect_media_type_upstream_detects_image_types_when_top_level_type_is_image`, `detect_media_type_upstream_detects_audio_types_when_top_level_type_is_audio`, `detect_media_type_upstream_detects_video_types_when_top_level_type_is_video`, `detect_media_type_upstream_detects_document_types_when_top_level_type_is_application`, `detect_media_type_upstream_returns_undefined_for_text_top_level_segment`, `detect_media_type_upstream_returns_undefined_for_unknown_top_level_segments`, `detect_media_type_upstream_returns_undefined_when_segment_table_does_not_match`, `detect_media_type_upstream_without_top_level_type_detects_image_types`, `detect_media_type_upstream_without_top_level_type_detects_audio_types`, `detect_media_type_upstream_without_top_level_type_detects_video_types`, `detect_media_type_upstream_without_top_level_type_detects_document_types`, `detect_media_type_upstream_without_top_level_type_returns_undefined_for_no_signature`, `convert_image_model_file_to_data_uri_returns_url_as_is`, `convert_image_model_file_to_data_uri_upstream_returns_url_as_is_for_url_files`, `convert_image_model_file_to_data_uri_upstream_handles_urls_with_query_parameters`, `convert_image_model_file_to_data_uri_embeds_base64_data`, `convert_image_model_file_to_data_uri_upstream_returns_data_uri_for_base64_string_data`, `convert_image_model_file_to_data_uri_upstream_handles_different_media_types`
- `convert_image_model_file_to_data_uri_encodes_raw_bytes`, `convert_image_model_file_to_data_uri_upstream_converts_uint8_array_to_base64_data_uri`, `convert_image_model_file_to_data_uri_handles_empty_raw_bytes`, `convert_image_model_file_to_data_uri_upstream_handles_empty_uint8_array`, `convert_image_model_file_to_data_uri_upstream_handles_different_media_types_with_uint8_array`, `download_error_retains_status_and_cause_messages`, `validate_download_url_upstream_should_allow_https_urls`, `validate_download_url_upstream_should_allow_http_urls`, `validate_download_url_upstream_should_allow_public_ip_addresses`, `validate_download_url_upstream_should_allow_urls_with_ports`, `validate_download_url_upstream_should_allow_data_urls`, `validate_download_url_upstream_should_block_file_urls`, `validate_download_url_upstream_should_block_ftp_urls`, `validate_download_url_upstream_should_block_javascript_urls`, `validate_download_url_upstream_should_block_invalid_urls`, `validate_download_url_upstream_should_block_localhost`, `validate_download_url_upstream_should_block_localhost_with_port`, `validate_download_url_upstream_should_block_local_domains`, `validate_download_url_upstream_should_block_localhost_domains`, `validate_download_url_upstream_should_block_127_0_0_1_loopback`
- `validate_download_url_upstream_should_block_127_range`, `validate_download_url_upstream_should_block_10_range_private`, `validate_download_url_upstream_should_block_172_16_to_172_31_private`, `validate_download_url_upstream_should_allow_172_15_and_172_32_public`, `validate_download_url_upstream_should_block_192_168_range_private`, `validate_download_url_upstream_should_block_169_254_link_local_cloud_metadata`, `validate_download_url_upstream_should_block_0_0_0_0`, `validate_download_url_upstream_should_block_ipv6_loopback`, `validate_download_url_upstream_should_block_ipv6_unspecified`, `validate_download_url_upstream_should_block_fc00_7_unique_local`, `validate_download_url_upstream_should_block_fe80_10_link_local`, `validate_download_url_upstream_should_block_ipv4_mapped_127_0_0_1`, `validate_download_url_upstream_should_block_ipv4_mapped_10_0_0_1`, `validate_download_url_upstream_should_block_ipv4_mapped_169_254_169_254`, `validate_download_url_upstream_should_allow_ipv4_mapped_public_ip`, `extract_response_headers_preserves_response_header_entries`, `extract_response_headers_lets_later_entries_override_duplicates`, `response_handler_result_serializes_optional_metadata`, `response_handler_result_deserializes_minimal_result`, `event_source_response_handler_options_use_camel_case_json`
- `event_source_response_handler_options_deserialize_missing_body`, `parse_json_event_stream_parses_data_events_and_ignores_done`, `parse_json_event_stream_preserves_parse_and_validation_failures`, `create_event_source_response_handler_returns_results_and_headers`, `create_event_source_response_handler_returns_empty_body_error_for_missing_body`, `binary_response_handler_options_use_camel_case_json`, `binary_response_handler_options_deserialize_missing_body`, `create_binary_response_handler_returns_bytes_and_headers`, `create_binary_response_handler_preserves_empty_byte_body`, `create_binary_response_handler_returns_api_call_error_for_missing_body`, `json_error_response_handler_options_use_camel_case_json`, `create_json_error_response_handler_uses_parsed_error_data`, `create_json_error_response_handler_falls_back_for_empty_body`, `create_json_error_response_handler_falls_back_for_invalid_json`, `json_response_handler_options_use_camel_case_json`, `create_json_response_handler_returns_validated_value_raw_value_and_headers`, `create_json_response_handler_returns_api_call_error_for_invalid_json`, `create_json_response_handler_returns_api_call_error_for_validation_failure`, `status_code_error_response_handler_options_use_camel_case_json`, `create_status_code_error_response_handler_returns_api_call_error_result`
- `combine_headers_returns_empty_map_for_missing_groups`, `combine_headers_preserves_keys_and_combines_groups`, `combine_headers_allows_missing_values_to_override_present_values`, `normalize_headers_returns_empty_map_for_missing_input`, `normalize_headers_lowercases_keys_and_filters_missing_values`, `normalize_headers_preserves_empty_strings_and_allows_later_overrides`, `with_user_agent_suffix_creates_user_agent_header`, `with_user_agent_suffix_appends_to_existing_header_and_filters_empty_parts`, `with_user_agent_suffix_removes_missing_headers_before_appending`, `with_user_agent_suffix_sets_empty_user_agent_when_no_parts_exist`, `with_provider_utils_user_agent_adds_version_and_runtime_suffixes`, `with_provider_utils_user_agent_appends_to_existing_user_agent`, `provider_api_request_serializes_upstream_prepared_request_shape`, `provider_api_request_body_supports_binary_content`, `provider_api_request_body_supports_form_data_content`, `provider_api_response_serializes_upstream_response_metadata_shape`, `provider_api_response_body_supports_binary_content`, `provider_api_response_success_status_matches_fetch_ok_range`, `provider_api_response_builds_text_response_handler_options`, `provider_api_response_builds_binary_and_event_source_handler_options`
- `provider_api_response_decodes_binary_text_like_fetch_response_text`, `provider_api_response_handler_error_serializes_tagged_shape`, `handle_provider_api_response_returns_successful_handler_result`, `handle_provider_api_response_returns_failed_handler_api_error`, `handle_provider_api_response_wraps_non_api_handler_failures`, `handle_provider_api_response_passes_through_api_handler_failures`, `execute_provider_api_request_sends_prepared_request_and_handles_success`, `execute_provider_api_request_normalizes_transport_failures`, `execute_provider_api_request_preserves_abort_transport_failures`, `get_from_api_options_serialize_camel_case_request_metadata`, `get_from_api_prepares_request_and_handles_success`, `get_from_api_normalizes_transport_failures`, `post_json_to_api_options_serialize_camel_case_request_metadata`, `post_json_to_api_prepares_request_and_handles_success`, `post_json_to_api_normalizes_transport_failures`, `post_form_data_to_api_options_serialize_camel_case_request_metadata`, `post_form_data_to_api_prepares_request_and_handles_success`, `post_to_api_options_serialize_camel_case_request_metadata`, `post_to_api_prepares_request_and_handles_success`, `post_to_api_normalizes_transport_failures`
- `prepare_get_from_api_request_matches_upstream_request_setup`, `prepare_post_to_api_request_matches_upstream_binary_request_setup`, `prepare_post_json_to_api_request_matches_upstream_request_setup`, `prepare_post_json_to_api_request_allows_header_overrides`, `prepare_post_form_data_to_api_request_matches_upstream_request_setup`, `runtime_environment_serializes_camel_case_shape`, `runtime_environment_omits_unknown_indicators`, `get_runtime_environment_user_agent_upstream_returns_correct_user_agent_for_browsers`, `get_runtime_environment_user_agent_upstream_returns_correct_user_agent_for_test`, `get_runtime_environment_user_agent_upstream_returns_correct_user_agent_for_edge_runtime`, `get_runtime_environment_user_agent_upstream_returns_correct_user_agent_for_node_js`, `get_runtime_environment_user_agent_lowercases_navigator_user_agents`, `get_runtime_environment_user_agent_returns_unknown_for_missing_runtime`, `get_runtime_environment_user_agent_uses_upstream_probe_precedence`, `is_abort_error_matches_upstream_error_names`, `fetch_error_info_serializes_camel_case_shape`, `handled_fetch_error_serializes_tagged_api_call_result`, `handle_fetch_error_returns_abort_errors_unchanged`, `handle_fetch_error_wraps_node_fetch_failed_type_errors`, `handle_fetch_error_wraps_browser_failed_to_fetch_type_errors`
- `handle_fetch_error_leaves_fetch_failed_type_errors_without_cause_unchanged`, `handle_fetch_error_wraps_bun_network_errors`, `handle_fetch_error_returns_unknown_errors_unchanged`, `create_tool_name_mapping_maps_provider_defined_tools`, `create_tool_name_mapping_ignores_function_tools`, `create_tool_name_mapping_passes_through_unknown_provider_tool_ids`, `create_tool_name_mapping_handles_mixed_and_empty_tool_sets`, `tool_constructor_prepares_upstream_function_tool_shape`, `tool_dynamic_description_uses_context_and_sandbox_when_prepared`, `tool_metadata_is_retained_but_not_sent_to_provider`, `tool_title_is_retained_but_not_sent_to_provider`, `provider_defined_tool_factory_round_trips_upstream_config_shape`, `provider_defined_tool_factory_creates_provider_tool`, `provider_executed_tool_factory_round_trips_upstream_config_and_creates_tool`, `provider_defined_tool_factory_omits_missing_output_schema`, `provider_executed_tool_factory_omits_missing_deferred_results_support`, `tool_execution_options_serialize_as_camel_case`, `tool_model_output_options_round_trip_upstream_shape`, `tool_model_output_callback_receives_tool_result_context`, `sandbox_command_contracts_round_trip_upstream_shape`
- `tool_execution_options_carry_runtime_sandbox_without_serializing_it`, `is_executable_tool_matches_upstream_execute_tool_behavior`, `execute_tool_output_round_trips_upstream_shape`, `execute_tool_runs_executor_and_wraps_final_output`, `execute_tool_reports_non_executable_tools`, `tool_executor_returns_json_results`, `tool_execution_error_retains_message`, `load_api_key_returns_explicit_value_without_reading_environment`, `load_api_key_reads_environment_when_value_is_missing`, `load_api_key_reports_upstream_missing_message`, `load_api_key_reports_non_unicode_environment_values_as_non_strings`, `load_setting_returns_explicit_value_without_reading_environment`, `load_setting_reads_environment_when_value_is_missing`, `load_setting_reports_upstream_missing_message`, `load_setting_reports_non_unicode_environment_values_as_non_strings`, `load_optional_setting_prefers_explicit_value`, `load_optional_setting_reads_environment_when_value_is_missing`, `load_optional_setting_returns_none_for_missing_or_non_unicode_environment_values`, `media_type_to_extension_maps_audio_media_types`, `media_type_to_extension_lowercases_subtypes_and_handles_invalid_values`
- `without_trailing_slash_removes_one_trailing_slash`, `without_trailing_slash_preserves_values_without_trailing_slash`, `without_trailing_slash_preserves_missing_url`, `without_trailing_slash_only_removes_the_final_slash`, `resolve_provider_reference_returns_provider_specific_identifier`, `resolve_provider_reference_reports_missing_provider_context`, `resolve_provider_reference_rejects_empty_references`

## Provider Package Helper Inventory

The remaining exact-name helper mappings from `crates/ai-sdk-provider/src` are listed below.

### embedding_model.rs
- `call_options_serializes_upstream_shape_with_headers_and_provider_options`, `call_options_deserializes_minimal_values_and_omits_missing_options`, `result_serializes_embeddings_usage_response_metadata_and_warnings`, `result_deserializes_empty_warnings_and_omits_optional_fields`

### file_data.rs
- `file_data_serializes_base64_data_variant`, `file_data_serializes_url_variant_as_string`, `file_data_round_trips_provider_reference_variant`, `provider_reference_rejects_reserved_type_key`, `missing_provider_reference_error_matches_upstream_context`, `missing_provider_reference_error_accepts_upstream_custom_message`

### files.rs
- `upload_file_call_options_serializes_data_filename_and_provider_options`, `upload_file_call_options_deserializes_text_data_and_omits_optional_fields`, `upload_file_result_serializes_reference_metadata_and_warnings`, `upload_file_result_deserializes_empty_warnings_and_omits_optional_fields`

### headers.rs
- `headers_serialize_as_string_map`

### image_model.rs
- `call_options_serializes_upstream_shape_with_files_mask_and_provider_options`, `call_options_deserializes_minimal_required_fields_with_empty_provider_options`, `image_model_trait_exposes_upstream_v4_identity_capability_and_generate_boundary`, `result_serializes_images_response_usage_metadata_and_warnings`, `result_deserializes_empty_warnings_and_omits_optional_fields`, `image_response_metadata_matches_upstream_shape`, `no_image_generated_error_matches_upstream_default_message`, `no_image_generated_error_retains_response_metadata`

### json.rs
- `aliases_cover_json_object_array_and_value_shapes`, `json_value_guard_accepts_all_serde_json_values`, `json_array_guard_matches_only_arrays`, `json_object_guard_matches_only_objects`, `non_null_json_value_round_trips_non_null_values`, `non_null_json_value_rejects_null_values`

### language_model.rs
- `language_model_trait_exposes_upstream_v4_boundaries`, `supported_urls_serializes_as_media_type_pattern_map`, `finish_reason_uses_upstream_kebab_case_names`, `usage_uses_upstream_camel_case_token_fields`, `usage_deserializes_when_optional_counts_are_missing`, `response_metadata_uses_upstream_camel_case_and_rfc3339_timestamp`, `response_metadata_deserializes_when_optional_fields_are_missing`, `text_part_serializes_upstream_shape_with_provider_metadata`, `text_part_deserializes_and_omits_missing_provider_metadata`, `reasoning_part_serializes_upstream_shape_with_provider_metadata`
- `reasoning_part_rejects_other_content_types`, `custom_content_serializes_upstream_shape_with_provider_metadata`, `custom_content_deserializes_and_omits_missing_provider_metadata`, `file_part_serializes_upstream_data_shape_with_provider_metadata`, `reasoning_file_part_deserializes_url_data_and_omits_missing_provider_metadata`, `language_model_file_data_rejects_prompt_only_file_variants`, `tool_approval_request_serializes_upstream_shape_with_provider_metadata`, `tool_approval_request_deserializes_and_omits_missing_provider_metadata`, `tool_approval_request_rejects_other_content_types`, `url_source_serializes_upstream_shape_with_optional_title_and_metadata`
- `document_source_deserializes_and_omits_missing_optional_fields`, `source_rejects_other_content_types`, `tool_call_serializes_upstream_shape_with_optional_flags_and_metadata`, `tool_call_deserializes_and_omits_missing_optional_fields`, `tool_call_rejects_other_content_types`, `tool_result_serializes_upstream_shape_with_optional_flags_and_metadata`, `tool_result_deserializes_and_omits_missing_optional_fields`, `tool_result_rejects_null_results`, `tool_result_rejects_other_content_types`, `content_union_serializes_upstream_generated_content_shapes`
- `content_union_deserializes_tool_result_variant`, `content_union_rejects_unknown_content_types`, `request_metadata_serializes_high_level_messages_and_body`, `response_metadata_serializes_high_level_messages_and_provider_data`, `generate_result_serializes_upstream_shape_with_request_response_and_warnings`, `generate_result_deserializes_empty_warnings_and_omits_optional_fields`, `stream_result_serializes_upstream_metadata_envelope`, `stream_result_deserializes_minimal_stream_and_omits_metadata`, `stream_block_parts_serialize_upstream_shapes`, `stream_lifecycle_parts_serialize_upstream_shapes`
- `stream_part_union_deserializes_generated_content_and_finish_variants`, `stream_start_requires_warnings_array`, `function_tool_serializes_upstream_shape_with_schema_examples_and_options`, `function_tool_deserializes_and_omits_missing_optional_fields`, `provider_tool_deserializes_record_args`, `tool_union_deserializes_provider_tool_variant`, `tool_union_rejects_unknown_tool_types`, `tool_choice_serializes_upstream_tagged_shapes`, `tool_choice_deserializes_specific_tool_selection`, `call_options_serializes_upstream_shape_with_generation_controls`
- `call_options_deserializes_minimal_prompt_and_omits_missing_options`, `call_options_deserializes_text_format_and_provider_default_reasoning`, `prompt_serializes_system_and_user_messages_with_provider_options`, `assistant_message_deserializes_reasoning_custom_tool_call_and_approval_request_parts`, `assistant_message_serializes_tool_approval_request_part`, `tool_result_output_serializes_tagged_output_variants_with_provider_options`, `prompt_message_rejects_unknown_roles`

### provider.rs
- `model_type_serializes_as_upstream_model_type_strings`, `model_type_deserializes_from_upstream_model_type_string`, `specification_version_serializes_as_upstream_v4_literal`, `specification_version_deserializes_from_upstream_v4_literal`, `provider_resolves_required_v4_model_interfaces`, `provider_required_model_lookup_reports_missing_model_type`, `provider_extension_traits_resolve_optional_v4_interfaces`, `no_such_model_error_matches_upstream_context`, `no_such_model_error_accepts_upstream_custom_message`, `unsupported_functionality_error_matches_upstream_default_message`
- `unsupported_functionality_error_accepts_provider_specific_message`, `load_api_key_error_uses_upstream_message_contract`, `load_setting_error_uses_upstream_message_contract`, `invalid_argument_error_uses_upstream_message_contract`, `invalid_prompt_error_uses_upstream_message_prefix_and_retains_prompt`, `type_validation_context_serializes_as_upstream_camel_case_shape`, `type_validation_error_matches_upstream_message_without_context`, `type_validation_error_matches_upstream_context_prefixes`, `type_validation_error_accepts_display_causes`, `api_call_error_serializes_upstream_shape_and_retains_context`
- `api_call_error_uses_upstream_default_retry_rule`, `api_call_error_allows_retry_override_and_explicit_null_data`, `json_parse_error_matches_upstream_message_contract`, `json_parse_error_accepts_display_causes`, `empty_response_body_error_matches_upstream_default_message`, `empty_response_body_error_accepts_provider_specific_message`, `no_content_generated_error_matches_upstream_default_message`, `no_content_generated_error_accepts_provider_specific_message`, `invalid_response_data_error_matches_upstream_default_message`, `invalid_response_data_error_accepts_provider_specific_message`
- `too_many_embedding_values_error_matches_upstream_message_and_context`, `provider_options_serialize_as_nested_provider_objects`

### reranking_model.rs
- `call_options_serializes_text_documents_with_top_n_and_provider_options`, `call_options_deserializes_object_documents_and_omits_missing_options`, `result_serializes_ranking_response_metadata_and_warnings`, `result_deserializes_without_optional_warnings_and_omits_empty_warnings`

### skills.rs
- `upload_skill_call_options_serializes_files_title_and_provider_options`, `upload_skill_call_options_deserializes_minimal_files_and_omits_optional_fields`, `upload_skill_result_serializes_reference_metadata_and_warnings`, `upload_skill_result_deserializes_empty_warnings_and_omits_optional_fields`

### speech_model.rs
- `call_options_serializes_upstream_shape_with_speech_settings_and_headers`, `call_options_deserializes_minimal_text_and_omits_optional_fields`, `result_serializes_audio_response_metadata_and_warnings`, `result_deserializes_raw_audio_bytes_and_empty_warnings`, `result_requires_warnings_and_response_metadata`, `response_metadata_serializes_upstream_shape_with_body`, `response_metadata_converts_from_provider_response_without_dropping_body`, `no_speech_generated_error_matches_upstream_message`, `no_speech_generated_error_retains_response_metadata`

### transcription_model.rs
- `call_options_serializes_upstream_shape_with_audio_options_and_headers`, `call_options_deserializes_raw_audio_bytes_and_omits_optional_fields`, `no_transcript_generated_error_matches_upstream_message_and_retains_responses`, `response_metadata_drops_provider_response_body`, `result_serializes_upstream_shape_with_segments_response_and_metadata`, `result_deserializes_empty_warnings_and_omits_optional_fields`, `result_requires_warnings_and_response_metadata`

### video_model.rs
- `call_options_serializes_upstream_shape_with_image_and_provider_options`, `call_options_deserializes_minimal_required_fields_with_empty_provider_options`, `experimental_video_aliases_share_upstream_v4_contracts`, `result_serializes_videos_response_metadata_and_warnings`, `result_deserializes_empty_warnings_and_omits_optional_fields`, `response_metadata_serializes_upstream_shape_with_provider_metadata`, `response_metadata_deserializes_minimal_shape_and_can_be_built_from_provider_response`, `no_video_generated_error_matches_upstream_default_message`, `no_video_generated_error_retains_response_metadata_and_custom_message`

### warning.rs
- `warning_serializes_optional_details_for_unsupported_features`, `warning_omits_missing_optional_details`, `warning_deserializes_deprecated_and_other_variants`

## Open Responses Exact-Name Inventory

- `open_responses_stream_parts_from_events`

## Gateway Package Helper Inventory

The remaining exact-name helper mappings from `crates/ai-sdk-gateway/src` are listed below.

### gateway.rs
- `gateway_embedding_model_passes_abort_signal_to_provider_api_request`, `gateway_image_model_passes_abort_signal_to_provider_api_request`, `gateway_reranking_model_passes_abort_signal_to_provider_api_request`, `gateway_video_model_passes_abort_signal_to_provider_api_request`, `gateway_language_model_sets_basic_properties`, `gateway_language_model_passes_headers_correctly`, `gateway_language_model_extracts_text_response`, `gateway_language_model_extracts_usage_information`, `gateway_language_model_removes_abort_signal_from_request_body`, `gateway_language_model_passes_abort_signal_to_fetch_when_provided`
- `gateway_language_model_does_not_pass_abort_signal_to_fetch_when_not_provided`, `gateway_language_model_includes_o11y_headers_in_request`, `gateway_language_model_converts_api_call_errors_to_gateway_errors`, `gateway_language_model_handles_malformed_error_responses`, `gateway_language_model_handles_rate_limit_errors`, `gateway_language_model_handles_invalid_request_errors`, `gateway_language_model_does_not_modify_prompt_without_image_parts_for_generate`, `gateway_language_model_encodes_uint8_array_image_part_to_inline_base64_data_with_default_mime_type_for_generate`, `gateway_language_model_encodes_uint8_array_image_part_to_inline_base64_data_with_specified_mime_type_for_generate`, `gateway_language_model_does_not_modify_image_part_with_url_for_generate`
- `gateway_language_model_handles_mixed_content_types_correctly_for_generate`, `gateway_language_model_handles_various_error_types_with_proper_conversion`, `gateway_language_model_includes_actual_response_body_when_api_call_error_has_no_data`, `gateway_language_model_uses_raw_response_body_when_json_parsing_fails`, `gateway_language_model_streams_text_deltas`, `gateway_language_model_passes_streaming_headers`, `gateway_language_model_removes_abort_signal_from_streaming_request_body`, `gateway_language_model_passes_abort_signal_to_fetch_when_provided_for_streaming`, `gateway_language_model_does_not_pass_abort_signal_to_fetch_when_not_provided_for_streaming`, `gateway_language_model_includes_o11y_headers_in_streaming_request`
- `gateway_language_model_converts_api_call_errors_to_gateway_errors_in_streaming`, `gateway_language_model_handles_authentication_errors_in_streaming`, `gateway_language_model_handles_invalid_request_errors_in_streaming`, `gateway_language_model_handles_malformed_error_responses_in_streaming`, `gateway_language_model_does_not_modify_prompt_without_image_parts_for_streaming`, `gateway_language_model_encodes_uint8_array_image_part_to_inline_base64_data_with_default_mime_type_for_streaming`, `gateway_language_model_encodes_uint8_array_image_part_to_inline_base64_data_with_specified_mime_type_for_streaming`, `gateway_language_model_does_not_modify_image_part_with_url_for_streaming`, `gateway_language_model_handles_mixed_content_types_correctly_for_streaming`, `gateway_language_model_filters_raw_chunks_based_on_include_raw_chunks_option`
- `gateway_language_model_includes_raw_chunks_when_include_raw_chunks_is_true`, `gateway_language_model_converts_timestamp_strings_to_offset_date_time_in_response_metadata_chunks`, `gateway_language_model_preserves_response_metadata_without_timestamp`, `gateway_language_model_handles_null_response_metadata_timestamp_gracefully`, `gateway_language_model_ignores_extra_timestamp_fields_on_non_metadata_stream_parts`, `gateway_language_model_passes_provider_routing_order_for_generate`, `gateway_language_model_passes_single_provider_in_order_array`, `gateway_language_model_works_without_provider_options`, `gateway_language_model_passes_provider_routing_order_for_stream`, `gateway_language_model_validates_provider_options_against_schema`
- `gateway_language_model_passes_provider_timeouts_for_generate`, `gateway_language_model_passes_provider_timeouts_for_stream`, `gateway_language_model_passes_zero_data_retention_option`, `gateway_language_model_passes_disallow_prompt_training_option`, `gateway_language_model_passes_hipaa_compliant_option`, `gateway_language_model_passes_both_zero_data_retention_and_hipaa_compliant_options`, `gateway_language_model_passes_quota_entity_id_option`, `gateway_language_model_passes_quota_entity_id_with_other_options`, `gateway_language_model_maps_generate_transport_failure_to_gateway_response_error_metadata`, `gateway_language_model_maps_stream_transport_failure_to_gateway_response_error_part`
- `gateway_language_model_preserves_error_cause_chain_as_gateway_metadata`, `gateway_function_uses_default_gateway_provider`

### gateway_error.rs
- `gateway_internal_server_error_matches_default_and_variant_checks`, `gateway_errors_append_generation_id_to_message`, `gateway_authentication_contextual_messages_match_auth_source`, `create_gateway_error_from_response_handles_invalid_response_shapes`, `create_gateway_error_from_api_call_maps_data_and_status`, `create_gateway_error_from_api_call_preserves_message_without_error_data`, `as_gateway_error_maps_handled_fetch_errors`, `gateway_headers_from_auth_method_uses_upstream_header_name`
