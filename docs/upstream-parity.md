...
| Language model middleware: wrap/default settings | verified | `src/language_model_middleware.rs` | `wrap_language_model_*`; `default_settings_middleware_*` tests | Mirrors upstream v4 model-level hooks. |
...
2. Continue `streamObject` parity with repair callbacks, telemetry,
   ...
   edge cases.
3. Port add-tool-input-examples, extract-json, extract-reasoning, and
