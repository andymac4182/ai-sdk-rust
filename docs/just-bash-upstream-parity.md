...
structured-data/query rows, JBC-38 closes 597 bounded small-command rows, and
JBC-39 closes optional language boundary rows while documenting host-runtime/package
runtime exceptions. After regeneration the combined Just Bash ledger is `5,558`
verified / `3,297` pending / `1,081` JS-only, with `3,297` strict gate gaps. These slices
