...
runtime exceptions, JBC-43 closes 25 exact grep rows, and JBC-44 closes the
remaining query-engine safe-object helper rows. After regeneration the combined
Just Bash ledger is `5,668` verified / `3,180` pending / `1,088` JS-only, with
`3,180` strict gate gaps. These slices
