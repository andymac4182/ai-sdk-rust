...
additional JS-only runtime bridge exceptions, and JBC-36 closes the owned
filesystem/core runtime/CLI/session child slice. After regeneration the combined
Just Bash ledger is `4,472` verified / `4,680` pending / `784` JS-only, with
`4,680` strict gate gaps. These slices
