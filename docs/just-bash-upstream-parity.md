...
JBC-25 closes 113 additional exact AWK rows, and JBC-26 closes 405 additional
filesystem/path case rows plus three source rows. After regeneration the
combined Just Bash ledger is `3,375` verified / `6,402` pending / `159`
JS-only, with `6,413` strict gate gaps. These slices do not
...
JBC-10, JBC-11, JBC-13, JBC-16, JBC-22, JBC-23, JBC-24, and JBC-25 command-family row mappings. Seed smoke
...
per-domain tables. JBC-26 filesystem/path closure is tracked in the filesystem
table below.
