...
AI-02G marks only the TogetherAI package row `verified`. The other AI-02
provider rows stay `in-progress` until their current portable upstream cases are
either covered by named Rust tests or moved from the exception column below into
explicit exceptions by their owning closure lanes.
...
AI-02G adds the first TogetherAI ignored live proof because deterministic
TogetherAI request, validation, metadata, and exception mapping is closed above.
The other AI-02 providers still need their owning closure lanes to remove named
deterministic gaps before adding live tests.
