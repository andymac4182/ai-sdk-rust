...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,types}.rs::tests` (17 + 13 + 34 colocated tests; first two are 1:1 ports of upstream `*.test.ts`, the `types` tests are additive Rust-only serde coverage because upstream `types.ts` has no test file). | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, and six layers of `types` (standalone + emoji + Lock + concurrency/Author/UserInfo + DurationString/TranscriptsConfig + ChannelInfo/ListThreadsOptions) ported. 2/23 upstream test files mapped, 64 Rust tests total. Phase-1. |
