...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,types}.rs::tests` (17 + 13 + 5 + 33 + 16 + 73 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122 cases. cards ports 16 of 28 cases (leaf-element builders only). types is additive Rust-only serde coverage. | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, `chat_singleton`, `markdown` (markdown-rs scaffold + AST builders + parse_markdown + 14 type guards + get_node_children/value + to_plain_text + walk_ast + table_to_ascii), **`cards` (leaf elements + builders for Button/LinkButton/Text/Image/Divider/Field/Fields/Table/CardLink)** modules, plus fourteen layers of `types` covering 54 upstream interfaces/enums + FormattedContent placeholder + placeholder Adapter/StateAdapter traits. 5/23 upstream test files partially mapped, 157 Rust tests total. Phase-1. |
