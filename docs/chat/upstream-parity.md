...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,modals,types}.rs::tests` (17 + 13 + 5 + 33 + 24 + 13 + 73 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122. cards ports 24 of 28 (full data shape). modals ports 13 of 29 (full data shape - leaf elements + ModalChild union + ModalElement + Modal builder + is_modal_element + filter_modal_children + VALID_MODAL_CHILD_TYPES). types is additive Rust-only serde coverage. | 54 src files, 23 test files upstream. Crate skeleton + 6 source modules. cards + modals data shapes are now BOTH complete via the slice 5(d) discriminated-union recipe (per-struct kind + #[serde(untagged)] + From<T> impls). 6/23 upstream test files partially mapped, 178 Rust tests total. Phase-1. |
| `packages/adapter-shared` (`@chat-sdk/adapter-shared`) | shared adapter utilities | in-progress | `crates/chat-sdk-adapter-shared` | `crates/chat-sdk-adapter-shared/src/{errors,card_utils,buffer_utils}.rs::tests` (24 + 7 + 7 colocated tests). errors is 1:1 with upstream. card_utils ports 7 of 39 cases (PlatformName + button_style_mapping + map_button_style + escape_table_cell + render_gfm_table; createEmojiConverter and cardToFallbackText deferred until chat::emoji and JSX-side fallback helpers land). buffer_utils ports 7 of 16 cases (to_buffer + to_buffer_sync + buffer_to_data_uri); the 9 remaining upstream cases assert JS-runtime-type plumbing for Buffer/ArrayBuffer/Blob discrimination that collapses to Vec<u8> in Rust and is documented js-only-adjacent in the module. | 10 src files, 4 test files upstream. Crate skeleton + `errors` + `card_utils` + `buffer_utils` modules. 3/4 test files partially mapped, 38 Rust tests total. Remaining: `adapter-utils`, plus the deferred card_utils helpers. Phase-1. Name uses upstream-mirroring "shared" exception (already in `scripts/check-naming-conventions.sh`). |
