...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,modals,types}.rs::tests` (17 + 13 + 5 + 33 + 24 + 13 + 76 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122. cards ports 24 of 28 (full data shape). modals ports 13 of 29 (full data shape - leaf elements + ModalChild union + ModalElement + Modal builder + is_modal_element + filter_modal_children + VALID_MODAL_CHILD_TYPES). types is additive Rust-only serde coverage + AdapterPostableMessage untagged enum union (PostableRaw/PostableMarkdown/PostableAst/PostableCard/CardElement/String) with From<T> impls. | 54 src files, 23 test files upstream. Crate skeleton + 6 source modules. cards + modals data shapes are now BOTH complete via the slice 5(d) discriminated-union recipe (per-struct kind + #[serde(untagged)] + From<T> impls). 6/23 upstream test files partially mapped, 181 Rust tests total. Phase-1. |
