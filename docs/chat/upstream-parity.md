...
| `packages/adapter-shared` (`@chat-sdk/adapter-shared`) | shared adapter utilities | in-progress | `crates/chat-sdk-adapter-shared` | `crates/chat-sdk-adapter-shared/src/{errors,card_utils}.rs::tests` (24 + 7 colocated tests). errors is 1:1 with upstream. card_utils ports 7 of 39 cases (PlatformName + button_style_mapping + map_button_style + escape_table_cell + render_gfm_table; createEmojiConverter and cardToFallbackText deferred until chat::emoji and JSX-side fallback helpers land). | 10 src files, 4 test files upstream. Crate skeleton + `errors` + `card_utils` modules. 2/4 test files partially mapped, 31 Rust tests total. Remaining: `adapter-utils`, `buffer-utils`, plus the deferred card_utils helpers. Phase-1. Name uses upstream-mirroring "shared" exception (already in `scripts/check-naming-conventions.sh`). |
