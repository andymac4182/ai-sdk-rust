...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,types}.rs::tests` (17 + 13 + 5 + 43 colocated tests; the first three are 1:1 ports of upstream `*.test.ts`, the `types` tests are additive Rust-only serde coverage because upstream `types.ts` has no test file). | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, `chat_singleton` modules, plus seven layers of `types` (standalone + emoji + Lock + concurrency/Author/UserInfo + DurationString/TranscriptsConfig + ChannelInfo/ListThreadsOptions + StreamChunk/StreamOptions + placeholder Adapter/StateAdapter traits). 3/23 upstream test files mapped, 78 Rust tests total. Phase-1. |
