...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,modals,emoji,types}.rs::tests` (17 + 13 + 5 + 33 + 24 + 13 + 42 + 76 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122. cards ports 24 of 28 (full data shape). modals ports 13 of 29 (full data shape - leaf elements + ModalChild union + ModalElement + Modal builder + is_modal_element + filter_modal_children + VALID_MODAL_CHILD_TYPES). emoji ports 42 of 42 cases (EmojiResolver from_slack/from_gchat/from_teams/to_slack/to_gchat/to_discord/matches/extend + DEFAULT_EMOJI_MAP + DEFAULT_EMOJI_RESOLVER + WELL_KNOWN_EMOJI + get_emoji singleton via Arc + EmojiHelper + convert_emoji_placeholders + create_emoji). Also fixed EmojiValue::placeholder() to return upstream `{{emoji:name}}` format (was incorrectly `:name:`). types is additive Rust-only serde coverage + AdapterPostableMessage untagged enum union with From<T> impls. | 54 src files, 23 test files upstream. Crate skeleton + 7 source modules. 7/23 upstream test files partially mapped (emoji 1:1 complete), 223 Rust tests total. Phase-1. |
| `packages/adapter-shared` (`@chat-sdk/adapter-shared`) | shared adapter utilities | in-progress | `crates/chat-sdk-adapter-shared` | `crates/chat-sdk-adapter-shared/src/{errors,card_utils,buffer_utils,adapter_utils,crypto}.rs::tests` (24 + 7 + 7 + 21 + 15 colocated tests). errors is 1:1 with upstream. card_utils ports 7 of 39 cases (PlatformName + button_style_mapping + map_button_style + escape_table_cell + render_gfm_table; createEmojiConverter and cardToFallbackText deferred until chat::emoji and JSX-side fallback helpers land). buffer_utils ports 7 of 16 cases (to_buffer + to_buffer_sync + buffer_to_data_uri); the 9 remaining upstream cases assert JS-runtime-type plumbing for Buffer/ArrayBuffer/Blob discrimination that collapses to Vec<u8> in Rust and is documented js-only-adjacent in the module. adapter_utils ports 17 of 25 cases (extract_card + extract_files + extract_postable_attachments over the typed AdapterPostableMessage enum); the 8 remaining cases assert null/undefined/Blob/ArrayBuffer runtime-type plumbing that the Rust type system rejects at compile time. crypto ports encrypt_token + decrypt_token + is_encrypted_token_data + decode_key + EncryptedTokenData (AES-256-GCM via aes-gcm crate, 12-byte random IV, hex/base64 key decode); upstream has no `crypto.test.ts` so the 15 colocated tests are additive Rust-side roundtrip/decode/shape coverage. | 6 src files (adapter-utils, buffer-utils, card-utils, crypto, errors, index), 4 test files upstream. Crate skeleton + 5 source modules - only `index.ts` re-exports remain, mapped naturally onto lib.rs `pub mod` declarations. 4/4 upstream test files partially mapped + crypto additive coverage, 74 Rust tests total. Remaining: deferred card_utils helpers blocked on chat::emoji + JSX fallback layer. Phase-1. Name uses upstream-mirroring "shared" exception (already in `scripts/check-naming-conventions.sh`). |
