...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,modals,types}.rs::tests` (17 + 13 + 5 + 33 + 16 + 7 + 73 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122. cards ports 16 of 28 (leaf elements). modals ports 7 of 29 (Select/RadioSelect/TextInput/ExternalSelect leaf elements - Modal container + ModalChild union deferred). types is additive Rust-only serde coverage. | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, `chat_singleton`, `markdown`, `cards`, **`modals` (TextInputElement/SelectElement/SelectOptionElement/ExternalSelectElement/RadioSelectElement leaf data shapes + builders text_input/select/select_option/external_select/radio_select)** modules, plus fourteen layers of `types`. 6/23 upstream test files partially mapped, 164 Rust tests total. The cards ActionsElement and SectionElement are now unblocked (their SelectElement+RadioSelectElement children-type deps are landed). Phase-1. |
