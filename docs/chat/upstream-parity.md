...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,types}.rs::tests` (17 + 13 + 5 + 20 + 73 colocated tests; the first three are 1:1 ports of upstream `*.test.ts`, the `markdown` tests are a growing 1:1 subset of `markdown.test.ts`, the `types` tests are additive Rust-only serde coverage because upstream `types.ts` has no test file). | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, `chat_singleton`, **`markdown` (markdown-rs 1.0 scaffold + AST builders + parse_markdown + 14 is_*_node guards + get_node_children + get_node_value)** modules, plus fourteen layers of `types` covering 54 upstream interfaces/enums + FormattedContent placeholder + placeholder Adapter/StateAdapter traits. 4/23 upstream test files partially mapped (errors/logger/chat-singleton full, markdown subset = 20 of 122 cases), 128 Rust tests total. Phase-1. |
