...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,chat_singleton,markdown,cards,modals,types}.rs::tests` (17 + 13 + 5 + 33 + 19 + 7 + 73 colocated tests). errors/logger/chat-singleton are 1:1 with upstream `*.test.ts`. markdown ports 33 of 122. cards ports 19 of 28 (leaf elements + ActionsElement + ActionsChild union; Section/CardChild/CardElement/Card builder/isCardElement deferred). modals ports 7 of 29 (leaf interactive elements; Modal container + ModalChild union deferred). types is additive Rust-only serde coverage. | 54 src files, 23 test files upstream. Crate skeleton + 6 source modules. cards now covers ActionsElement and ActionsChild (#[serde(untagged)] over Button/LinkButton/Select/RadioSelect with From<...> impls for ergonomic construction). 6/23 upstream test files partially mapped, 167 Rust tests total. SectionElement + CardChild + CardElement are next. Phase-1. |
