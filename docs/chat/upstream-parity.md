...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger,types}.rs::tests` (17 + 13 + 30 colocated tests; first two are 1:1 ports of upstream `*.test.ts`, the `types` tests are additive Rust-only serde coverage because upstream `types.ts` has no test file). | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger`, and five layers of `types` (standalone + emoji + Lock + concurrency/Author/UserInfo + DurationString/TranscriptsConfig) ported. 2/23 upstream test files mapped, 60 Rust tests total. Phase-1. |
| `packages/adapter-shared` (`@chat-sdk/adapter-shared`) | shared adapter utilities | in-progress | `crates/chat-sdk-adapter-shared` | `crates/chat-sdk-adapter-shared/src/errors.rs::tests` (24 colocated 1:1 ports of `packages/adapter-shared/src/errors.test.ts`) | 10 src files, 4 test files upstream. Crate skeleton + `errors` module ported (1/4 test files, 24 cases). Remaining upstream modules: adapter-utils, buffer-utils, card-utils. Phase-1. Name uses upstream-mirroring "shared" exception (already in `scripts/check-naming-conventions.sh`). |
