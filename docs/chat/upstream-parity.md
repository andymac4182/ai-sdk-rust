...
| --- | --- | --- | --- | --- | --- |
| `packages/chat` (`@chat-sdk/chat`) | core SDK package | in-progress | `crates/chat-sdk-chat` | `crates/chat-sdk-chat/src/{errors,logger}.rs::tests` (17 + 13 colocated ports of `packages/chat/src/{errors,logger}.test.ts`) | 54 src files, 23 test files upstream. Crate skeleton + `errors`, `logger` modules ported (2/23 test files, 30 cases) with idiomatic Rust `#[cfg(test)] mod tests` colocated in each source file. Phase-1. |
