...
| `packages/adapter-slack` (`@chat-sdk/adapter-slack`) | adapter package | in-progress | `crates/chat-sdk-adapter-slack` | `crates/chat-sdk-adapter-slack/src/lib.rs::tests` (14 colocated tests). | 24 src files, 11 test files. Slack adapter. Phase-2. Slice 139 scaffolds the crate: `SlackAdapter` impl-ing `chat_sdk_chat::types::Adapter` with `name = "slack"`, `SlackAdapterOptions` (bot_token + signing_secret + optional app_token for Socket Mode + API base), and `encode_thread_id`/`decode_thread_id`/`is_slack_thread_id` helpers for the upstream `slack:<channel_id>:<thread_ts>` wire format. `DecodedSlackThreadId::is_dm()` / `is_group()` predicates detect channel-id prefixes (`D...` / `G...`). Web API + Socket Mode I/O deferred until async HTTP client lands. |
| `packages/adapter-teams` (`@chat-sdk/adapter-teams`) | adapter package | in-progress | `crates/chat-sdk-adapter-teams` | `crates/chat-sdk-adapter-teams/src/lib.rs::tests` (12 colocated tests). | 16 src files, 6 test files. Microsoft Teams adapter. Phase-2. Slice 138 scaffolds the crate: `TeamsAdapter` impl-ing `chat_sdk_chat::types::Adapter` with `name = "teams"`, `TeamsAdapterOptions` (app_id + app_password + tenant_id + API base), and `encode_thread_id`/`decode_thread_id`/`is_teams_thread_id` helpers for the upstream `teams:<conversation_id>:<message_id>` wire format. Notable: decode_thread_id splits on the LAST colon so Bot Framework's `19:abc;tenant=def` conversation-id encoding survives intact. HTTP I/O + Bot Framework auth deferred until async HTTP client lands. |
...
| `packages/state-redis` (`@chat-sdk/state-redis`) | state backend (Redis) | in-progress | `crates/chat-sdk-state-redis` | `crates/chat-sdk-state-redis/src/lib.rs::tests` (11 colocated tests). | 2 src files, 1 test file. Production Redis state. Phase-3. Slice 140 scaffolds the crate: `RedisStateAdapter` impl-ing `chat_sdk_chat::types::StateAdapter` with all 5 required methods returning `StateAdapterError::NotConnected` (placeholder until `redis-rs` + `bb8-redis` are wired in per the port-chat-sdk.md "Phase 2/3 prep" runtime decision). `RedisStateAdapterOptions` (Redis URL + key prefix) + `prefixed_key()` namespace helper. Lock primitive + pub/sub deferred. |
