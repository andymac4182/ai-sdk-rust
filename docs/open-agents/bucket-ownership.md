...
| OP-01 Open Plugin Spec manifest gate | `open-agents-core::plugin`, `docs/open-agents/open-plugin-spec.md`, `scripts/open-plugin-spec-gate.mjs` | `.plugin/plugin.json` parsing, metadata, `skills`, `mcpServers`, path safety, name validation, diagnostics, and conformance tracker | Open Plugin Spec v1.0.0 | `cargo test -p open-agents-core`; `node scripts/open-plugin-spec-gate.mjs --check`; `scripts/master-parity-gate.sh` |
| OP-02 Open Plugin skill discovery | `ai-sdk-rust::skills` | default and manifest-declared skill directory discovery, namespaced skill metadata, `SKILL.md` loading handoff | OP-01 | Tests for default `skills/`, custom paths, missing directories, and namespaced skill IDs |
| OP-03 Open Plugin MCP integration | `open-agents-core::open_plugin` | `.mcp.json`, path config, inline `mcpServers`, conflict diagnostics, non-fatal config failures, `${PLUGIN_ROOT}` expansion | OP-01 | Tests for MCP config discovery, conflict determinism, invalid-config continuation, and runtime placeholder expansion |
| OP-04 Open Plugin host conformance closure | `open-agents-service` | service-level plugin load path, partial-support diagnostics, at least one end-to-end core component type | OP-02, OP-03 | Service tests proving minimum host conformance without requiring live credentials |
