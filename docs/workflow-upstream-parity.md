...
| `packages/world` (`@workflow/world`) | `5.0.0-beta.5` | portable Rust runtime | in-progress | `crates/workflow-world` | Source: `src/index.ts`, `src/interfaces.ts`, `src/runs.ts`, `src/steps.ts`, `src/events.ts`, `src/hooks.ts`, `src/queue.ts`, `src/serialization.ts`, `src/spec-version.ts`, `src/waits.ts`. Tests: `src/attributes.test.ts`. | Shared queue/storage/pagination contracts now exist to support `world-postgres` and `world-vercel`; the package-owned `attributes.test.ts` rows remain not-started. |
...
| `workflow-world` | `packages/world` (`@workflow/world`) | in-progress | Shared queue, storage, event, pagination, and payload contracts used by package-owned world implementations; package-owned rows are still pending. |
...
4. Continue expanding `workflow-world` package-owned rows (attributes and full
   trait coverage) and then use those contracts to finish `workflow-world-local`.
