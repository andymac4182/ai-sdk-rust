...
| Upstream commit | `ae3c833acd4f44ab84db65b44eb2ba2646eaecf9` |
| Upstream commit date | `2026-06-01T09:07:50Z` |
...
The current inventory contains zero `needs-review` rows. Any future upstream
drift that initially lands as `needs-review` is blocking inventory debt, not an
exclusion, and must be reclassified as portable, `js-only-documented`, or
`type-system-impossible` before the drift audit can complete.
...
Refresh: `npx opensrc fetch https://github.com/vercel/workflow`

Compared upstream `1ee63b870afbf9754eb1022b1bb5f02d0ab042f9` to current HEAD
`ae3c833acd4f44ab84db65b44eb2ba2646eaecf9`.

Drift found:

- `packages/core/e2e/event-log-race-repro.test.ts` changed diagnostic labeling
  for the CI stress repro.
- No package manifests were added, renamed, or removed.
- No test files or generated inventory rows were added, renamed, or removed;
  the inventory remains 2,679 rows across 22 summary packages.
- The generated inventory now records zero `needs-review` rows.
...
| --- | --- | --- | --- | --- | --- | --- |
| `packages/ai` (`@workflow/ai`) | `5.0.0-beta.6` | portable Rust runtime | verified | `crates/workflow-ai` | Source: `src/index.ts`, `src/workflow-chat-transport.ts`, `src/stream-iterator.ts`, `src/agent/*.ts`, `src/providers/*.ts`. Tests: 10 files including `src/agent/durable-agent.test.ts`, `src/agent/do-stream-step.test.ts`, `src/workflow-chat-transport.test.ts`. | Package-owned `workflow-ai` crate bridges to the Rust AI SDK/chat-sdk compatible internals and maps all 181 portable upstream cases to named Rust tests; 3 JavaScript host/legacy compatibility rows are documented as JS-only and 3 TypeScript inference-only rows are classified as type-system-impossible. |
...
| `packages/next` (`@workflow/next`) | `5.0.0-beta.10` | host-framework binding | js-only-documented | none | Source: `src/index.ts`, `src/builder.ts`, `src/builder-deferred.ts`, `src/builder-eager.ts`, `src/runtime.ts`, `src/loader.ts`, `src/socket-server.ts`. Tests: `src/builder.test.ts`, `src/index.test.ts`. | Sixteen rows cover Next.js env, builder, webpack, and externalization integration. No Rust runtime counterpart is claimed. |
| `packages/nitro` (`@workflow/nitro`) | `5.0.0-beta.10` | host-framework binding | js-only-documented | none | Source: `src/index.ts`, `src/builders.ts`, `src/types.ts`, `src/vite.ts`. Tests: `src/index.test.ts`. | Nineteen rows cover Nitro virtual handlers, Vercel functionRules, version routing, and externals forwarding. No Rust runtime counterpart is claimed. |
| `packages/nuxt` (`@workflow/nuxt`) | `5.0.0-beta.10` | host-framework binding | js-only-documented | none | Source: `src/module.ts`. Tests: none. | Nuxt module wrapper with no upstream test rows. No Rust runtime counterpart is claimed unless a future Nuxt host adapter becomes an explicit Rust surface. |
...
| `packages/sveltekit` (`@workflow/sveltekit`) | `5.0.0-beta.10` | host-framework binding | js-only-documented | none | Source: `src/index.ts`, `src/builder.ts`, `src/plugin.ts`, `src/vc-config.ts`. Tests: `src/vc-config.test.ts`. | Three rows cover SvelteKit/Vercel config queue-trigger filtering. No Rust runtime counterpart is claimed. |
...
| `packages/tsconfig` (`@workflow/tsconfig`) | `5.0.0-beta.0` | TypeScript config | type-system-impossible | none | Source: `base.json`. Tests: none. | TypeScript compiler configuration package only; Rust has no runtime or compile-test analogue for this package. |
...
| `packages/vitest` (`@workflow/vitest`) | `5.0.0-beta.10` | docs/test-only | js-only-documented | none | Source: `src/index.ts`, `src/options.ts`, `src/global-setup.ts`, `src/setup-file.ts`, `src/vitest-context.d.ts`. Tests: `src/index.test.ts`. | Five rows cover JavaScript Vitest harness setup/options and generated local-world test directories. Outside Rust runtime parity. |
...
| `workflow-world` | `packages/world` (`@workflow/world`) | verified | Shared World contracts, all 23 portable upstream attributes tests, and portable `packages/web-shared` event ID, event duration, and v1/v2 trace data-contract tests. |
...
1. Continue with the host classification and parity gate buckets now that
   runtime, AI, tooling, shared, local, Postgres, and Vercel world contracts are
   merged.
...
3. Implement or otherwise close the 84 portable `core` E2E rows now classified
   as `not-started`; no `needs-review` rows remain in the Workflow inventory.
4. Host/web/docs packages listed as `js-only-documented` above are classified
   debt-free for Rust runtime parity; do not reopen them unless a Rust host
   adapter or UI crate becomes an explicit target.
