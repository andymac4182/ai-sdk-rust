...
| `packages/core` (`@workflow/core`) | `5.0.0-beta.10` | portable Rust runtime | in-progress | `crates/workflow-core` | Source: `runtime.js`, `runtime.d.ts`, `src/workflow.ts`, `src/step.ts`, `src/runtime/*.ts`, `src/serialization/*.ts`, `src/vm/*.ts`, `e2e/*.ts`. Tests: 52 files including `src/workflow.test.ts`, `src/step.test.ts`, `src/runtime/start.test.ts`, `src/serialization/serialization.test.ts`, `src/vm/index.test.ts`, and e2e tests. | WF 05 ports the portable serialization, encryption, stream framing, observability hydration, async-deserialization ordering, UUID, and base64/hex utility rows. Runtime execution, workflow/step orchestration, and host/VM-only rows remain follow-up work. |
...
| `workflow-core` | `packages/core` (`@workflow/core`) | in-progress | Core ownership marker plus WF 05 portable serialization, encryption, stream framing, observability hydration, ordering, UUID, and base64/hex utilities. |
