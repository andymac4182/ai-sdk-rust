...
| `packages/nest` (`@workflow/nest`) | `5.0.0-beta.10` | host-framework binding | js-only-documented | none | Source: `src/index.ts`, `src/builder.ts`, `src/cli.ts`, `src/workflow.controller.ts`, `src/workflow.module.ts`. Tests: `src/cjs-rewrite.test.ts`, `src/parse-module-type.test.ts`. | Twenty-one rows cover NestJS CJS rewrite, dist path mapping, and TypeScript module parsing for the JS adapter. No Rust runtime counterpart is claimed. |
...
| `packages/vite` (`@workflow/vite`) | `5.0.0-beta.10` | host/build adapter | js-only-documented | none | Source: `src/index.ts`, `src/hot-update.ts`. Tests: none. | Vite plugin wrapper around JS build transforms and hot updates. No Rust runtime counterpart is claimed. |
...
| `workflow-world` | `packages/world` (`@workflow/world`) | in-progress | World interface ownership marker plus portable `packages/web-shared` event ID, event duration, and v1/v2 trace data-contract tests. |
...
6. Host/web/docs packages listed as `js-only-documented` above are classified
