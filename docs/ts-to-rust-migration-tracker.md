...
| Just Bash | JB-01 added the current-upstream inventory ledger and gate at `d64009aef6bc1556e7c84b22ed455863275ea953`. JB-03 adds the portable virtual filesystem/path/encoding backend crate. JB-02 layers the reusable core session/executor contract onto `crates/just-bash` without replacing Open Agents runtime wiring. The ledger now maps 159 exact upstream rows to named Rust tests; 9,777 rows remain `portable-pending`, so the strict gate is still intentionally open. | `crates/just-bash`; `docs/open-agents/just-bash-parity.md`; `node scripts/just-bash-test-inventory.mjs --check`; `cargo test -p just-bash` | Sibling Just Bash implementation threads must convert remaining owned rows to `portable-verified` with named Rust tests or explicit exceptions, then make `node scripts/just-bash-test-inventory.mjs --strict` pass. |
...
| JB-02 | `019e7d5d-6e30-72d3-813e-e6f7c72014db` | Just Bash core API and executor contract |
| JB-03 | `019e7d5d-6e30-72d3-813e-e6f7c72014db` | Just Bash filesystem/path/encoding backend |
