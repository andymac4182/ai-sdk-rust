...
| AI SDK | The strict row-level inventory is still open after refreshing upstream to `43e84c8e39e540aa23e25986031183227a77d531`: after AIS-10 landed, the current gate reports 9,013 cases, 4,583 portable mapped rows, 3,104 portable unmapped rows, and 1,326 exceptions. AIS-08, AIS-09, AIS-11, and AIS-12 remain active against the largest remaining buckets: generate-text/object/output, UI/prompt conversion, AI `packages/workflow`, and concrete provider gaps. | `docs/upstream-parity.md`, `docs/package-progress.md`, `docs/ai-strict-test-inventory.md`, `node scripts/ai-strict-test-inventory.mjs --check`, `scripts/master-parity-gate.sh --check` | Land AIS-08, AIS-09, AIS-11, and AIS-12, then continue converting every remaining portable row to a named Rust test or explicit exception. Closure requires zero portable-unmapped rows in `docs/ai-strict-test-inventory.md`; package-level progress alone is not a 1:1 claim. |
...
| AIS-11 | active | AI strict packages/workflow closure | Close the AI SDK `packages/workflow` rows, distinct from the standalone Workflow SDK gate, with named tests for workflow-agent, workflow-agent-compat, workflow-chat-transport, callbacks, runtime/tools context, reconnect, UI-message collection, telemetry, and error formatting. | Fresh upstream fetch; focused workflow tests; owning crate tests; strict inventory check; master gate where feasible; fmt/clippy/naming/diff gates. |
