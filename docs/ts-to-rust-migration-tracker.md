...
| JBC-18 | active | CLI, package entrypoint, and distribution closure | Close portable CLI/package rows for argument parsing, command invocation shape, CJS/ESM package entry behavior, help/version output, and documented JS-only packaging exceptions where runtime Rust parity is impossible. | CLI fixture tests or NAPI harness tests; `cargo test -p just-bash`; `npm test --prefix crates/just-bash-napi` where relevant; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
