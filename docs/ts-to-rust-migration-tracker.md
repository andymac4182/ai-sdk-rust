...
| AI SDK | In progress. Package progress is 95.2% average, 48 of 52 package rows closed, 38 of 42 portable rows strictly verified, 4 in progress, and 0 not started. | `docs/upstream-parity.md`, `docs/package-progress.md` | Complete every in-progress provider package with package-owned tests, then regenerate progress to 100% portable verified or explicitly documented JS-only/type-system exceptions. |
...
| AI-02 | active | AI SDK OpenAI-compatible major providers | Initial provider foundations for `@ai-sdk/xai`, `@ai-sdk/groq`, `@ai-sdk/cohere`, `@ai-sdk/fireworks`, and `@ai-sdk/togetherai` landed. TogetherAI is verified; xAI, Groq, Cohere, and Fireworks remain in-progress under AI-02C through AI-02F. | Package-owned tests plus shared OpenAI-compatible regression tests where applicable; progress regeneration. |
...
| AI-02F | active | Fireworks provider full closure | Map every portable upstream `@ai-sdk/fireworks` case to named Rust tests or explicit exceptions, including factory split/options, unsupported families, image error splits, edit input variants, multi-poll timing, and failure/timeout metadata. | `cargo test fireworks_ --all-features`; package row mapping; progress regeneration; master parity gate. |
