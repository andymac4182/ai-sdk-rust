...
| Just Bash | JB-01 through JB-07 landed the current-upstream inventory gate, in-memory filesystem/path/encoding backend, reusable session/executor contract, parser/interpreter slice, command/runtime slice, security/network policy seams, and default Open Agents Just Bash backend wiring. JBC-01 through JBC-08 landed the NAPI bridge, JS dual-engine harness, generated conformance corpus, Rust corpus runner, CI gate wiring, core command parity, text/search/structured command parity, and Open Agents service conformance proof. JBC-09 through JBC-12 then landed AWK row closure, exact ripgrep row closure, stronger generated conformance-corpus row closure, and syntax/transform row closure. JBC-13 landed the advanced filesystem adapters and exact overlay/read-write/mountable filesystem row closure. JBC-14 landed security/sandbox/fuzz/prototype-pollution row closure with narrow worker/browser JS-only classifications. JBC-15 landed interpreter core, builtins, and expansion row closure for builtin dispatch, scoped variables, command substitution, arithmetic, arrays, aliases/functions, loops, pipefail/status, and diagnostics. JBC-16 landed structured/data command closure for bounded `jq`, `yq`, `xan`, and `sqlite3` behavior. JBC-17 landed executor-package and public example behavior through the shared session/executor API, NAPI-backed JS smoke coverage, and explicit no-host-shell semantics. JBC-18 landed focused CLI/package closure for argument parsing, invocation shape, help/version output, CJS/ESM NAPI entrypoints, and explicit JS-only distribution exceptions. The ledger now maps 1,941 exact upstream rows to named Rust tests, NAPI-backed JS proofs, or generated corpus proofs; 7,836 rows remain `portable-pending`, 159 rows are documented JS-only, and 7,847 strict gaps remain, so strict parity is still intentionally open. JBC-19 through JBC-32 are the remaining tracked Just Bash closure wave. | `crates/just-bash`; `crates/open-agents-*`; `docs/open-agents/just-bash-parity.md`; `docs/open-agents/just-bash-conformance.md`; `docs/just-bash-upstream-parity.md`; `node scripts/just-bash-test-inventory.mjs --check`; `node scripts/just-bash-conformance-corpus.mjs --check`; `cargo test -p just-bash`; `scripts/open-agents-local-e2e.sh --just-bash-conformance` | Dispatch and land JBC-19 through JBC-32, converting remaining owned rows to `portable-verified` with named Rust tests, NAPI-backed JS proofs, generated corpus proofs, or explicit exceptions. Closure requires `node scripts/just-bash-test-inventory.mjs --strict` and the shared conformance harness passing against both TypeScript and Rust engines. |
...
| JBC-19 | active | Shell syntax, transform, heredoc, and plugin pipeline closure | Close remaining parser/serializer/transform/plugin/heredoc rows, including AST round trips, transform metadata, plugin ordering, quoting, redirection, here-doc execution equivalence, and shell pipeline edge cases. | Focused parser/transform tests; `cargo test -p just-bash`; corpus subset for syntax/transform; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
| JBC-20 | active | Core runtime, environment, and session semantics closure | Close remaining core/runtime rows for cwd/env persistence, scoped variables, command status, trap-like diagnostics where portable, cancellation/time limits, error formatting, sandbox result metadata, and comparison rows that exercise core execution semantics. | Focused runtime/session tests; `cargo test -p just-bash`; conformance corpus subset; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
| JBC-21 | active | Small POSIX-style command family closure | Close portable rows for small bounded commands such as `base64`, `basename`, `chmod`, `clear`, `column`, `comm`, `date`, `dirname`, `du`, `expand`, `file`, `fold`, `gzip`, `join`, `ln`, `md5sum`, `nl`, `od`, `paste`, `readlink`, `rev`, `seq`, `sleep`, `split`, `stat`, `strings`, `tac`, `tar`, `tee`, `test`, `time`, `timeout`, `tree`, `true`, `which`, `whoami`, and `xargs`. | Focused command-family tests; `cargo test -p just-bash`; conformance corpus command subsets; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
| JBC-22 | active | Find, curl, network, and external resource seams closure | Close portable `find`, `curl`, network policy, and resource-fetch rows with deterministic in-memory or fake HTTP transports, while narrowly documenting browser/worker/live-network-only rows as JS-only only when proven. | Fake network/resource tests; focused `find`/`curl` tests; `cargo test -p just-bash`; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
| JBC-23 | active | Text command deep parity closure | Close remaining portable rows for `grep`, `sed`, `sort`, `printf`, `echo`, `head`, `tail`, `cut`, `tr`, `uniq`, `wc`, and related text-stream behavior, preserving exact stdout/stderr/exit-code expectations. | Focused text command tests; conformance corpus text subsets; `cargo test -p just-bash`; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
| JBC-24 | active | Structured data command deep parity closure | Continue exact row closure for the larger `jq`, `yq`, `xan`, `sqlite3`, query-engine, and search-engine surfaces beyond the bounded JBC-16 slice, using deterministic in-memory data fixtures. | Focused structured/data tests; corpus structured subsets; `cargo test -p just-bash`; inventory/corpus checks; shared fmt/clippy/naming/diff gates. |
