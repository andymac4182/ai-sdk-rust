...
| AI SDK | The strict row-level inventory is still open after refreshing upstream to `43e84c8e39e540aa23e25986031183227a77d531`: the current gate reports 9,013 cases, 4,181 portable mapped rows, 3,548 portable unmapped rows, and 1,284 exceptions. AIS-07 through AIS-12 are active against the largest remaining buckets: stream-text, generate-text/object/output, UI/prompt conversion, MCP/OTEL, AI `packages/workflow`, and concrete provider gaps. | `docs/upstream-parity.md`, `docs/package-progress.md`, `docs/ai-strict-test-inventory.md`, `node scripts/ai-strict-test-inventory.mjs --check`, `scripts/master-parity-gate.sh --check` | Land AIS-07 through AIS-12, then continue converting every remaining portable row to a named Rust test or explicit exception. Closure requires zero portable-unmapped rows in `docs/ai-strict-test-inventory.md`; package-level progress alone is not a 1:1 claim. |
...
| AIS-07 | active | AI strict stream-text row closure | Close the largest `packages/ai` strict cluster in `packages/ai/src/generate-text/stream-text.test.ts` with row-level Rust tests for stream request assembly, stream parts, tool calls/results, finish reasons, abort/error behavior, usage, telemetry, and message/UI outputs. | Fresh `npx opensrc fetch https://github.com/vercel/ai`; focused stream-text tests; owning crate tests; `node scripts/ai-strict-test-inventory.mjs --check`; `scripts/master-parity-gate.sh --check`; fmt/clippy/naming/diff gates. |
