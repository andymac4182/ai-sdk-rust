...
| AI SDK | The strict row-level inventory is still open after refreshing upstream to `43e84c8e39e540aa23e25986031183227a77d531`: after AIS-07 landed, the current gate reports 9,013 cases, 4,377 portable mapped rows, 3,352 portable unmapped rows, and 1,284 exceptions. AIS-08 through AIS-12 are active against the largest remaining buckets: generate-text/object/output, UI/prompt conversion, MCP/OTEL, AI `packages/workflow`, and concrete provider gaps. | `docs/upstream-parity.md`, `docs/package-progress.md`, `docs/ai-strict-test-inventory.md`, `node scripts/ai-strict-test-inventory.mjs --check`, `scripts/master-parity-gate.sh --check` | Land AIS-08 through AIS-12, then continue converting every remaining portable row to a named Rust test or explicit exception. Closure requires zero portable-unmapped rows in `docs/ai-strict-test-inventory.md`; package-level progress alone is not a 1:1 claim. |
...
| AIS-10 | active | AI strict MCP and OTEL closure | Close `packages/mcp` OAuth/transport rows and `packages/otel` legacy/current telemetry rows with fake transports and local deterministic telemetry fixtures. | Fresh upstream fetch; `cargo test -p ai-sdk-mcp -p ai-sdk-otel` or focused equivalents; strict inventory check; master gate where feasible; fmt/clippy/naming/diff gates. |
