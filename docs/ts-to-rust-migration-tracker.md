...
| AI-02 | active | AI SDK OpenAI-compatible major providers | Initial provider foundations for `@ai-sdk/xai`, `@ai-sdk/groq`, `@ai-sdk/cohere`, `@ai-sdk/fireworks`, and `@ai-sdk/togetherai` landed; all five package rows remain in-progress. AI-02C through AI-02G own per-provider full closure. | Package-owned tests plus shared OpenAI-compatible regression tests where applicable; progress regeneration. |
...
| AI-02G | active | TogetherAI provider full closure | Map every portable upstream `@ai-sdk/togetherai` case to named Rust tests or explicit exceptions, finishing validation/error/deprecation/live-proof gaps from the AI-02B exact case map. | `cargo test togetherai_ --all-features`; package row mapping; progress regeneration; master parity gate. |
