...
| --- | --- | --- | --- |
| Open Agents | In progress with the OA-01 inventory gate in place. The Rust service has Slack webhook ingress, real Vercel AI Gateway runtime config, Vercel sandbox support, deployment docs, and a generated upstream inventory covering all 6 package manifests, 461 source files, and 106 test files. OA-03B owns the remaining proof that Slack events complete through the real async Vercel AI Gateway runtime rather than only reporting Gateway configuration. | `docs/open-agents/upstream-parity.md`, `scripts/open-agents-test-inventory.mjs`, `docs/open-agents/bucket-ownership.md`, `docs/open-agents/slack-remote-agent-architecture.md`, `docs/open-agents/deployment-verification.md`, `crates/open-agents-*` | Close the remaining real Gateway runtime proof gap with deterministic async coverage and an ignored live Gateway smoke path, then keep the Open Agents gate green. |
...
| AI SDK | In progress. Package progress is 92.7% average, 46 of 52 package rows closed, 36 of 42 portable rows strictly verified, 6 in progress, and 0 not started. | `docs/upstream-parity.md`, `docs/package-progress.md` | Complete every in-progress and not-started provider package with package-owned tests, then regenerate progress to 100% portable verified or explicitly documented JS-only/type-system exceptions. |
...
| OA-03B | active | Open Agents real Gateway runtime proof | Replace the one-poll `poll_ready` Gateway path with a real async execution path, add deterministic Pending-then-Ready coverage, and preserve fixture/durable Slack behavior. | `cargo test -p open-agents-service`; `cargo test -p open-agents-runtime` if runtime traits change; `scripts/master-parity-gate.sh`; ignored live Gateway smoke when credentials exist. |
...
| AI-03B | active | AI SDK media provider full parity closure | Close the remaining `@ai-sdk/black-forest-labs`, `@ai-sdk/luma`, `@ai-sdk/fal`, `@ai-sdk/klingai`, `@ai-sdk/prodia`, and `@ai-sdk/replicate` rows after AI-03's initial media slice. | Focused tests for all six provider crates; package row mapping in `docs/upstream-parity.md`; regenerated `docs/package-progress.md`; `scripts/master-parity-gate.sh`. |
...
| AI-07 | active | AI SDK Alibaba provider | Initial `crates/ai-sdk-alibaba` crate landed with named chat/video tests and package progress moved to 70%. AI-07B owns closing the remaining row-level gaps before any verified claim. | Package-owned Alibaba tests, explicit exceptions for non-portable rows, and progress regeneration. |
| AI-07B | active | AI SDK Alibaba verified closure | Map every remaining portable Alibaba upstream case to named Rust tests or explicit exceptions and move the package to verified only when row-level evidence supports it. | `cargo test -p ai-sdk-alibaba`; regenerated `docs/package-progress.md`; `scripts/master-parity-gate.sh`. |
