//! Convert a chat-sdk [`CardElement`] to Linear-flavored markdown.
//!
//! 1:1 port of `packages/adapter-linear/src/cards.ts`. Linear
...
//! buttons (no interactivity in Linear).
...
/// Convert a [`CardElement`] to Linear-flavored markdown. 1:1 port
/// of upstream `cardToLinearMarkdown(card)`.
pub fn card_to_linear_markdown(card: &CardElement) -> String {
...
mod tests {
    ...
    // ---------- cardToLinearMarkdown: 6 ported upstream cases ----------

    #[test]
    fn should_render_a_simple_card_with_title() {
        let result = card_to_linear_markdown(&empty_card("Hello World"));
    ...
    fn should_render_card_with_title_and_subtitle() {
        ...
        let result = card_to_linear_markdown(&card);
    ...
    fn renders_card_link_as_markdown_link() {
        ...
        let markdown = card_to_linear_markdown(&card);
    ...
    fn empty_card_renders_to_empty_string() {
        ...
        assert_eq!(card_to_linear_markdown(&card), "");
    }
