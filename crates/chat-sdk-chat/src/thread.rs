...
impl SentMessage {
    ...
    /// and the adapter that posted it. 1:1 with upstream's
    /// `new SentMessage(message, adapter)` shape.
    ///
    /// Used by [`crate::channel::Channel::post_sent_message`] (slice 487) to
    /// wrap the synthesized Message returned from
