    ...
    pub fn events_create(
        ...
        if let Some(run) = &current_run {
            if let Some(spec_version) = run.spec_version {
                if spec_version > world::SPEC_VERSION_CURRENT {
                    return Err(LocalWorldError::RunNotSupported {
                        required: spec_version,
                        current: world::SPEC_VERSION_CURRENT,
                    });
                }
            }
        ...
        let spec_version = Some(world::SPEC_VERSION_CURRENT);
        let mut event = Event {
...
impl world::World for LocalWorld {
    type Error = LocalWorldError;

    fn spec_version(&self) -> u32 {
        world::SPEC_VERSION_CURRENT
    }
...
mod tests {
    ...
    fn world_local_reenqueue_active_runs_portable_parity() {
        ...
        assert_eq!(world.spec_version(), world::SPEC_VERSION_CURRENT);
        world.close().unwrap();
    }
